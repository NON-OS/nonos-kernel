...
pub fn draw_progress_bar(x: u32, y: u32, width: u32, height: u32) {
    let bg_color = 0xFF2D3F4Fu32;
    let fill_color = 0xFF00D469u32;
    let shimmer_color = 0xFFFFFFFFu32;

    fill_rect(x, y, width, height, bg_color);

    let fill_width = (width as f32 * unsafe { PROGRESS_PERCENT } / 100.0) as u32;
    if fill_width > 4 {
        fill_rect(x + 2, y + 2, fill_width - 4, height - 4, fill_color);
        ...
        if shimmer_x < x + fill_width && shimmer_x + 20 > x {
            for i in 0..20 {
                let alpha = 255 * (20 - i) / 20;
                let color = ((alpha << 24) | (shimmer_color & 0x00FFFFFF)) as u32;
                if shimmer_x + i < x + fill_width {
                    fill_rect(shimmer_x + i, y + 2, 1, height - 4, color);
                }
            }
        }
    }
    ...
    draw_string(x + width + 20, y + 8, percentage_text.as_bytes(), COLOR_TEXT_WHITE);
}
