...
pub fn map(device_id: u64, claim_epoch: u64, bar_size: u64) -> NvmeResult<MmioMapOut> {
    let mut out = MmioMapOut { user_va: 0, length: 0, grant_id: 0 };
    let r = mk_mmio_map(device_id, claim_epoch, NVME_BAR_INDEX as u32, 0, 0, bar_size, &mut out);
    if r < 0 {
