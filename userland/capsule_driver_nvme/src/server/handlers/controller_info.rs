...
pub fn handle(driver: &Driver, req: &Request, tx: &mut [u8]) {
    ...
    tx[o..o + 8].copy_from_slice(&info.cap.to_le_bytes());
    o += 8;
    for v in [
        info.version,
        info.cc,
    ...
    tx[o..o + 2].copy_from_slice(&info.max_queue_entries().to_le_bytes());
    o += 2;
    tx[o] = info.timeout_units();
    tx[o + 1] = info.doorbell_stride();
    tx[o + 2] = info.min_page_shift();
    tx[o + 3] = info.max_page_shift();
    tx[o + 4] = info.nvm_supported() as u8;
    tx[o + 5] = info.ready() as u8;
    tx[o + 6] = info.fatal() as u8;
    tx[o + 7] = 0;
    tx[o + 8] = 0;
    tx[o + 9] = 0;
