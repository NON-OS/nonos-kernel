...
pub fn render_dropdown(
	buf: &mut [u32], stride: usize, w: u32, h: u32, x: u32, y: u32, rw: u32, row_h: u32, selected: &[u8], expanded: bool, options: &[&[u8]], index: usize, style: DropdownStyle,
) {
	fill_rect(buf, stride, w, h, x, y, rw, row_h, style.bg.as_u32());
	draw_text(buf, stride, w, h, x.saturating_add(4), y.saturating_add(4), selected, style.fg.as_u32());
	if !expanded {
		return;
	}
	let mut i = 0usize;
