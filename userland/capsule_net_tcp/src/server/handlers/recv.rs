...
use crate::protocol::{E_BAD_LEN, E_NO_SOCKET, E_OK, E_RX_EMPTY, OP_RECV};
...
pub fn handle(sender_pid: u32, req: &Request, body: &[u8], tx: &mut [u8]) {
    ...
    if TABLE.lock().owned_mut(sender_pid, handle).is_none() {
        return status(sender_pid, req, E_NO_SOCKET, tx);
    }
...
fn answer(sender_pid: u32, req: &Request, payload: &[u8], tx: &mut [u8]) {
    tx[20..20 + payload.len()].copy_from_slice(payload);
    let _ = respond(sender_pid, OP_RECV, E_OK, req.request_id, payload.len() as u32, tx);
}

fn status(sender_pid: u32, req: &Request, errno: u16, tx: &mut [u8]) {
    let _ = respond(sender_pid, OP_RECV, errno, req.request_id, 0, tx);
}
