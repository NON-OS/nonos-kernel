...
use crate::constants::AHCI_ABAR_BAR;
use crate::error::{AhciError, AhciResult};

pub fn map(device_id: u64, claim_epoch: u64, abar_size: u64) -> AhciResult<MmioMapOut> {
    let mut out = MmioMapOut { user_va: 0, length: 0, grant_id: 0 };
    let r = mk_mmio_map(device_id, claim_epoch, AHCI_ABAR_BAR as u32, 0, 0, abar_size, &mut out);
    if r < 0 {
