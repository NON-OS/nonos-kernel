...
use crate::protocol::{
    decode_request, E_INVAL, HDR_LEN, OP_CONTROLLER_INFO, OP_HEALTHCHECK, OP_PORT_LIST,
    PORT_ENTRY_BYTES, PORT_LIST_HEADER_BYTES, RESP_HDR_LEN, SERVICE_NAME,
};
use crate::server::{error, handlers};
use crate::setup::Driver;

const TX_LEN: usize = RESP_HDR_LEN + 4 + PORT_LIST_HEADER_BYTES + MAX_PORTS * PORT_ENTRY_BYTES;
...
pub fn run(driver: Driver) -> ! {
    ...
    loop {
        ...
        if mk_irq_poll(driver.handles.irq_grant_id(), &mut irq as *mut _) >= 0
            && irq.seq != last_irq_seq
        {
            last_irq_seq = irq.seq;
        ...
        match req.op {
            ...
            OP_PORT_LIST => handlers::port_list::handle(&driver, &req, &mut tx),
