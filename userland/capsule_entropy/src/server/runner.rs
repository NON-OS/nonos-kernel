...
pub fn run() -> ! {
    ...
    loop {
        ...
        let resp = match decode_request(&buf[..n]) {
            Ok(req) => dispatch(&pool, req),
            // Malformed envelope — emit a structurally valid error reply
            // so the kernel client gets EINVAL rather than a transport
            // timeout.
