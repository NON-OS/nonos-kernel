...
// Decode a request frame. Returns `Err` on any structural problem so
// the server can map to EINVAL/EMSGSIZE. Never panics; never unwraps.
pub fn decode_request(buf: &[u8]) -> Result<Request<'_>, DecodeError> {
    ...
    Ok(Request { op, flags, request_id, payload })
}
