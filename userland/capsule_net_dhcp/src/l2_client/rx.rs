...
const ERRNO_RX_EMPTY: u16 = 8;
const ERRNO_NO_LINK: u16 = 5;
...
// Drain one inbound frame. DHCP's caller filters for the DHCP
// echo signature (BOOTP cookie + matching XID) and ignores
// everything else.
pub fn poll_frame(l2_port: u32) -> Result<Vec<u8>, RxError> {
    ...
    if errno == ERRNO_RX_EMPTY {
        return Err(RxError::Empty);
    }
    if errno == ERRNO_NO_LINK {
