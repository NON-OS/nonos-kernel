...
pub fn handle(driver: &Driver, req: &Request, tx: &mut [u8]) {
    ...
    tx[o..o + 2].copy_from_slice(&mask.to_le_bytes());
    tx[o + 2..o + 4].copy_from_slice(&0u16.to_le_bytes());
    tx[o + 4..o + 8].copy_from_slice(&(mask.count_ones()).to_le_bytes());
    let _ = mk_ipc_send(KERNEL_REPLY_ENDPOINT, tx.as_ptr(), RESP_HDR_LEN + 12);
}
