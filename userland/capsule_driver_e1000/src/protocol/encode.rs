...
//! Response encoder. The header echoes op/flags/request_id from the
//! request so the kernel client can match the reply; the first
//! four bytes after the header are an `i32` status, with per-op
//! data following.
