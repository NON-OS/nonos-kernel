...
//! `good_msix_info()` reports `table_size = 7`, so `vector_count`
//! is 8. Asking for 9 must be rejected before any slot allocation
//! or MSI-X programming runs.
...
#[test]
fn vector_count_over_device_table_rejects() {
    let _g = TEST_LOCK.lock();
    let epoch = fresh(&FAKE);
    let err = bind(PID, msix_request(epoch, 9)).unwrap_err();
