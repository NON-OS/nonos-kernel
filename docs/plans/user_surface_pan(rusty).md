...
mk_service_lookup("wm"), mk_service_lookup("compositor"), mk_service_lookup("input.router").
OP_WINDOW_OPEN against wm → window_id.
OP_INPUT_SUBSCRIBE against compositor for the matching layer.
Loop: mk_ipc_recv_from input events from compositor, update app state, render with toolkit into a surface, OP_SCENE_SUBMIT to compositor with damage rect, repeat.
...
make -B nonos-mk-<slug> builds with zero warnings, three triples.
make nonos-mk-<slug>-sign produces cert + manifest.
Row in docs/production-roadmap/capsule_integration_matrix.md.
README contract section in the capsule's own README.
New gate in nonos-ci/run-static-checks.sh pinning the new architectural rule.
Boot integration: init/entry.rs cfg-gated spawn call + spawn shim, ordered after the substrate capsules.
QEMU serial trace shows the capsule joining the chain and answering at least OP_HEALTHCHECK from a probe driver.
...
Order of attack
Toolkit (font + design + image first slice). Unblocks every later deliverable.
capsule_image_codec.
capsule_wallpaper (real).
capsule_clipboard (small, fast win).
capsule_login.
capsule_desktop_shell.
App wave 1 in parallel where possible (about + calculator first; terminal and file_manager carry more state).
Don't do
