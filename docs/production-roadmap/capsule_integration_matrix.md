...
| 17 | `userland/capsule_driver_e1000` | `driver_e1000` | `nonos-mk-driver-e1000` | `driver.e1000_0` | `nonos-capsule-driver-e1000` | `src/hardware/e1000_capsule/` | `src/userspace/init/entry.rs` (under feature) | `src/hardware/e1000_capsule/client/` (`healthcheck`, `link_status`, `mac_address`, `tx_packet`, `rx_packet`) | `microkernel-driver-e1000-smoketest` profile (boot smoke `tests/boot/e1000_round_trip.sh` not yet written); INTx-only on bring-up, MSI-X migration is a future slice now that 5B.2 landed the broker primitive | embedded | 0 | cert + manifest pair pending host signer ceremony; QEMU smoke not yet written | run `make nonos-mk-driver-e1000-sign` to produce the cert/manifest pair, then write `tests/boot/e1000_round_trip.sh` for the QEMU `-device e1000,netdev=user` round trip |
| 18 | `userland/capsule_driver_rtl8139` | `driver_rtl8139` | `nonos-mk-driver-rtl8139` | `driver.rtl8139_0` | n/a — kernel-side glue not yet wired | n/a yet | n/a — userland capsule lands first, kernel-side `src/hardware/rtl8139_capsule/` mirror is the follow-up slice | n/a yet | n/a — boot smoke not yet written; INTx-only on bring-up | build-only | 0 | kernel embed/spawn/client + boot smoke not written; cert + manifest pair pending host signer ceremony | mirror the e1000 layout in `src/hardware/rtl8139_capsule/`, then `make nonos-mk-driver-rtl8139-sign` to produce the cert/manifest pair and write `tests/boot/rtl8139_round_trip.sh` for the QEMU `-device rtl8139,netdev=user` round trip |
| 19 | `userland/capsule_driver_rtl8169` | `driver_rtl8169` | `nonos-mk-driver-rtl8169` | `driver.rtl8169_0` | n/a — kernel-side glue not yet wired | n/a yet | n/a — userland capsule lands first, kernel-side `src/hardware/rtl8169_capsule/` mirror is the follow-up slice | n/a yet | n/a — boot smoke not yet written; INTx-only on bring-up | build-only | 0 | kernel embed/spawn/client + boot smoke not written; cert + manifest pair pending host signer ceremony | mirror the e1000 layout in `src/hardware/rtl8169_capsule/`, then `make nonos-mk-driver-rtl8169-sign` to produce the cert/manifest pair and write `tests/boot/rtl8169_round_trip.sh` for the QEMU round trip (real RTL8169 hardware or compatible) |
| 20 | `userland/capsule_driver_ahci` | `driver_ahci` | `nonos-mk-driver-ahci` | `driver.ahci0` | n/a — kernel-side glue not yet wired | n/a yet | n/a — userland capsule lands first, kernel-side `src/hardware/ahci_capsule/` mirror is the follow-up slice | n/a yet | n/a — boot smoke not yet written; P0 maps ABAR, binds INTx, enables AHCI mode, and reports controller registers, port signatures, and live PxIS/PxCMD/PxTFD/PxSERR/PxSACT/PxCI status | build-only | 0 | kernel embed/spawn/client + boot smoke not written; cert + manifest pair pending host signer ceremony; command-list/FIS/PRDT DMA are P1 | mirror the e1000 layout in `src/hardware/ahci_capsule/`, then `make nonos-mk-driver-ahci-sign` to produce the cert/manifest pair and write `tests/boot/ahci_controller_probe.sh` for `-device ich9-ahci` |
| 21 | `userland/capsule_driver_hda` | `driver_hda` | `nonos-mk-driver-hda` | `driver.hda0` | n/a — kernel-side glue not yet wired | n/a yet | n/a — userland capsule lands first, kernel-side `src/hardware/hda_capsule/` mirror is the follow-up slice | n/a yet | n/a — boot smoke not yet written; P0 maps BAR0, binds the controller IRQ, leaves reset, and reports GCAP, codec mask, and GCAP-derived stream descriptor layout | build-only | 0 | kernel embed/spawn/client + boot smoke not written; cert + manifest pair pending host signer ceremony; CORB/RIRB + stream DMA are P1 | mirror the e1000 layout in `src/hardware/hda_capsule/`, then `make nonos-mk-driver-hda-sign` to produce the cert/manifest pair and write `tests/boot/hda_controller_probe.sh` for `-device intel-hda -device hda-output` |
| 22 | `userland/capsule_driver_nvme` | `driver_nvme` | `nonos-mk-driver-nvme` | `driver.nvme0` | n/a — kernel-side glue not yet wired | n/a yet | n/a — userland capsule lands first, kernel-side `src/hardware/nvme_capsule/` mirror is the follow-up slice | n/a yet | n/a — boot smoke not yet written; current slice maps BAR0, enables bus mastering, binds MSI-X, allocates admin queue DMA, enables the controller, reports Identify Controller plus Identify Namespace data for NSID 1, and exposes SMART / health log fields | build-only | 0 | kernel embed/spawn/client + boot smoke not written; cert + manifest pair pending host signer ceremony; IO queues and PRP/SGL block I/O are next | mirror the e1000 layout in `src/hardware/nvme_capsule/`, then `make nonos-mk-driver-nvme-sign` to produce the cert/manifest pair and write `tests/boot/nvme_controller_identify.sh` for `-device nvme` |
