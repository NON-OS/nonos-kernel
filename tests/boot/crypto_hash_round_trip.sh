...
#
# Boot-test harness for the crypto capsule hash surface. Boots the
# kernel with the crypto capsule + crypto-hash smoketest features on
# under QEMU, serial captured to a log file, grades the run by
# greping for deterministic markers.

set -euo pipefail

readonly REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
readonly LOG="${REPO_ROOT}/target/boot-test-crypto-hash.log"
...
echo "[harness] building brutal-minimum microkernel + capsules with crypto-hash smoketest"
make nonos-mk-crypto-hash-test >/dev/null
...
EXPECTED=(
    "[INIT] Starting"
    "[CRYPTO] capsule spawned"
    "[CRYPTO-HASH-TEST] blake3 ok"
    "[CRYPTO-HASH-TEST] sha3 ok"
    "[CRYPTO-HASH-TEST] sha256 ok"
    "[CRYPTO-HASH-TEST] sha512 ok"
    "[CRYPTO-HASH-TEST] PASS"
)

DEADLINE=$(( $(date +%s) + TIMEOUT_SECS ))
while [ $(date +%s) -lt ${DEADLINE} ]; do
    if grep -q '\[CRYPTO-HASH-TEST\] PASS' "${LOG}" 2>/dev/null; then
        break
    fi
    if grep -q '\[CRYPTO-HASH-TEST\] FAIL' "${LOG}" 2>/dev/null; then
        break
    fi
...
if grep -qF '[CRYPTO-HASH-TEST] FAIL' "${LOG}"; then
    echo "[harness] FAIL: smoketest reported failure"
    grep -F '[CRYPTO-HASH-TEST] FAIL' "${LOG}"
    exit 1
fi
...
echo "[harness] PASS: all markers seen, crypto-hash round trip verified"
exit 0
