...
#
# Boot-test harness for the VFS capsule path. Builds the kernel with
# the VFS capsule + smoketest features on, boots under QEMU, serial
# captured, grades the run by greping for deterministic markers.

set -euo pipefail

readonly REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
readonly LOG="${REPO_ROOT}/target/boot-test-vfs.log"
...
echo "[harness] building brutal-minimum microkernel + capsules with vfs smoketest"
make nonos-mk-vfs-test >/dev/null
...
while [ $(date +%s) -lt ${DEADLINE} ]; do
    if grep -q '\[VFS-TEST\] PASS' "${LOG}" 2>/dev/null; then
        break
    fi
    if grep -q '\[VFS-TEST\] FAIL' "${LOG}" 2>/dev/null; then
        break
    fi
...
if grep -qF '[VFS-TEST] FAIL' "${LOG}"; then
    echo "[harness] FAIL: smoketest reported failure"
    grep -F '[VFS-TEST] FAIL' "${LOG}"
    exit 1
fi
...
echo "[harness] PASS: all markers seen, vfs capsule round trip verified"
exit 0
