...
use super::helpers::{local_unix_secs, date_from_secs, DOW_NAMES, MON_NAMES};
...
pub fn format_date_short(buf: &mut [u8; 20]) -> usize {
    ...
    pos += 3;
    buf[pos] = b' '; pos += 1;
    if parts.day >= 10 { buf[pos] = b'0' + (parts.day / 10) as u8; pos += 1; }
    buf[pos] = b'0' + (parts.day % 10) as u8; pos += 1;
    buf[pos] = b' '; pos += 1;
    if hour_12 >= 10 { buf[pos] = b'0' + (hour_12 / 10); pos += 1; }
    buf[pos] = b'0' + (hour_12 % 10); pos += 1;
    buf[pos] = b':'; pos += 1;
    buf[pos] = b'0' + (time.minute / 10); pos += 1;
    buf[pos] = b'0' + (time.minute % 10); pos += 1;
    buf[pos] = am_pm[0]; pos += 1;
    buf[pos] = am_pm[1]; pos += 1;
    pos
}
