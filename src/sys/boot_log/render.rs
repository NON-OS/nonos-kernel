...
// Boot-log framebuffer renderer. Only compiled in the legacy tree
// because the microkernel build does not link `crate::display`. The
// microkernel boot log is serial-only; `write_line` is a no-op so
// `output.rs` calls remain valid in both builds.
...
mod fb {
    use super::super::state::{CHAR_HEIGHT, DISPLAY_ENABLED, LEFT_MARGIN, LOG_Y};
    ...
    pub(in super::super) fn write_line(_tag: &str, msg: &str, color: u32) {
