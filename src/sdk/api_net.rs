...
impl NetworkAccess for SdkApi {
    fn http_get(&self, url: &[u8]) -> AppResult<Vec<u8>> {
        if !self.has_permission(AppPermission::Network) { return Err(AppError::PermissionDenied); }
...
fn parse_url(url: &str) -> AppResult<(String, String)> {
    ...
    if host.is_empty() { return Err(AppError::InvalidInput); }
    Ok((host, path))
}

fn build_request(method: &[u8], host: &[u8], path: &[u8], body: &[u8]) -> Vec<u8> {
    let mut req = Vec::with_capacity(512 + body.len());
    req.extend_from_slice(method); req.extend_from_slice(b" ");
    req.extend_from_slice(path); req.extend_from_slice(b" HTTP/1.1\r\nHost: ");
    req.extend_from_slice(host); req.extend_from_slice(b"\r\nConnection: close\r\n");
    if !body.is_empty() {
        req.extend_from_slice(b"Content-Length: ");
        req.extend_from_slice(alloc::format!("{}", body.len()).as_bytes());
        req.extend_from_slice(b"\r\n\r\n"); req.extend_from_slice(body);
    } else { req.extend_from_slice(b"\r\n"); }
    req
}
