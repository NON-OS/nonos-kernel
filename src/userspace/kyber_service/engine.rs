...
use crate::crypto::pqc::quantum::{kyber768_keypair, kyber1024_keypair};
use crate::crypto::pqc::quantum::{kyber768_encapsulate, kyber1024_encapsulate};
use crate::crypto::pqc::quantum::{kyber768_decapsulate, kyber1024_decapsulate};
...
pub(super) fn keygen_768() -> Option<(Vec<u8>, Vec<u8>)> {
    ...
    if result.is_some() { KEYGEN_COUNT.fetch_add(1, Ordering::Relaxed); }
    result
}
...
pub(super) fn encapsulate_768(pk: &[u8]) -> Option<(Vec<u8>, Vec<u8>)> {
    ...
    if result.is_some() { ENCAPS_COUNT.fetch_add(1, Ordering::Relaxed); }
    result
}
...
pub(super) fn decapsulate_768(ct: &[u8], sk: &[u8]) -> Option<Vec<u8>> {
    ...
    if result.is_some() { DECAPS_COUNT.fetch_add(1, Ordering::Relaxed); }
    result
}
...
pub(super) fn get_stats() -> (u64, u64, u64) {
    (KEYGEN_COUNT.load(Ordering::Relaxed), ENCAPS_COUNT.load(Ordering::Relaxed), DECAPS_COUNT.load(Ordering::Relaxed))
}
