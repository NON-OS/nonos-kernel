...
use crate::crypto::chacha20poly1305::{chacha20_block, aead_encrypt_in_place, aead_decrypt_in_place};
...
pub(super) fn encrypt(key: &[u8; 32], nonce: &[u8; 12], counter: u32, data: &mut [u8]) {
    for (i, chunk) in data.chunks_mut(64).enumerate() {
        ...
        for (j, byte) in chunk.iter_mut().enumerate() { *byte ^= keystream[j]; }
    }
...
pub(super) fn encrypt_poly1305(key: &[u8; 32], nonce: &[u8; 12], data: &mut [u8], tag: &mut [u8; 16]) {
    let len = data.len();
...
pub(super) fn decrypt_poly1305(key: &[u8; 32], nonce: &[u8; 12], data: &mut [u8], tag: &[u8; 16]) -> bool {
    let len = data.len();
    ...
    if result { data.copy_from_slice(&buffer[..len]); }
    ENCRYPT_COUNT.fetch_add(1, Ordering::Relaxed);
    BYTES_PROCESSED.fetch_add(data.len() as u64, Ordering::Relaxed);
    result
}
