...
pub(super) fn sign(message: &[u8], sk: &[u8]) -> Option<Vec<u8>> {
    ...
    if result.is_some() { SIGN_COUNT.fetch_add(1, Ordering::Relaxed); }
    result
}
...
pub(super) fn get_stats() -> (u64, u64, u64) {
    (KEYGEN_COUNT.load(Ordering::Relaxed), SIGN_COUNT.load(Ordering::Relaxed), VERIFY_COUNT.load(Ordering::Relaxed))
}
