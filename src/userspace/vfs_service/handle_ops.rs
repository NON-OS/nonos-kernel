...
pub(super) fn handle_open_req(req: ServiceRequest) -> ServiceResponse {
    if req.payload.len() < 13 { return ServiceResponse::err(req.seq, ERR_INVAL); }
    let path_len = u32::from_le_bytes([req.payload[1], req.payload[2], req.payload[3], req.payload[4]]) as usize;
    let flags = i32::from_le_bytes([req.payload[5], req.payload[6], req.payload[7], req.payload[8]]);
    if req.payload.len() < 13 + path_len { return ServiceResponse::err(req.seq, ERR_INVAL); }
    let path_bytes = &req.payload[13..13 + path_len];
    let path = match core::str::from_utf8(path_bytes) {
        Ok(s) => s, Err(_) => return ServiceResponse::err(req.seq, ERR_INVAL),
    };
...
pub(super) fn handle_read_req(req: ServiceRequest) -> ServiceResponse {
    if req.payload.len() < 17 { return ServiceResponse::err(req.seq, ERR_INVAL); }
...
pub(super) fn handle_write_req(req: ServiceRequest) -> ServiceResponse {
    ...
    if req.payload.len() < 17 + len { return ServiceResponse::err(req.seq, ERR_INVAL); }
...
pub(super) fn handle_close_req(req: ServiceRequest) -> ServiceResponse {
    if req.payload.len() < 9 { return ServiceResponse::err(req.seq, ERR_INVAL); }
    let handle = u64::from_le_bytes(req.payload[1..9].try_into().unwrap_or([0; 8]));
    close_handle(handle);
