...
use super::ops::{app_list, app_resume, app_start, app_stop, app_suspend};
...
pub(super) fn handle_request(req: ServiceRequest) -> ServiceResponse {
    match req.op {
        ...
        ServiceOp::Ioctl => handle_apps_op(req),
        _ => ServiceResponse::err(req.seq, ERR_INVAL),
    }
}

fn handle_apps_op(req: ServiceRequest) -> ServiceResponse {
    ...
    match op {
        OP_START => app_start(req.seq, data),
        OP_STOP => app_stop(req.seq, data),
        OP_SUSPEND => app_suspend(req.seq, data),
        OP_RESUME => app_resume(req.seq, data),
        OP_LIST => app_list(req.seq),
        _ => ServiceResponse::err(req.seq, ERR_INVAL),
    }
}
