...
pub(super) fn verify_ecdsa(pubkey: &[u8; 33], message_hash: &[u8; 32], signature: &[u8; 64]) -> bool {
...
pub(super) fn get_stats() -> (u64, u64, u64, u64) {
    (SIGN_COUNT.load(Ordering::Relaxed), VERIFY_COUNT.load(Ordering::Relaxed),
     KEYGEN_COUNT.load(Ordering::Relaxed), ECDH_COUNT.load(Ordering::Relaxed))
}
