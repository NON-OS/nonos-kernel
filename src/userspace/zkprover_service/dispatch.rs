...
pub(super) fn process_request(data: &[u8]) -> [u8; 1024] {
    ...
    match data[0] {
        0x01 => handle_generate_proof(data, &mut response),
        0x02 => handle_verify_proof(data, &mut response),
...
fn handle_generate_proof(data: &[u8], resp: &mut [u8; 1024]) {
    if data.len() < 9 {
        resp[0] = 0xFE;
        return;
    }
...
fn handle_verify_proof(data: &[u8], resp: &mut [u8; 1024]) {
    if data.len() < 3 {
        resp[0] = 0xFE;
        return;
    }
    ...
    if let Ok(zk) = crate::zk_engine::get_zk_engine() {
        if let Ok(proof) = zk.deserialize_proof(proof_data) {
            let valid = engine::verify_proof(&proof);
            resp[0] = 0x01;
