...
use super::super::protocol::{build_get_report_command, build_set_report_command, build_set_idle_command, HidPowerState, HidReportType};
use super::utils::spin_wait;

impl I2cHidDevice {
    pub fn sleep(&mut self) -> Result<(), I2cHidError> { self.set_power(HidPowerState::Sleep) }
    pub fn wake(&mut self) -> Result<(), I2cHidError> { self.set_power(HidPowerState::On) }

    pub fn get_feature_report(&mut self, report_id: u8) -> Result<Vec<u8>, I2cHidError> {
        let cmd = build_get_report_command(self.hid_desc.command_register, self.hid_desc.data_register, HidReportType::Feature, report_id);
    ...
    pub fn set_feature_report(&mut self, report_id: u8, data: &[u8]) -> Result<(), I2cHidError> {
        let cmd = build_set_report_command(self.hid_desc.command_register, self.hid_desc.data_register, HidReportType::Feature, report_id, data);
        crate::drivers::i2c::write(self.controller, self.address, cmd[0], &cmd[1..])?;
        spin_wait(100);
        Ok(())
    }
