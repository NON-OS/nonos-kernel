...
pub(crate) fn try_parse_elan(data: &[u8], state: &mut TouchpadState) -> bool {
    ...
    if data[0] != 0x04 && data[0] != 0x0D { return false; }
    let fingers = data[1] & 0x0F;
    if fingers == 0 { state.contact_count = 0; state.buttons = 0; return true; }
    ...
    for i in 0..(fingers as usize).min(MAX_CONTACTS) {
        ...
        state.contacts[i] = TouchPoint {
            id: i as u8, x, y, tip: pressure > 30, pressure, width: 0, height: 0,
        };
        offset += 5;
    }
    true
