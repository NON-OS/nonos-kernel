...
pub(crate) fn try_parse_standard_touchpad(
    data: &[u8], state: &mut TouchpadState, logical_max_x: i32, logical_max_y: i32,
) -> bool {
    ...
    if !tip { state.contact_count = 0; state.buttons = 0; return true; }
    state.contact_count = 1;
    state.contacts[0] = TouchPoint {
        id: 0, x, y, tip, pressure: if confidence { 200 } else { 100 }, width: 0, height: 0,
    };
    if data.len() > 5 && tip { state.buttons = data[5] & 0x03; }
