...
pub(crate) fn try_parse_precision_touchpad(
    data: &[u8], state: &mut TouchpadState, max_contacts: u8, logical_max_x: i32, logical_max_y: i32,
) -> bool {
    if data.len() < 6 { return false; }
    let contact_count = data[0] & 0x1F;
    if contact_count == 0 { state.contact_count = 0; state.buttons = 0; return true; }
    if contact_count > max_contacts.min(5) { return false; }
    let contact_size = 5;
    if data.len() < 1 + (contact_count as usize * contact_size) { return false; }
    ...
    if contact_count > 0 && valid_count == 0 { state.contact_count = 0; }
    if offset < data.len() && valid_count > 0 { state.buttons = data[offset] & 0x03; }
