...
use super::super::types::{TouchpadState, Gesture};
use super::super::constants::{TAP_TIMEOUT_US, DOUBLE_TAP_TIMEOUT_US};
use super::types::{TouchpadDriver, timestamp};
...
impl TouchpadDriver {
    pub fn process_report(&mut self, _report_id: u8, data: &[u8]) -> Option<TouchpadState> {
        ...
        match touch {
            ...
            None => {
                if self.was_touching && self.tap_enabled { state.gesture = self.detect_tap(now); }
    ...
    pub(super) fn detect_tap(&mut self, now: u64) -> Gesture {
        ...
        if tap_duration > TAP_TIMEOUT_US { self.tap_count = 0; return Gesture::None; }
        let dx = (self.last_x - self.tap_start_x).abs();
        let dy = (self.last_y - self.tap_start_y).abs();
        let max_tap_movement = self.logical_max_x / 50;
        if dx > max_tap_movement || dy > max_tap_movement { self.tap_count = 0; return Gesture::None; }
