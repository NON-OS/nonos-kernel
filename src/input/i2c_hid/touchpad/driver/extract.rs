...
impl TouchpadDriver {
    pub(super) fn extract_touch(&self, data: &[u8]) -> Option<(i32, i32)> {
        if self.is_using_layout() {
            if let Some(coords) = self.extract_from_layout(data) { return Some(coords); }
        }
    ...
    fn extract_from_layout(&self, data: &[u8]) -> Option<(i32, i32)> {
        ...
        if self.is_valid_coordinate(x, y) { Some((x, y)) } else { None }
    }
    ...
    fn extract_fallback(&self, data: &[u8]) -> Option<(i32, i32)> {
        ...
        if parsing::try_parse_hp_precision_touchpad(data, &mut state, 5, self.logical_max_x, self.logical_max_y) {
            if state.contact_count > 0 && state.contacts[0].tip { return Some((state.contacts[0].x, state.contacts[0].y)); }
        }
    ...
    fn try_format_raw(&self, data: &[u8]) -> Option<(i32, i32)> {
        ...
        if x > 100 && y > 100 && self.is_valid_coordinate(x, y) { Some((x, y)) } else { None }
    }
