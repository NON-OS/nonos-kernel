...
use super::script::{NoxScript, Value, Function};
use super::script_expr::eval_expr;
use super::script_ops::is_truthy;
use super::script_parse::find_block_end;

pub fn exec_if(s: &mut NoxScript, lines: &[&str], start: usize) -> Result<(Value, usize), &'static str> {
    ...
    let skip = if else_start > 0 { let (e, _) = find_block_end(lines, else_start); e - start + 1 } else { body_end - start + 1 };
    Ok((result, skip))
}

pub fn exec_while(s: &mut NoxScript, lines: &[&str], start: usize) -> Result<(Value, usize), &'static str> {
    let line = lines[start].trim();
    ...
    while is_truthy(&eval_expr(s, cond)?) {
        ...
        iters += 1;
        if iters > 10000 { return Err("infinite loop"); }
    }
...
pub fn parse_fn(s: &mut NoxScript, lines: &[&str], start: usize) -> Result<usize, &'static str> {
    ...
    let params: Vec<String> = if params_str.is_empty() { Vec::new() } else {
...
pub fn call_fn(s: &mut NoxScript, expr: &str) -> Result<Value, &'static str> {
    ...
    if s.call_depth > 100 { return Err("stack overflow"); }
    let args: Vec<Value> = if args_str.is_empty() { Vec::new() } else {
        args_str.split(',').map(|a| eval_expr(s, a.trim()).unwrap_or(Value::Nil)).collect()
    };
    let old_vars = s.vars.clone();
    for (i, p) in func.params.iter().enumerate() { s.vars.insert(p.clone(), args.get(i).cloned().unwrap_or(Value::Nil)); }
    s.call_depth += 1;
