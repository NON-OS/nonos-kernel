...
impl Compiler {
    pub fn new() -> Self { Self { bc: Bytecode::new(), locals: Vec::new() } }
    ...
    fn compile_line(&mut self, line: &str) -> Result<(), &'static str> {
        ...
        match parts.get(0).copied() {
            ...
            Some("return") => { self.compile_expr(parts.get(1).unwrap_or(&"0"))?; self.bc.emit(Op::Ret); Ok(()) }
    ...
    fn compile_let(&mut self, rest: &str) -> Result<(), &'static str> {
        ...
        if parts.len() != 2 { return Err("syntax"); }
    ...
    fn compile_expr(&mut self, expr: &str) -> Result<(), &'static str> {
        ...
        if expr.contains('+') { return self.compile_binop(expr, '+', Op::Add); }
        if expr.contains('-') { return self.compile_binop(expr, '-', Op::Sub); }
        if expr.contains('*') { return self.compile_binop(expr, '*', Op::Mul); }
        self.bc.emit(Op::Push);
        self.bc.emit_i64(0);
        Ok(())
    }
