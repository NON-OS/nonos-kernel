...
pub fn parse_let(s: &mut NoxScript, rest: &str) -> Result<Value, &'static str> {
    ...
    if parts.len() != 2 { return Err("syntax: let x = value"); }
...
pub fn eval_expr(s: &mut NoxScript, expr: &str) -> Result<Value, &'static str> {
    let expr = expr.trim();
    if expr == "true" { return Ok(Value::Bool(true)); }
    if expr == "false" { return Ok(Value::Bool(false)); }
    if expr == "nil" { return Ok(Value::Nil); }
    if expr.starts_with('"') && expr.ends_with('"') && expr.len() > 1 {
        return Ok(Value::Str(expr[1..expr.len()-1].to_string()));
    }
    if let Ok(i) = expr.parse::<i64>() { return Ok(Value::Int(i)); }
    if let Ok(f) = expr.parse::<f64>() { return Ok(Value::Float(f)); }
    if let Some(v) = s.vars.get(expr) { return Ok(v.clone()); }
    if expr.contains("==") { return eval_cmp(s, expr, "=="); }
    if expr.contains("!=") { return eval_cmp(s, expr, "!="); }
    if expr.contains(">=") { return eval_cmp(s, expr, ">="); }
    if expr.contains("<=") { return eval_cmp(s, expr, "<="); }
    if expr.contains('>') { return eval_cmp(s, expr, ">"); }
    if expr.contains('<') { return eval_cmp(s, expr, "<"); }
    if expr.contains("&&") { return eval_logic(s, expr, "&&"); }
    if expr.contains("||") { return eval_logic(s, expr, "||"); }
    if expr.contains('+') { return eval_binop(s, expr, '+'); }
    if expr.contains('-') && !expr.starts_with('-') { return eval_binop(s, expr, '-'); }
    if expr.contains('*') { return eval_binop(s, expr, '*'); }
    if expr.contains('/') { return eval_binop(s, expr, '/'); }
    if expr.contains('%') { return eval_binop(s, expr, '%'); }
    if expr.starts_with('!') { return eval_not(s, &expr[1..]); }
    if expr.contains('(') { return super::script_control::call_fn(s, expr); }
    Ok(Value::Nil)
}
