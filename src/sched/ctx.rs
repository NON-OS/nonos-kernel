...
/// Prepare a brand‑new kernel context on `stack_top`.
...
// Compute field offsets for inline-consts in asm (keeps layout in one place).
...
// (You can wire CR0.TS and #NM handler to grab FPU on first use per task.)
// ————————————————————————————————————————————————————————————————————————

#[cfg(feature="xsave")]
#[inline(always)]
pub fn xsave_area_len(xcr0_mask: u64) -> usize {
    // For now return XSAVE_MAX; refine later by CPUID.(EAX=0xD,ECX=0).
