...
fn arr(a: &[JsValue]) -> Option<Rc<RefCell<Vec<JsValue>>>> {
    if let Some(JsValue::Array(ref rc)) = a.first() { Some(rc.clone()) } else { None }
}

fn push(a: &[JsValue]) -> JsValue {
    if let Some(rc) = arr(a) { for v in &a[1..] { rc.borrow_mut().push(v.clone()); } JsValue::Number(rc.borrow().len() as f64) } else { JsValue::Undefined }
}
...
fn shift(a: &[JsValue]) -> JsValue {
    if let Some(rc) = arr(a) { let mut v = rc.borrow_mut(); if v.is_empty() { JsValue::Undefined } else { v.remove(0) } } else { JsValue::Undefined }
}
...
fn unshift(a: &[JsValue]) -> JsValue {
    if let Some(rc) = arr(a) {
        ...
        for (i, item) in items.into_iter().enumerate() { v.insert(i, item); }
...
fn index_of(a: &[JsValue]) -> JsValue {
    if let Some(rc) = arr(a) {
        ...
        for (i, v) in rc.borrow().iter().enumerate() { if v.to_string() == search.to_string() { return JsValue::Number(i as f64); } }
        JsValue::Number(-1.0)
    } else { JsValue::Number(-1.0) }
}
...
fn concat(a: &[JsValue]) -> JsValue {
    ...
    if let Some(rc) = arr(a) { result.extend(rc.borrow().iter().cloned()); }
    for v in &a[1..] { if let JsValue::Array(ref a2) = v { result.extend(a2.borrow().iter().cloned()); } else { result.push(v.clone()); } }
    JsValue::Array(Rc::new(RefCell::new(result)))
}

fn reverse(a: &[JsValue]) -> JsValue {
    if let Some(rc) = arr(a) { rc.borrow_mut().reverse(); JsValue::Array(rc) } else { JsValue::Undefined }
}

fn flat(a: &[JsValue]) -> JsValue {
    if let Some(rc) = arr(a) {
        let mut result = Vec::new();
        for v in rc.borrow().iter() { if let JsValue::Array(ref inner) = v { result.extend(inner.borrow().iter().cloned()); } else { result.push(v.clone()); } }
        JsValue::Array(Rc::new(RefCell::new(result)))
    } else { JsValue::Undefined }
}
