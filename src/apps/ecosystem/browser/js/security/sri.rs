...
pub fn parse_integrity(attr: &str) -> Vec<SriHash> {
    attr.split_whitespace().filter_map(|token| {
...
pub fn strongest_algorithm(hashes: &[SriHash]) -> Option<&SriAlgorithm> {
    if hashes.iter().any(|h| h.algorithm == SriAlgorithm::Sha512) { return Some(&SriAlgorithm::Sha512); }
    if hashes.iter().any(|h| h.algorithm == SriAlgorithm::Sha384) { return Some(&SriAlgorithm::Sha384); }
    if hashes.iter().any(|h| h.algorithm == SriAlgorithm::Sha256) { return Some(&SriAlgorithm::Sha256); }
    None
}
