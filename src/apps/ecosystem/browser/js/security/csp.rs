...
impl CspPolicy {
    pub fn parse(header: &str, report_only: bool) -> Self {
        let directives = header.split(';').filter_map(|d| {
...
pub fn csp_allows(policy: &CspPolicy, directive_name: &str, source: &str, page_origin: &str) -> bool {
    let directive = match policy.get_directive(directive_name) {
        Some(d) => d,
        None => return true,
    };
    if directive.sources.iter().any(|s| s == "'none'") { return false; }
    for src in &directive.sources {
        if matches_source(src, source, page_origin) { return true; }
    }
    false
}
...
fn matches_source(pattern: &str, source: &str, page_origin: &str) -> bool {
    match pattern {
        ...
        "'unsafe-eval'" => true,
        _ => {
            if pattern.starts_with("'nonce-") { return false; }
            if pattern.starts_with("'sha") { return false; }
