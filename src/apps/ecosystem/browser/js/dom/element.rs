...
pub struct JsElement { pub tag: String, pub attrs: BTreeMap<String, String>, pub children: Vec<JsElement>, pub text: String }

impl JsElement {
    pub fn new(tag: &str) -> Self { Self { tag: String::from(tag), attrs: BTreeMap::new(), children: Vec::new(), text: String::new() } }
    pub fn to_js_value(&self) -> JsValue {
        ...
        obj.insert(String::from("id"), self.attrs.get("id").map(|s| JsValue::String(s.clone())).unwrap_or(JsValue::String(String::new())));
        obj.insert(String::from("className"), self.attrs.get("class").map(|s| JsValue::String(s.clone())).unwrap_or(JsValue::String(String::new())));
        obj.insert(String::from("style"), self.create_style_object());
        obj.insert(String::from("children"), self.create_children_array());
        obj.insert(String::from("firstChild"), if self.children.is_empty() { JsValue::Null } else { self.children[0].to_js_value() });
        obj.insert(String::from("lastChild"), self.children.last().map(|c| c.to_js_value()).unwrap_or(JsValue::Null));
        ...
        obj.insert(String::from("insertBefore"), JsValue::NativeFunc(|args| args.get(0).cloned().unwrap_or(JsValue::Undefined)));
        obj.insert(String::from("replaceChild"), JsValue::NativeFunc(|args| args.get(0).cloned().unwrap_or(JsValue::Undefined)));
        ...
        obj.insert(String::from("getBoundingClientRect"), JsValue::NativeFunc(native_get_bounding_rect));
        obj.insert(String::from("querySelector"), JsValue::NativeFunc(|_| JsValue::Null));
        obj.insert(String::from("querySelectorAll"), JsValue::NativeFunc(|_| JsValue::Array(Rc::new(RefCell::new(Vec::new())))));
        JsValue::Object(Rc::new(RefCell::new(obj)))
    }
    fn create_style_object(&self) -> JsValue { JsValue::Object(Rc::new(RefCell::new(BTreeMap::new()))) }
    fn create_children_array(&self) -> JsValue { JsValue::Array(Rc::new(RefCell::new(self.children.iter().map(|c| c.to_js_value()).collect()))) }
}

fn native_get_bounding_rect(_args: &[JsValue]) -> JsValue {
    let mut obj = BTreeMap::new();
    obj.insert(String::from("x"), JsValue::Number(0.0)); obj.insert(String::from("y"), JsValue::Number(0.0));
    obj.insert(String::from("width"), JsValue::Number(0.0)); obj.insert(String::from("height"), JsValue::Number(0.0));
    obj.insert(String::from("top"), JsValue::Number(0.0)); obj.insert(String::from("right"), JsValue::Number(0.0));
    obj.insert(String::from("bottom"), JsValue::Number(0.0)); obj.insert(String::from("left"), JsValue::Number(0.0));
    JsValue::Object(Rc::new(RefCell::new(obj)))
}
