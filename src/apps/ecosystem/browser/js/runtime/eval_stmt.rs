...
impl JsRuntime {
    pub fn eval_stmt(&mut self, stmt: &Stmt) -> JsValue {
        if self.thrown.is_some() { return JsValue::Undefined; }
        match stmt {
            ...
            Stmt::Block(stmts) => {
                ...
                for s in stmts {
                    r = self.eval_stmt(s);
                    if self.return_val.is_some() || self.break_flag || self.continue_flag || self.thrown.is_some() { break; }
                }
                self.scope.pop();
                r
            }
            ...
            Stmt::Var { decls, .. } => {
                for d in decls {
                    let val = d.init.as_ref().map(|e| self.eval_expr(e)).unwrap_or(JsValue::Undefined);
                    self.scope.declare(d.name.clone(), val);
                }
                JsValue::Undefined
            }
            Stmt::If { cond, then_br, else_br } => {
                if self.eval_expr(cond).to_bool() { self.eval_stmt(then_br) }
                else if let Some(eb) = else_br { self.eval_stmt(eb) }
            ...
            Stmt::For { init, cond, update, body } => {
                self.scope.push();
                if let Some(i) = init { self.eval_stmt(i); }
            ...
            Stmt::Return(e) => {
                self.return_val = Some(e.as_ref().map(|ex| self.eval_expr(ex)).unwrap_or(JsValue::Undefined));
                JsValue::Undefined
            }
            Stmt::Throw(e) => { self.thrown = Some(self.eval_expr(e)); JsValue::Undefined }
            Stmt::Try { block, catch, finally } => {
                ...
                if let Some(f) = finally { self.eval_stmt(f); }
                JsValue::Undefined
            }
            Stmt::Function { name, params, body, .. } => {
                let func = JsValue::Function(Rc::new(JsFuncInner { name: name.clone(), params: params.clone(), body: body.clone() }));
                if let Some(n) = name { self.scope.declare(n.clone(), func.clone()); }
                func
            }
                ...
                for c in cases {
                    if !matched {
                        if let Some(test) = &c.test {
                            ...
                            if self.strict_eq(&val, &test_val) { matched = true; }
                        } else { matched = true; }
                    }
                    if matched {
                        for s in &c.body {
                            self.eval_stmt(s);
                            if self.break_flag { self.break_flag = false; return JsValue::Undefined; }
                        }
                    }
                }
                JsValue::Undefined
