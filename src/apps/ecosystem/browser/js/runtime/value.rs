...
impl JsValue {
    pub fn to_bool(&self) -> bool { match self { Self::Undefined | Self::Null => false, Self::Bool(b) => *b, Self::Number(n) => *n != 0.0 && !n.is_nan(), Self::String(s) => !s.is_empty(), _ => true } }
    pub fn to_number(&self) -> f64 { match self { Self::Undefined => f64::NAN, Self::Null => 0.0, Self::Bool(b) => if *b { 1.0 } else { 0.0 }, Self::Number(n) => *n, Self::String(s) => s.parse().unwrap_or(f64::NAN), _ => f64::NAN } }
    pub fn to_string(&self) -> String {
        match self {
            Self::Undefined => String::from("undefined"), Self::Null => String::from("null"),
            Self::Bool(b) => if *b { String::from("true") } else { String::from("false") },
            Self::Number(n) => { if n.is_nan() { String::from("NaN") } else if n.is_infinite() { if *n > 0.0 { String::from("Infinity") } else { String::from("-Infinity") } } else { alloc::format!("{}", n) } }
