...
use crate::network::stack::async_ops::{tcp_start_connect, dns_start_query};
use super::types::*;
...
pub(super) fn start_next_image() {
    ...
    let (line_idx, elem_idx, url) = match entry { Some(e) => e, None => { finish_all_images(); return; } };
    ...
    let mut targets = vec![(line_idx, elem_idx)];
    {
        ...
        let mut i = 0;
        while i < queue.len() {
            if queue[i].2 == url { targets.push((queue[i].0, queue[i].1)); queue.remove(i); }
            else { i += 1; }
        }
    }
    *IMG_TARGETS.lock() = targets;
    let (host, port, path, is_https) = match parse_image_url(&url) {
        Some(parts) => parts,
        None => { crate::sys::serial::println(b"[IMG-FETCH] bad url, skip"); skip_current_image(); return; }
    };
...
fn try_same_host_connect(host: &str, port: u16) {
    ...
    *IMG_IP.lock() = None;
    match dns_start_query(host) {
        ...
        Err(e) => { crate::sys::serial::print(b"[IMG-FETCH] dns start failed: ");
            crate::sys::serial::println(e.as_bytes()); skip_current_image(); }
    }
}
