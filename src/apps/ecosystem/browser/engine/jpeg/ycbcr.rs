...
use super::huffman::{HuffmanTable, BitReader};
...
pub(super) fn decode_ycbcr(data: &[u8], markers: &JpegMarkers, width: u32, height: u32) -> Option<ImageData> {
    ...
    for sc in scan_comps { for (ci, comp) in comps.iter().enumerate() { if comp.id == sc.component_id { scan_lookup[ci] = Some(sc); } } }
            ...
            for ci in 0..comps.len() {
                ...
                for bv in 0..blocks_v {
                    for bh in 0..blocks_h {
                        let coeffs = reader.decode_block(&dc_tables[ci], &ac_tables[ci], &mut prev_dc[ci])?;
                        ...
                        for by in 0..8 { for bx in 0..8 { let idx = (base_y + by) * pw + base_x + bx; if idx < planes[ci].len() { planes[ci][idx] = block[by * 8 + bx] as u8; } } }
                    }
                }
            }
        }
...
fn convert_planes_to_argb(planes: &[Vec<u8>], plane_widths: &[usize], comps: &[super::markers::ComponentInfo], width: u32, height: u32) -> Option<ImageData> {
    ...
    let pixels = ycbcr_to_argb(&y_plane, &cb_plane, &cr_plane, width, height, comps[0].h_sampling, comps[0].v_sampling, comps[1].h_sampling, comps[1].v_sampling);
