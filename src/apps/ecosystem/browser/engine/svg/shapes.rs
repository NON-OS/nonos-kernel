...
pub(super) fn draw_svg_rect(node: &Node, pixels: &mut [u32], canvas_w: u32, canvas_h: u32) {
    ...
    let (x1, y1, x2, y2) = (x.min(canvas_w), y.min(canvas_h), (x + w).min(canvas_w), (y + h).min(canvas_h));
    for py in y1..y2 { for px in x1..x2 { pixels[(py * canvas_w + px) as usize] = fill; } }
    if let Some(stroke) = get_attribute(node, "stroke") {
        if let Some(color) = parse_svg_color(&stroke) {
            for px in x1..x2 { if y1 < canvas_h { pixels[(y1 * canvas_w + px) as usize] = color; } if y2 > 0 && y2 - 1 < canvas_h { pixels[((y2 - 1) * canvas_w + px) as usize] = color; } }
            for py in y1..y2 { if x1 < canvas_w { pixels[(py * canvas_w + x1) as usize] = color; } if x2 > 0 && x2 - 1 < canvas_w { pixels[(py * canvas_w + x2 - 1) as usize] = color; } }
        }
    }
}
...
pub(super) fn draw_svg_polyline(node: &Node, pixels: &mut [u32], canvas_w: u32, canvas_h: u32) {
    ...
    for i in 1..coords.len() { bresenham_line(coords[i-1].0, coords[i-1].1, coords[i].0, coords[i].1, color, pixels, canvas_w, canvas_h); }
}
