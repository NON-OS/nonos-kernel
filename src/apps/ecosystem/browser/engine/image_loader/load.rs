...
use super::types::{ImageFormat, MAX_IMAGE_BYTES, MAX_DIMENSION, MAX_IMAGES_PER_PAGE};
use super::state::{IMAGE_LOAD_COUNT, FETCH_DISABLED};
use super::resolve::resolve_url;
use super::super::ImageData;

pub fn detect_image_format(data: &[u8]) -> ImageFormat {
    if data.len() >= 2 && data[0] == 0xFF && data[1] == 0xD8 { ImageFormat::Jpeg }
...
fn fetch_image_bytes(url: &str) -> Option<Vec<u8>> {
    ...
    if !response.is_success() { return None; }
    if response.body.len() > MAX_IMAGE_BYTES { return None; }
    Some(response.body)
}

pub fn load_image(src: &str, base_url: &str) -> Option<ImageData> {
    if FETCH_DISABLED.load(Ordering::Acquire) { return None; }
    let count = IMAGE_LOAD_COUNT.fetch_add(1, Ordering::Relaxed);
    if count >= MAX_IMAGES_PER_PAGE { return None; }
    ...
    if image.width > MAX_DIMENSION || image.height > MAX_DIMENSION { return None; }
    Some(image)
}
