...
pub fn estimate_output(amount_in: u128, reserve_in: u128, reserve_out: u128, fee_basis_points: u16) -> SwapQuote {
    if amount_in == 0 || reserve_in == 0 || reserve_out == 0 {
        return SwapQuote { amount_in, amount_out: 0, price_impact: 0.0, fee: 0, minimum_received: 0 };
    }
    ...
    let execution_price = if amount_in_with_fee > 0 { amount_out as f64 / amount_in_with_fee as f64 } else { 0.0 };
    let price_impact = if spot_price > 0.0 { ((spot_price - execution_price) / spot_price) * 100.0 } else { 0.0 };
    let slippage_tolerance = 50;
    let minimum_received = (amount_out * (BASIS_POINTS - slippage_tolerance as u128)) / BASIS_POINTS;
    SwapQuote { amount_in, amount_out, price_impact, fee, minimum_received }
}

pub fn estimate_input(amount_out: u128, reserve_in: u128, reserve_out: u128, fee_basis_points: u16) -> SwapQuote {
    if amount_out == 0 || reserve_in == 0 || reserve_out == 0 || amount_out >= reserve_out {
        return SwapQuote { amount_in: 0, amount_out, price_impact: 0.0, fee: 0, minimum_received: amount_out };
    }
    ...
    let amount_in = (amount_in_before_fee * BASIS_POINTS) / (BASIS_POINTS - fee_basis_points as u128);
