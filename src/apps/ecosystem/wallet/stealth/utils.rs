...
pub(super) fn generate_random_key() -> CryptoResult<SecretKey> {
    ...
    for i in 0..32 { if key[i] < SECP256K1_ORDER[i] { return Ok(key); } if key[i] > SECP256K1_ORDER[i] { return generate_random_key(); } }
    generate_random_key()
}
...
pub(super) fn fallback_stealth_address(spending_pubkey: &PublicKey, shared_secret: &[u8; 65]) -> [u8; 20] {
...
pub(super) fn fallback_shared_secret(viewing_pubkey: &PublicKey, ephemeral_pubkey: &PublicKey) -> [u8; 65] {
    let mut input = Vec::with_capacity(130);
...
pub(super) fn decompress_pubkey(compressed: &[u8]) -> CryptoResult<PublicKey> {
    if compressed.len() != 33 { return Err(CryptoError::InvalidLength); }
    if compressed[0] != 0x02 && compressed[0] != 0x03 { return Err(CryptoError::InvalidInput); }
    crate::crypto::asymmetric::secp256k1::decompress_pubkey(compressed)
}

pub(super) fn hex_to_bytes(hex: &str) -> CryptoResult<Vec<u8>> {
    if hex.len() % 2 != 0 { return Err(CryptoError::InvalidLength); }
    let mut bytes = Vec::with_capacity(hex.len() / 2);
    for i in (0..hex.len()).step_by(2) { bytes.push(u8::from_str_radix(&hex[i..i + 2], 16).map_err(|_| CryptoError::InvalidInput)?); }
    Ok(bytes)
}
