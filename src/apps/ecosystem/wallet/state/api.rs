...
pub(super) struct WalletStateInner {
    pub keys: WalletKeys, pub stealth_keys: Option<StealthKeyPair>,
    pub accounts: Vec<AccountInfo>, pub network: Network, pub password_hash: [u8; 32],
}

impl Drop for WalletStateInner {
    fn drop(&mut self) { for byte in self.password_hash.iter_mut() { unsafe { ptr::write_volatile(byte, 0) }; } compiler_fence(Ordering::SeqCst); }
}

pub fn init_wallet(keys: WalletKeys, password: &[u8]) -> Result<(), CryptoError> {
    if WALLET_INITIALIZED.load(Ordering::SeqCst) { return Err(CryptoError::InvalidState); }
    ...
    let inner = WalletStateInner { keys, stealth_keys: None, accounts: alloc::vec![first_account], network: Network::Sepolia, password_hash };
    { let mut guard = WALLET_STATE.write(); *guard = Some(inner); }
    WALLET_LOCKED.store(false, Ordering::SeqCst);
    WALLET_INITIALIZED.store(true, Ordering::SeqCst);
    Ok(())
}

pub fn get_wallet() -> Option<WalletState> { if !WALLET_INITIALIZED.load(Ordering::SeqCst) { return None; } Some(WalletState::current()) }
pub fn lock_wallet() { WALLET_LOCKED.store(true, Ordering::SeqCst); }
...
pub fn unlock_wallet(password: &[u8]) -> Result<(), CryptoError> {
    ...
    if password_hash != inner.password_hash { return Err(CryptoError::AuthenticationFailed); }
    drop(guard);
    WALLET_LOCKED.store(false, Ordering::SeqCst);
    Ok(())
}

pub fn is_locked() -> bool { WALLET_LOCKED.load(Ordering::SeqCst) }
pub fn is_initialized() -> bool { WALLET_INITIALIZED.load(Ordering::SeqCst) }
pub fn set_network(network: Network) { let mut guard = WALLET_STATE.write(); if let Some(inner) = guard.as_mut() { inner.network = network; } }
pub fn get_network() -> Network { let guard = WALLET_STATE.read(); guard.as_ref().map(|i| i.network).unwrap_or(Network::Mainnet) }
pub fn set_active_account(index: u8) { ACTIVE_ACCOUNT.store(index, Ordering::Relaxed); }
pub fn get_active_account() -> u8 { ACTIVE_ACCOUNT.load(Ordering::Relaxed) }
...
pub fn get_account(index: u32) -> Option<AccountInfo> { let guard = WALLET_STATE.read(); let inner = guard.as_ref()?; inner.accounts.get(index as usize).cloned() }
pub fn get_all_accounts() -> Vec<AccountInfo> { let guard = WALLET_STATE.read(); match guard.as_ref() { Some(inner) => inner.accounts.clone(), None => Vec::new() } }
...
pub fn get_stealth_keys() -> Option<StealthKeyPair> { let guard = WALLET_STATE.read(); guard.as_ref().and_then(|i| i.stealth_keys.clone()) }

pub fn destroy_wallet() {
    let mut guard = WALLET_STATE.write(); *guard = None; drop(guard);
