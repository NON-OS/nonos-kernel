...
use super::types::{WalletKeys, BIP44_PURPOSE, BIP44_ETH_COIN, hex_char};
use crate::crypto::application::bip32::{derive_master_key, derive_child};
...
impl WalletKeys {
    ...
    pub fn from_mnemonic(mnemonic: &Mnemonic, passphrase: &str) -> CryptoResult<Self> { Self::from_seed(&mnemonic.to_seed(passphrase)) }

    pub fn derive_secret_key(&self, index: u32) -> CryptoResult<[u8; 32]> { Ok(*derive_child(&self.account_key, index)?.secret_key()) }
