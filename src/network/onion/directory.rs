...
//!
//! Notes:
//! * Signature verification covers the raw consensus body we store. Tor's
//!   canonicalization is more nuanced; adapt later match spec bit-for-bit.
...
/// Relay descriptor with comprehensive information (populated from
/// consensus+microdesc)
#[derive(Debug, Clone)]
...
impl DirectoryService {
    ...
    fn materialize_relays_from_microdescs(&self) -> Result<(), OnionError> {
        ...
        for e in &c.relays {
            let Some(md_sha) = e.microdesc_sha256 else {
                continue;
            };
            let Some(text) = micro.get(&md_sha) else {
                continue;
            };
            ...
            if parsed.ntor_key.len() != 32 {
                continue;
            }

            let mut ed = [0u8; 32];
            if let Some(x) = e.ed25519_id {
                ed = x;
            }
    ...
    fn fetch_from_authority(
        &self,
        authority: &DirectoryAuthority,
        path: &str,
    ) -> Result<Vec<u8>, OnionError> {
        let Some(net) = get_network_stack() else {
        ...
        let req = format!(
            "GET {} HTTP/1.1\r\n\
             ...
             Connection: close\r\n\r\n",
            path,
            ipv4_to_string(authority.address),
            authority.dir_port
        );

        let resp = net
            .http_request(authority.address, authority.dir_port, req.as_bytes())
    ...
    fn parse_consensus(&self, raw: &[u8]) -> Result<NetworkConsensus, OnionError> {
        ...
        for line in text.lines() {
            ...
            match tag {
                ...
                "valid-after" => {
                    c.valid_after =
                        parse_timestamp_fields(it.collect::<Vec<_>>().as_slice()).unwrap_or(0);
                }
                "fresh-until" => {
                    c.fresh_until =
                        parse_timestamp_fields(it.collect::<Vec<_>>().as_slice()).unwrap_or(0);
                    ...
                    for kv in it {
                        if let Some((k, v)) = kv.split_once('=') {
                            if let Ok(val) = v.parse::<i32>() {
                                c.params.insert(k.to_string(), val);
                            }
                        }
                    }
                }
    ...
    fn validate_consensus(&self, c: &mut NetworkConsensus) -> Result<(), OnionError> {
        ...
        // Build an identity->ed25519 public key map from our hardcoded authorities
        // list.
        let auths = self.authorities.read();
        let mut id_to_ed: BTreeMap<[u8; 20], [u8; 32]> = BTreeMap::new();
        for a in auths.iter() {
            if let Some(ed) = a.ed25519_identity {
                // Legacy identity digest is informational; consensus gives the legacy digest in
                // signatures. We cannot derive SHA1 of ed25519 pubkey
                // deterministically here, so we match by nickname where possible.
                // In practice you'd ship both mappings; here we associate the first matching
                // header by nickname.
        ...
        for s in &c.signatures {
            if s.signing_alg != SigAlg::Ed25519 {
                continue;
            }
            let Some(pk) = id_to_ed.get(&s.identity).copied() else {
                continue;
            };
    ...
    pub fn select_path_with_constraints(
        &self,
        ...
        let exits: Vec<_> = c
            ...
            .iter()
            .filter(|r| {
                r.flags.is_exit && r.flags.is_running && r.flags.is_valid && !r.flags.is_bad_exit
            })
            .filter(|r| r.bandwidth.unwrap_or(0) >= constraints.min_bandwidth)
            .collect();
        ...
        for _ in 0..8 {
            if !family_conflict(guard, middle)
                && !family_conflict(guard, exit)
        ...
        // Ensure RelayDescriptors are materialized; if missing md, try fetching
        // on-demand
    ...
    /// Default directory authorities (subset, with ed25519 identities when
    /// known).
    fn default_authorities() -> Vec<DirectoryAuthority> {
        ...
        vec![
            DirectoryAuthority {
                ...
                identity_fingerprint: hex_to_vec("0000000000000000000000000000000000000000")
                    .unwrap_or_default(),
                address: [128, 31, 0, 39],
                dir_port: 9131,
                or_port: 9101,
...
fn parse_timestamp_fields(parts: &[&str]) -> Option<u64> {
    ...
    let date = parts[0];
    let time = parts[1];
    // crude but deterministic conversion to unix time: defer to kernel time utils
    // if available Here, parse and approximate: YYYY-MM-DD HH:MM:SS -> seconds
    // since epoch using a simple algorithm or call into time crate if provided.
    // For v1, we fallback to "now - X" unsafe; but better: treat as valid (return
    // now) if parsers fail.
    let _ = (date, time);
...
fn ipv4_net16(a: [u8; 4]) -> u16 {
    ((a[0] as u16) << 8) | (a[1] as u16)
}

fn family_conflict(a: &ConsensusEntry, b: &ConsensusEntry) -> bool {
    // We don't have full family strings here; later we can augment by parsing
    // family from md and mapping back. Use ed25519 identity equality as "same
    // node" guard.
...
fn subnet16_conflict(a: [u8; 4], b: [u8; 4]) -> bool {
