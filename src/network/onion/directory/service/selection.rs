...
use crate::network::onion::directory::types::RelayDescriptor;
use crate::network::onion::directory::path::{family_conflict, subnet16_conflict, weighted_pick};
...
impl DirectoryService {
    ...
    /*
     * selects 3-hop path: guard -> middle -> exit.
     * applies bandwidth weighting and avoids family/subnet conflicts.
     */
    pub fn select_path_with_constraints(&self, constraints: &PathConstraints) -> Result<Vec<RelayDescriptor>, OnionError> {
        ...
        /* retry path selection up to 8 times to avoid conflicts */
        for _ in 0..8 {
        ...
        self.build_path_from_entries(&[guard, middle, exit])
    }

    /*
     * selects path where exit supports all required ports.
     * used for http/https which need ports 80/443.
     */
    ...
    fn build_path_from_entries(&self, entries: &[&crate::network::onion::directory::types::ConsensusEntry]) -> Result<Vec<RelayDescriptor>, OnionError> {
