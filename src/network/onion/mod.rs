...
//!
//! This module is the public façade: initialize once, then use the helpers
//! below to build circuits, open streams, and pass application data.
...
pub use nonos_crypto::{RSAKeyPair, RealCurve25519, RealDH, RealEd25519, RealRSA};
pub use real_network::{get_tor_network, init_tor_network, TorNetworkManager};
pub use relay::{OnionRelay, RelayConfig, RelayManager};
pub use security::{check_client_security, init_security, secure_zero, SecurityManager};
...
impl Clone for RelayStats {
    fn clone(&self) -> Self {
        Self {
            cells_processed: AtomicU32::new(
                self.cells_processed.load(core::sync::atomic::Ordering::Relaxed),
            ),
            bytes_relayed: AtomicU32::new(
                self.bytes_relayed.load(core::sync::atomic::Ordering::Relaxed),
            ),
            circuits_created: AtomicU32::new(
                self.circuits_created.load(core::sync::atomic::Ordering::Relaxed),
            ),
            streams_opened: AtomicU32::new(
                self.streams_opened.load(core::sync::atomic::Ordering::Relaxed),
            ),
        }
    }
}
...
impl OnionRouter {
    ...
    pub fn process_cell(&mut self, cell: Cell) -> Result<(), OnionError> {
        self.cell_processor.process_cell(
            cell,
            &mut self.circuit_manager,
