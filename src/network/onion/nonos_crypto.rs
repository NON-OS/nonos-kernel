...
use crate::crypto::{bigint::BigUint, curve25519, entropy, hash, hmac, rsa, sig, vault};
...
impl RealDH {
    ...
    pub fn generate_keypair() -> Result<(Vec<u8>, Vec<u8>), OnionError> {
        ...
        let x_bn = &BigUint::from_bytes_be(&x) % &p;
        x = x_bn.to_bytes_be();
    ...
    fn prime_p() -> BigUint {
        // RFC 2409 1024-bit (Oakley Group 2 / Tor TAP)
...
impl X509 {
    ...
    pub fn parse_der(der: &[u8]) -> Result<X509Certificate, OnionError> {
        ...
        Ok(X509Certificate { tbs_certificate, signature_algorithm, signature, public_key })
    }
