...
pub(super) fn read_tlv_raw(data: &[u8], offset: usize) -> Option<(&[u8], usize)> { read_tlv(data, offset) }
...
fn read_der_length(data: &[u8], offset: usize) -> Option<(usize, usize)> {
    ...
    if first & 0x80 == 0 { Some((first as usize, offset + 1)) }
        ...
        if num_bytes == 0 || num_bytes > 4 || offset + 1 + num_bytes > data.len() { return None; }
        let mut len = 0usize;
        for i in 0..num_bytes { len = (len << 8) | data[offset + 1 + i] as usize; }
