...
use super::key_usage::{parse_key_usage, parse_ext_key_usage};
...
pub(crate) fn parse_extensions(parser: &mut DerParser, tbs_end: usize) -> Result<X509Extensions, OnionError> {
    ...
    while parser.offset < seq_end { parse_single_extension(parser, &mut exts)?; }
    Ok(exts)
}
...
fn parse_single_extension(parser: &mut DerParser, exts: &mut X509Extensions) -> Result<(), OnionError> {
    ...
    if oid_bytes == OID_BASIC_CONSTRAINTS { parse_basic_constraints(&parser.data[val_start..val_end], &mut exts.basic_constraints)?; }
    else if oid_bytes == OID_KEY_USAGE { exts.key_usage = parse_key_usage(&parser.data[val_start..val_end])?; }
    else if oid_bytes == OID_EXT_KEY_USAGE { parse_ext_key_usage(&parser.data[val_start..val_end], &mut exts.ext_key_usage)?; }
    else if oid_bytes == OID_SUBJECT_KEY_ID { exts.subject_key_id = Some(parse_octet_string_value(&parser.data[val_start..val_end])?); }
    else if oid_bytes == OID_AUTHORITY_KEY_ID { exts.authority_key_id = parse_authority_key_id(&parser.data[val_start..val_end])?; }
    else if oid_bytes == OID_SUBJECT_ALT_NAME { parse_san(&parser.data[val_start..val_end], &mut exts.san_dns_names)?; }
