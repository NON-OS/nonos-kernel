...
pub(crate) fn parse_response(data: &[u8]) -> Result<HttpResponse, &'static str> {
    ...
    if status_parts.len() < 2 { return Err("malformed status line"); }
    let status_code = parse_status_code(status_parts[1])?;
    let status_text = if status_parts.len() > 2 { String::from_utf8_lossy(status_parts[2]).into_owned() }
    else { String::new() };
    ...
    for line in header_lines.split(|&b| b == b'\n') {
        ...
        if let Some(colon_pos) = line.iter().position(|&b| b == b':') {
            ...
            if name_lower == "transfer-encoding" && value.to_ascii_lowercase().contains("chunked") { is_chunked = true; }
            if name_lower == "content-encoding" { content_encoding = Some(value.to_ascii_lowercase()); }
    ...
    Ok(HttpResponse { status_code, status_text, headers, body, final_url: String::new(), redirects: 0 })
}
