...
fn decompress_gzip(data: &[u8]) -> Option<Vec<u8>> {
    ...
    if flags & 0x08 != 0 { while offset < data.len() && data[offset] != 0 { offset += 1; } offset += 1; }
    if flags & 0x10 != 0 { while offset < data.len() && data[offset] != 0 { offset += 1; } offset += 1; }
    if flags & 0x02 != 0 { offset += 2; }
    if offset >= data.len().saturating_sub(8) { return None; }
...
fn decompress_deflate(data: &[u8]) -> Option<Vec<u8>> {
    if data.len() >= 2 {
        ...
        if (cmf as u16 * 256 + flg as u16) % 31 == 0 {
            if let Ok(decompressed) = decompress_to_vec_zlib(data) { return Some(decompressed); }
        }
    }
