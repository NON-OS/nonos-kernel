...
pub(super) fn is_response_complete(data: &[u8], header_end: usize) -> bool {
    ...
    if let Some(content_length) = parse_content_length(headers) { return body.len() >= content_length; }
    if has_chunked_encoding(headers) { return find_chunked_terminator(body); }
    false
}
...
fn parse_content_length(headers: &[u8]) -> Option<usize> {
    ...
    for line in header_str.split("\r\n") {
        ...
        if lower.starts_with("content-length:") { let value = line[15..].trim(); return value.parse().ok(); }
    }
    None
}

fn has_chunked_encoding(headers: &[u8]) -> bool {
    let header_str = match core::str::from_utf8(headers) { Ok(s) => s, Err(_) => return false };
    for line in header_str.split("\r\n") {
        let lower = line.to_ascii_lowercase();
        if lower.starts_with("transfer-encoding:") && lower.contains("chunked") { return true; }
    }
    false
}
