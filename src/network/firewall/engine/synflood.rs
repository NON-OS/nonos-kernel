...
struct SynState { count: AtomicU32, window_start: AtomicU64 }
...
pub fn check_syn_flood(src_ip: [u8; 4]) -> bool {
    ...
    let state = tracker.entry(ip_key).or_insert_with(|| SynState {
        count: AtomicU32::new(0), window_start: AtomicU64::new(now)
    });
    ...
    if count >= SYN_THRESHOLD {
        ...
        crate::security::monitoring::audit::log_security_event(
            "firewall", crate::security::monitoring::audit::AuditSeverity::Critical,
            alloc::format!("SYN flood detected from {}.{}.{}.{}", src_ip[0], src_ip[1], src_ip[2], src_ip[3]),
            None, None, None);
        return false;
    }
    true
...
pub fn generate_syn_cookie(src_ip: [u8; 4], src_port: u16, dst_port: u16, seq: u32) -> u32 {
    if SYN_COOKIES_ENABLED.load(Ordering::Relaxed) == 0 { return seq.wrapping_add(1); }
...
pub fn verify_syn_cookie(src_ip: [u8; 4], src_port: u16, dst_port: u16, ack: u32) -> bool {
    if SYN_COOKIES_ENABLED.load(Ordering::Relaxed) == 0 { return true; }
    let t_recv = ((ack >> 24) & 0xFF) as u32;
    let t_now = (crate::time::timestamp_millis() / 60000) as u32;
    if t_now.wrapping_sub(t_recv) > 2 { return false; }
...
pub fn unblock_ip(ip: [u8; 4]) { BLOCKED_IPS.lock().remove(&ip_to_u32(ip)); }
pub fn enable_syn_cookies(enable: bool) { SYN_COOKIES_ENABLED.store(enable as u32, Ordering::Relaxed); }
