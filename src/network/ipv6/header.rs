...
#[repr(u8)]
pub enum NextHeader {
    HopByHop = 0, Tcp = 6, Udp = 17, Ipv6Route = 43, Ipv6Frag = 44, Icmpv6 = 58,
    Ipv6NoNxt = 59, Ipv6Opts = 60, Unknown(u8),
}

impl From<u8> for NextHeader {
    fn from(v: u8) -> Self {
        match v {
            0 => Self::HopByHop, 6 => Self::Tcp, 17 => Self::Udp, 43 => Self::Ipv6Route,
            44 => Self::Ipv6Frag, 58 => Self::Icmpv6, 59 => Self::Ipv6NoNxt, 60 => Self::Ipv6Opts,
            n => Self::Unknown(n),
        }
    }
}
...
impl From<NextHeader> for u8 {
    fn from(h: NextHeader) -> u8 {
        match h {
            NextHeader::HopByHop => 0, NextHeader::Tcp => 6, NextHeader::Udp => 17,
            NextHeader::Ipv6Route => 43, NextHeader::Ipv6Frag => 44, NextHeader::Icmpv6 => 58,
            NextHeader::Ipv6NoNxt => 59, NextHeader::Ipv6Opts => 60, NextHeader::Unknown(n) => n,
        }
    }
}
...
impl Ipv6Header {
    ...
    pub fn parse(data: &[u8]) -> Option<Self> {
        ...
        let hl = data[7];
        let mut src = [0u8; 16]; src.copy_from_slice(&data[8..24]);
        let mut dst = [0u8; 16]; dst.copy_from_slice(&data[24..40]);
        Some(Self { version: 6, traffic_class: tc, flow_label: fl, payload_length: pl,
            next_header: nh, hop_limit: hl, src: Ipv6Address(src), dst: Ipv6Address(dst) })
    }
...
#[derive(Debug, Clone)]
pub struct Ipv6ExtHeader { pub next_header: NextHeader, pub length: u8, pub data: [u8; 254] }
