    ...
    pub fn new(src: Ipv6Address, dst: Ipv6Address, next: NextHeader, payload: Vec<u8>) -> Self {
        Self {
            header: Ipv6Header {
                ...
                payload_length: payload.len() as u16, next_header: next,
                hop_limit: 64, src, dst,
            },
            payload,
        }
    }

    pub fn src(&self) -> Ipv6Address { self.header.src }
    pub fn dst(&self) -> Ipv6Address { self.header.dst }
    pub fn next_header(&self) -> NextHeader { self.header.next_header }
    pub fn hop_limit(&self) -> u8 { self.header.hop_limit }
    pub fn payload(&self) -> &[u8] { &self.payload }

    pub fn decrement_hop_limit(&mut self) -> bool {
        if self.header.hop_limit == 0 { return false; }
        self.header.hop_limit -= 1;
        self.header.hop_limit > 0
    }
}

pub fn build_ipv6_packet(src: Ipv6Address, dst: Ipv6Address, next: NextHeader, hop_limit: u8, payload: &[u8]) -> Vec<u8> {
...
pub fn compute_pseudo_header_checksum(src: &Ipv6Address, dst: &Ipv6Address, next_header: u8, length: u32) -> u32 {
    let mut sum = 0u32;
