...
impl FilesystemManager {
    ...
    fn is_suspicious_operation(&self, operation: &FileOperation) -> bool {
        ...
        let sensitive_paths =
            ["/etc/passwd", "/etc/shadow", "/etc/sudoers", "/boot/", "/proc/", "/sys/"];
    ...
    pub fn get_access_statistics(&self) -> AccessStatistics {
        AccessStatistics {
            total_operations: AtomicU64::new(
                self.access_stats.total_operations.load(Ordering::Relaxed),
            ),
            read_operations: AtomicU64::new(
                self.access_stats.read_operations.load(Ordering::Relaxed),
            ),
            write_operations: AtomicU64::new(
                self.access_stats.write_operations.load(Ordering::Relaxed),
            ),
            create_operations: AtomicU64::new(
                self.access_stats.create_operations.load(Ordering::Relaxed),
            ),
            delete_operations: AtomicU64::new(
                self.access_stats.delete_operations.load(Ordering::Relaxed),
            ),
            ...
            failed_operations: AtomicU64::new(
                self.access_stats.failed_operations.load(Ordering::Relaxed),
            ),
            suspicious_activities: AtomicU64::new(
                self.access_stats.suspicious_activities.load(Ordering::Relaxed),
            ),
        }
    }
...
fn read_superblock(
    storage: &dyn crate::storage::StorageDevice,
) -> Result<Superblock, &'static str> {
    let mut buffer = vec![0u8; 4096];
    storage.read_blocks(1, 1, &mut buffer).map_err(|_| "Failed to read superblock")?;
    Ok(Superblock { block_size: 4096, total_blocks: storage.total_sectors(), root_inode: 2 })
}
...
fn search_directory_block(block: &[u8], name: &str) -> Option<u64> {
    ...
    while offset + 8 < block.len() {
        ...
        if offset + 8 + name_len <= block.len() {
            let entry_name =
                core::str::from_utf8(&block[offset + 8..offset + 8 + name_len]).unwrap_or("");
