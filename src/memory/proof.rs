...
// Backpressure: if queue is full, we either drop (with counter) OR block if called from non-IRQ context. Here we drop in IRQ paths and count drops.
//
// Safety notes:
...
pub const fn empty_cpu_array<const N: usize>() -> [CpuAudit; N] {
    ...
    // so this function is just a placeholder waiting to be programmed; prefer caller to allocate static mut.
    // Kept for API symmetry; we can remove if not needed.
