...
/// A pool of pre-allocated DMA regions.
pub struct DmaPool {
    /// Pre-allocated regions
    regions: Vec<DmaRegion>,
    /// Indices of free regions
...
impl DmaPool {
    ...
    /// Allocates a region from the pool.
    ...
    /// Deallocates a region back to the pool.
    ...
    /// Returns the total size of all regions.
    pub fn total_size(&self) -> usize {
        self.total_size
    }

    /// Returns the constraints for this pool.
    pub fn constraints(&self) -> DmaConstraints {
        self.constraints
    }

    /// Returns true if the pool is empty (all regions allocated).
    pub fn is_empty(&self) -> bool {
        self.free_regions.is_empty()
    }

    /// Returns true if the pool is full (all regions free).
    pub fn is_full(&self) -> bool {
        self.free_regions.len() == self.regions.len()
    }

    /// Returns the number of regions in the pool.
