...
impl DmaAllocator {
    ...
    pub(super) fn map_dma_page(&self, virt_addr: VirtAddr, phys_addr: PhysAddr, coherent: bool) -> DmaResult<()> {
        virt::map_page_4k(virt_addr, phys_addr, true, false, !coherent).map_err(|_| DmaError::MappingFailed)?;
        if coherent { compiler_fence(Ordering::SeqCst); }
        Ok(())
    }
    ...
    pub(super) fn needs_bounce_buffer(&self, buffer_va: VirtAddr, size: usize, constraints: &DmaConstraints) -> DmaResult<bool> {
        let page_count = pages_needed(size);
        let mut current_va = buffer_va;
        for _ in 0..page_count {
            let phys_addr = self.translate_to_physical(current_va)?;
            if constraints.dma32_only && !is_dma32_compatible(phys_addr.as_u64()) { return Ok(true); }
            if phys_addr.as_u64() % constraints.alignment as u64 != 0 { return Ok(true); }
    ...
    pub(super) fn copy_buffer(&self, src_va: VirtAddr, dst_va: VirtAddr, size: usize) -> DmaResult<()> {
        unsafe {
