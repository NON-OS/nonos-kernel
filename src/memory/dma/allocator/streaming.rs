...
impl DmaAllocator {
    pub fn map_streaming(&mut self, buffer_va: VirtAddr, size: usize, direction: DmaDirection, constraints: DmaConstraints, stats: &DmaStats) -> DmaResult<u64> {
        ...
        let (dma_addr, bounce_buffer) = if needs_bounce {
            ...
            if direction.writes_to_device() { self.copy_buffer(buffer_va, bounce_region.virt_addr, size)?; }
            stats.record_bounce_usage(true);
            (bounce_region.phys_addr, Some(bounce_region))
        } else { (self.translate_to_physical(buffer_va)?, None) };
        let mapping = StreamingMapping::new(mapping_id, buffer_va, dma_addr, size, direction, bounce_buffer);
    ...
    pub fn unmap_streaming(&mut self, mapping_id: u64, stats: &DmaStats) -> DmaResult<()> {
        let mapping = self.streaming_mappings.remove(&mapping_id).ok_or(DmaError::MappingNotFound)?;
        if let Some(bounce_region) = mapping.bounce_buffer {
            if mapping.direction.reads_from_device() { self.copy_buffer(bounce_region.virt_addr, mapping.buffer_va, mapping.size)?; }
    ...
    pub fn sync_for_device(&self, mapping_id: u64) -> DmaResult<()> {
        ...
        if let Some(bounce_region) = mapping.bounce_buffer {
            if mapping.direction.writes_to_device() { self.copy_buffer(mapping.buffer_va, bounce_region.virt_addr, mapping.size)?; }
        }
