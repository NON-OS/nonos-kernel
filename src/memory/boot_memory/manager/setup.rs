...
use super::helpers::{align_up, align_down};
use super::state::BootMemoryManager;

impl BootMemoryManager {
    pub(super) fn setup_regions_from_handoff(&mut self, handoff: &BootHandoff) -> BootMemoryResult<()> {
        self.regions.clear();
    ...
    pub(super) fn sort_regions(&mut self) { self.regions.sort_by_key(|r| r.start.as_u64()); }
}
