...
pub fn map_range(va: VirtAddr, pa: PhysAddr, size: usize, writable: bool, user: bool, executable: bool) -> VmResult<()> { VIRTUAL_MEMORY_MANAGER.lock().map_range(va, pa, size, build_flags(writable, user, executable)) }
pub fn unmap_range(va: VirtAddr, size: usize) -> VmResult<()> { VIRTUAL_MEMORY_MANAGER.lock().unmap_range(va, size) }
pub fn translate_addr(va: VirtAddr) -> VmResult<PhysAddr> { let (pa, _, _) = VIRTUAL_MEMORY_MANAGER.lock().translate(va)?; Ok(pa) }
pub fn translate_with_flags(va: VirtAddr) -> VmResult<(PhysAddr, VmFlags, usize)> { VIRTUAL_MEMORY_MANAGER.lock().translate(va) }
...
pub fn is_mapped(va: VirtAddr) -> bool { VIRTUAL_MEMORY_MANAGER.lock().find_mapped_range(va).is_some() }
...
pub fn validate_range(va: VirtAddr, size: usize, required_flags: VmFlags) -> bool {
    ...
    for i in 0..page_count {
        ...
        if let Some(range) = manager.find_mapped_range(page_va) { if !range.flags.contains(required_flags) { return false; } } else { return false; }
    }
    true
}
...
pub fn handle_page_fault(_va: VirtAddr, error_code: u64) -> VmResult<()> {
    ...
    if (error_code & PF_PRESENT) == 0 { return Err(VmError::AddressNotMapped); }
    if (error_code & PF_WRITE) != 0 { return Err(VmError::PermissionViolation); }
    Ok(())
}
