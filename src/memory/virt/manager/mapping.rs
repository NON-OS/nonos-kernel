...
impl VirtualMemoryManager {
    ...
    pub fn unmap_page(&mut self, va: VirtAddr, page_size: PageSize) -> VmResult<()> {
        if !self.initialized { return Err(VmError::NotInitialized); }
        let range_idx = self.mapped_ranges.iter().position(|r| r.start_va == va).ok_or(VmError::AddressNotMapped)?;
    ...
    pub fn map_range(&mut self, va: VirtAddr, pa: PhysAddr, size: usize, flags: VmFlags) -> VmResult<()> {
    ...
    pub fn unmap_range(&mut self, va: VirtAddr, size: usize) -> VmResult<()> {
        ...
        let page_count = (size + PAGE_SIZE_4K - 1) / PAGE_SIZE_4K;
        for i in 0..page_count {
            self.unmap_page(VirtAddr::new(va.as_u64() + (i * PAGE_SIZE_4K) as u64), PageSize::Size4K)?;
        }
        Ok(())
    }
}
