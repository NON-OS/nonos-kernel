...
impl MmioFlags {
    ...
    pub fn to_vm_flags(self) -> u32 {
        ...
        if !self.executable { flags |= VM_FLAG_NX; }
        if self.user_accessible { flags |= VM_FLAG_USER; }
        if !self.cacheable { flags |= VM_FLAG_CACHE_DISABLE; }
        if self.write_combining { flags |= VM_FLAG_WRITE_COMBINE; }
        flags
    }
}
