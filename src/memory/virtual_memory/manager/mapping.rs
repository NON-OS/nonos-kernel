...
impl VirtualMemoryManager {
    pub fn map_vm_area(&mut self, vm_area: VmArea, stats: &VirtualMemoryStatistics) -> Result<u64, &'static str> {
        if !self.initialized { return Err("Virtual memory manager not initialized"); }
        if self.has_overlap(&vm_area) { return Err("VM area overlaps with existing area"); }
        ...
        for i in 0..page_count {
            ...
            if vm_area.vm_type.is_demand_paged() {
                ...
                unsafe { core::ptr::write_bytes((layout::DIRECTMAP_BASE + pa.as_u64()) as *mut u8, 0, layout::PAGE_SIZE); }
            }
        }
        stats.record_vm_area(vm_area.size as u64, vm_area.vm_type);
        self.vm_areas.insert(area_id, vm_area);
        if let Some(address_space) = self.address_spaces.get_mut(&self.current_asid) { address_space.vm_areas.push(area_id); }
        Ok(area_id)
    }

    pub fn unmap_vm_area(&mut self, area_id: u64, stats: &VirtualMemoryStatistics) -> Result<(), &'static str> {
        let vm_area = self.vm_areas.remove(&area_id).ok_or("VM area not found")?;
        let page_count = (vm_area.size + layout::PAGE_SIZE - 1) / layout::PAGE_SIZE;
        for i in 0..page_count {
            let va = VirtAddr::new(vm_area.start.as_u64() + (i * layout::PAGE_SIZE) as u64);
            if let Ok(pa) = paging::unmap_page(va) { let _ = frame_alloc::deallocate_frame(pa); }
        }
        if let Some(address_space) = self.address_spaces.get_mut(&self.current_asid) { address_space.vm_areas.retain(|&id| id != area_id); }
        stats.record_vm_area_removal(vm_area.size as u64, vm_area.vm_type);
        Ok(())
    }

    pub fn protect_vm_area(&mut self, area_id: u64, new_protection: VmProtection) -> Result<(), &'static str> {
        ...
        let page_count = (vm_area.size + layout::PAGE_SIZE - 1) / layout::PAGE_SIZE;
        paging::protect_pages(vm_area.start, page_count, page_permissions).map_err(|_| "Failed to protect pages")?;
        Ok(())
    }
}
