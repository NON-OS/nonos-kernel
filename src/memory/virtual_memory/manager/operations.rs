...
impl VirtualMemoryManager {
    pub fn expand_heap(&mut self, new_size: usize, stats: &VirtualMemoryStatistics) -> Result<(), &'static str> {
        let current_asid = self.current_asid;
        let (heap_end, heap_start) = {
            let address_space = self.address_spaces.get(&current_asid).ok_or("Address space not found")?;
            (address_space.heap_end, address_space.heap_start)
        };
        let current_heap_size = (heap_end.as_u64() - heap_start.as_u64()) as usize;
        if new_size <= current_heap_size { return Ok(()); }
        ...
        let address_space = self.address_spaces.get_mut(&current_asid).ok_or("Address space not found")?;
        address_space.heap_end = new_heap_end;
        Ok(())
    }

    pub fn expand_stack(&mut self, additional_size: usize, stats: &VirtualMemoryStatistics) -> Result<(), &'static str> {
        let current_asid = self.current_asid;
        let stack_start = {
            let address_space = self.address_spaces.get(&current_asid).ok_or("Address space not found")?;
        ...
        let stack_area = VmArea::new(new_stack_start, additional_size, VmProtection::ReadWrite, VmType::Stack);
    ...
    pub fn handle_page_fault(&mut self, fault_addr: VirtAddr, error_code: u64, stats: &VirtualMemoryStatistics) -> Result<(), &'static str> {
        stats.record_page_fault();
        let vm_area = self.find_vm_area_by_address(fault_addr).ok_or("No VM area for fault address")?;
        ...
        paging::handle_page_fault(fault_addr, error_code).map_err(|_| "Failed to handle page fault")?;
        if let Some(area) = self.vm_areas.get_mut(&area_id) { area.fault_count += 1; area.access_count += 1; }
        Ok(())
    }
}
