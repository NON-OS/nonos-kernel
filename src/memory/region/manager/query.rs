...
impl RegionManager {
    ...
    pub fn get_fragmentation_info(&self) -> (usize, u64) {
        (self.free_regions.len(), self.free_regions.iter().map(|r| r.size as u64).max().unwrap_or(0))
    }
    ...
    pub fn get_stats(&self, stats: &RegionStatistics) -> RegionStats {
        ...
        RegionStats {
            total_regions: self.regions.len(), free_regions: self.free_regions.len(), allocated_bytes: stats.allocated_bytes(),
            free_bytes: stats.free_bytes(), allocation_count: stats.allocation_count(), deallocation_count: stats.deallocation_count(),
            merge_count: stats.merge_count(), split_count: stats.split_count(), fragment_count, largest_free_block: largest_free,
        }
    }

    pub fn region_count(&self) -> usize { self.regions.len() }
    pub fn free_region_count(&self) -> usize { self.free_regions.len() }
}
