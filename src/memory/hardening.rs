...
static ALLOCATION_TRACKER: Mutex<BTreeMap<VirtAddr, AllocationMetadata>> =
...
pub fn secure_free(addr: VirtAddr) -> Result<(), &'static str> {
    ...
    let metadata =
        tracker.remove(&addr).ok_or("Attempt to free untracked allocation (double free?)")?;
...
fn setup_guard_page(guard_addr: VirtAddr) -> Result<(), &'static str> {
    ...
    let phys_frame =
        PhysFrame::<Size4KiB>::from_start_address(frame).map_err(|_| "Invalid frame address")?;
    ...
    unsafe {
        let mut mapper =
            crate::memory::virt::get_kernel_mapper().map_err(|_| "Failed to get kernel mapper")?;
...
fn map_allocation_pages(start_addr: VirtAddr, num_pages: usize) -> Result<(), &'static str> {
    for i in 0..num_pages {
        ...
        let page =
            Page::<Size4KiB>::from_start_address(page_addr).map_err(|_| "Invalid page address")?;

        // Allocate physical frame
        let frame =
            crate::memory::frame_alloc::allocate_frame().ok_or("Failed to allocate frame")?;
...
pub fn get_memory_stats() -> MemoryStats {
    MemoryStats {
        guard_page_violations: AtomicU64::new(
            MEMORY_STATS.guard_page_violations.load(Ordering::SeqCst),
        ),
        wx_violations: AtomicU64::new(MEMORY_STATS.wx_violations.load(Ordering::SeqCst)),
        stack_overflows_detected: AtomicU64::new(
            MEMORY_STATS.stack_overflows_detected.load(Ordering::SeqCst),
        ),
        heap_corruptions_detected: AtomicU64::new(
            MEMORY_STATS.heap_corruptions_detected.load(Ordering::SeqCst),
        ),
        double_frees_prevented: AtomicU64::new(
            MEMORY_STATS.double_frees_prevented.load(Ordering::SeqCst),
        ),
        use_after_free_detected: AtomicU64::new(
            MEMORY_STATS.use_after_free_detected.load(Ordering::SeqCst),
        ),
        mapped_file_pages: AtomicU64::new(0),
        total_mapped_size: AtomicU64::new(0),
