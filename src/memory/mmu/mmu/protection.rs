...
impl MMU {
    pub fn change_page_protection(&self, virt_addr: VirtAddr, new_permissions: PagePermissions) -> MmuResult<()> {
        if new_permissions.is_wx_violation() { return Err(MmuError::WXViolation); }
    ...
    pub(super) fn update_page_entry(&self, pml4_virt: VirtAddr, virt_addr: VirtAddr, permissions: PagePermissions) -> MmuResult<()> {
        ...
        unsafe {
            ...
            if !pte_is_present(old_entry) { return Err(MmuError::NotMapped); }
