...
use x86_64::{
    ...
    structures::paging::{
        mapper::MapperAllSizes, FrameAllocator, Mapper, OffsetPageTable, Page, PageTable,
        PageTableFlags as PtF, PhysFrame, Size2MiB, Size4KiB, Translate,
    },
    PhysAddr, VirtAddr,
};

use crate::memory::kaslr::Kaslr;
use crate::memory::layout::{align_down, align_up, HUGE_2M, KERNEL_BASE, PAGE_SIZE};
use crate::memory::phys::{
    alloc as phys_alloc, alloc_contig as phys_alloc_contig, free as phys_free, AllocFlags, Frame,
};

// Optional: zk/onion audit hooks (implement these in memory/proof.rs)
...
pub struct AddressSpace {
    cr3_frame: PhysFrame,
    pcid: Option<u16>, // PCID plumbing when CR4.PCIDE is enabled
}
...
unsafe impl FrameAllocator<Size4KiB> for PhysAllocShim {
    fn allocate_frame(&mut self) -> Option<PhysFrame<Size4KiB>> {
        phys_alloc(AllocFlags::empty()).map(|f| PhysFrame::containing_address(f.addr))
    }
}
unsafe impl FrameAllocator<Size2MiB> for PhysAllocShim {
    fn allocate_frame(&mut self) -> Option<PhysFrame<Size2MiB>> {
        phys_alloc_contig(512, AllocFlags::empty())
            .map(|f| PhysFrame::containing_address(f[0].addr))
    }
}
...
pub fn unmap4k(va: VirtAddr) -> Result<(), VmErr> {
    ...
    phys_free(Frame { addr: frame.start_address() });
...
pub fn translate(va: VirtAddr) -> Result<(PhysAddr, VmFlags, usize), VmErr> {
    ...
    // Try to translate as 4KB page first
    if let Some(result) = mapper.translate_addr(va) {
        let phys_addr = result;
...
pub fn map_stack_with_guard(base: VirtAddr, size: usize, flags: VmFlags) -> Result<(), VmErr> {
    ...
    for p in 0..stack_pages {
        map4k_at(
            VirtAddr::new(base.as_u64() + (p * PAGE_SIZE) as u64),
            phys_alloc(AllocFlags::empty()).ok_or(VmErr::NoMemory)?.addr,
            flags,
        )?;
    }
    Ok(())
...
pub fn gc_tables() -> Result<(), VmErr> {
    // For simplicity, we skip a full walk here; we can implement a walker that
...
impl MapCtx {
    #[inline]
    pub fn root() -> Result<&'static mut OffsetPageTable<'static>, VmErr> {
        let mut root = root_mut()?;
        Ok(unsafe { &mut *(&mut root as *mut OffsetPageTable<'static>) })
    }
}
