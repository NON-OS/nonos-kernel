...
use x86_64::{
    ...
    structures::paging::{
        ...
        mapper::MapperAllSizes, OffsetPageTable,
...
pub fn translate(va: VirtAddr) -> Result<(PhysAddr, VmFlags, usize), VmErr> {
    ...
    if let Ok(result) = mapper.translate(va) {
        let phys_addr = result.0;
...
pub struct MapCtx;
impl MapCtx {
    #[inline] pub fn root() -> Result<&'static mut OffsetPageTable<'static>, VmErr> { root_mut() }
}
