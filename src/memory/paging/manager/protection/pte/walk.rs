...
impl PagingManager {
    pub(in super::super) fn update_pte(&self, va: VirtAddr, new_flags: u64) -> PagingResult<()> {
        ...
        unsafe {
            let l4_table = &*((layout::DIRECTMAP_BASE + cr3.as_u64()) as *const [u64; PAGE_TABLE_ENTRIES]);
            if !pte_is_present(l4_table[l4_idx]) { return Err(PagingError::Pml4NotPresent); }
            let l3_pa = PhysAddr::new(pte_address(l4_table[l4_idx]));
            let l3_table = &mut *((layout::DIRECTMAP_BASE + l3_pa.as_u64()) as *mut [u64; PAGE_TABLE_ENTRIES]);
            if !pte_is_present(l3_table[l3_idx]) { return Err(PagingError::PdptNotPresent); }
            ...
            let l2_table = &mut *((layout::DIRECTMAP_BASE + l2_pa.as_u64()) as *mut [u64; PAGE_TABLE_ENTRIES]);
            if !pte_is_present(l2_table[l2_idx]) { return Err(PagingError::PdNotPresent); }
            ...
            let l1_table = &mut *((layout::DIRECTMAP_BASE + l1_pa.as_u64()) as *mut [u64; PAGE_TABLE_ENTRIES]);
