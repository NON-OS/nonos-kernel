...
use super::{brand, render, input::*, state::*};
...
fn list(t: &str, items: &[&str], s: usize, c: usize, h: u32, step: usize) {
    ...
    for (i, n) in items.iter().enumerate() { render::menu_item(200, 100+i as u32*35, 400, &if i==c { alloc::format!("{} *", n) } else { (*n).into() }, i==s); }
...
fn input(cfg: &mut SetupConfig, scr: Screen, s: usize) -> Screen {
    let max = match scr { Screen::Language => LANGUAGES.len()-1, Screen::Wallpaper => WALLPAPERS.len()-1, Screen::Crypto => 4, _ => 0 };
    match poll_menu_input() {
        MenuAction::Up if s > 0 => { SEL.store(s-1, Ordering::Relaxed); scr }
        MenuAction::Down if s < max => { SEL.store(s+1, Ordering::Relaxed); scr }
        MenuAction::Select => { SEL.store(0, Ordering::Relaxed); sel(cfg, scr, s) }
        MenuAction::Back => { SEL.store(0, Ordering::Relaxed); match scr { Screen::Language => Screen::Welcome, Screen::Wallpaper => Screen::Language, Screen::Crypto => Screen::Wallpaper, x => x } }
        MenuAction::Skip => { SEL.store(0, Ordering::Relaxed); match scr { Screen::Welcome | Screen::Crypto => Screen::Complete, Screen::Language => Screen::Wallpaper, Screen::Wallpaper => Screen::Crypto, x => x } }
        _ => scr,
    }
}
...
fn sel(cfg: &mut SetupConfig, scr: Screen, s: usize) -> Screen {
    match scr {
        ...
        Screen::Language => { cfg.language_index = s; Screen::Wallpaper }
        Screen::Wallpaper => { cfg.wallpaper_index = Some(s); Screen::Crypto }
        Screen::Crypto if s == 4 => Screen::Complete,
        Screen::Crypto => { match s { 0 => cfg.generate_keys = !cfg.generate_keys, 1 => cfg.hardware_crypto = !cfg.hardware_crypto, 2 => cfg.zk_attestation = !cfg.zk_attestation, 3 => cfg.developer_mode = !cfg.developer_mode, _ => {} }; SEL.store(s, Ordering::Relaxed); Screen::Crypto }
        x => x,
    }
}
