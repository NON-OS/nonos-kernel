...
// pub mod entry; // Commented out - conflicts with direct UEFI boot approach in
// main.rs
pub mod multiboot;
...
/// Early initialization before memory allocator is ready (minimal to avoid
/// higher-half access)
pub fn init_early() {
...
unsafe fn init_memory(boot_info: &'static BootInfo) -> Result<(), &'static str> {
    ...
    for i in 0..(HEAP_SIZE / 4096) {
        ...
        let frame =
            crate::memory::phys::alloc(AllocFlags::empty()).expect("Failed to allocate heap frame");
...
unsafe fn init_cpu_structures() {
    ...
    // Load GDT and IDT
    x86_64::instructions::tables::load_tss(crate::arch::x86_64::gdt::selectors().tss);
}
