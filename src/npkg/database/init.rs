...
pub fn init_database() -> NpkgResult<()> {
    if DB_INITIALIZED.swap(true, Ordering::SeqCst) { return Ok(()); }
...
fn load_database(db: &mut PackageDatabase, data: &[u8]) -> NpkgResult<()> {
    if data.len() < 8 { return Err(NpkgError::DatabaseCorrupt); }
    let magic = u32::from_le_bytes([data[0], data[1], data[2], data[3]]);
    if magic != 0x4E504B47 { return Err(NpkgError::DatabaseCorrupt); }
    let version = u32::from_le_bytes([data[4], data[5], data[6], data[7]]);
    if version != DB_VERSION { return Err(NpkgError::DatabaseCorrupt); }
    let mut offset = 8;
    while offset + 4 <= data.len() {
        let entry_len = u32::from_le_bytes([data[offset], data[offset + 1], data[offset + 2], data[offset + 3]]) as usize;
        offset += 4;
