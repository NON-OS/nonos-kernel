...
use crate::npkg::types::{Package, InstalledPackage, PackageState, InstallReason};
...
use crate::npkg::hooks::{run_pre_install, run_post_install};
use crate::npkg::signature::verify_package;
use crate::npkg::error::{NpkgError, NpkgResult};
use super::options::InstallOptions;

pub(super) fn install_single_package(pkg: &Package, reason: InstallReason, options: &InstallOptions) -> NpkgResult<()> {
    ...
    if let Some(ref sig_bytes) = pkg.meta.signature {
        let sig = crate::npkg::signature::PackageSignature { bytes: *sig_bytes, key_id: [0u8; 8], timestamp: 0 };
        verify_package(&archive_data, &sig)?;
    }
    if !options.no_scripts {
        if let Some(ref script) = pkg.install_script { run_pre_install(&pkg.meta.name, script)?; }
    }
    ...
    let installed_pkg = InstalledPackage {
        meta: pkg.meta.clone(), install_time: crate::time::unix_timestamp(),
        install_reason: reason, files: installed_files, state: PackageState::Installed,
    };
    ...
    if !options.no_scripts {
        if let Some(ref script) = pkg.install_script { run_post_install(&pkg.meta.name, script)?; }
    }
