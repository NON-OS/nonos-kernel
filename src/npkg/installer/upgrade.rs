...
pub fn upgrade_packages(names: &[&str], options: &UpgradeOptions) -> NpkgResult<()> {
    for name in names { if !is_installed(name) { return Err(NpkgError::NotInstalled(String::from(*name))); } }
    let resolution = if options.no_deps {
        ...
        for name in names {
            let current = query_by_name(name).ok_or_else(|| NpkgError::NotInstalled(String::from(*name)))?;
            let latest = crate::npkg::repository::find_package(name).ok_or_else(|| NpkgError::PackageNotFound(String::from(*name)))?;
            if latest.meta.version > current.meta.version { result.to_upgrade.push((latest, current.meta.version)); }
            else { result.satisfied.push(String::from(*name)); }
        }
        result
    } else { resolve_dependencies(names)? };
    if resolution.to_upgrade.is_empty() && resolution.to_install.is_empty() {
        if !resolution.satisfied.is_empty() { return Err(NpkgError::UpgradeNotNeeded(resolution.satisfied[0].clone())); }
    }
    if options.download_only { for (pkg, _) in &resolution.to_upgrade { let _ = download_package(pkg)?; } return Ok(()); }
