...
use super::super::database::{query_by_name, query_by_file, get_database_stats, verify_database_integrity};
use super::super::cache::{clear_cache, cache_stats};
...
pub fn cmd_verify(_args: &[&str]) {
    ...
    match verify_database_integrity() {
        Ok(issues) => {
            if issues.is_empty() { print_line(b"database integrity OK"); }
            else {
                print_line_fmt(alloc::format!("found {} issues:", issues.len()).as_bytes());
                for issue in issues { print_line_fmt(alloc::format!("  {}", issue).as_bytes()); }
            }
        }
        Err(e) => print_line_fmt(alloc::format!("error: {}", e.message()).as_bytes()),
    }
...
pub fn cmd_files(args: &[&str]) {
    if args.is_empty() { print_line(b"usage: npkg files <package>"); return; }
    let name = args[0];
    if let Some(pkg) = query_by_name(name) { for file in &pkg.files { print_line_fmt(file.as_bytes()); } }
    else { print_line_fmt(alloc::format!("package not installed: {}", name).as_bytes()); }
}

pub fn cmd_owner(args: &[&str]) {
    if args.is_empty() { print_line(b"usage: npkg owner <file>"); return; }
    let path = args[0];
    if let Some(owner) = query_by_file(path) { print_line_fmt(alloc::format!("{} is owned by {}", path, owner).as_bytes()); }
    else { print_line_fmt(alloc::format!("{} is not owned by any package", path).as_bytes()); }
}
...
pub fn cmd_stats() {
    ...
    } else { print_line(b"database not initialized"); }
}
