...
use super::types::{PackageArchive, ArchiveEntry, NPKG_MAGIC, NPKG_VERSION, ENTRY_FILE};
use super::iterator::ArchiveIterator;

impl<'a> PackageArchive<'a> {
    pub fn open(data: &'a [u8]) -> NpkgResult<Self> {
        if data.len() < 24 { return Err(NpkgError::ArchiveCorrupt(String::from("too small"))); }
        let magic = u32::from_le_bytes([data[0], data[1], data[2], data[3]]);
        if magic != NPKG_MAGIC { return Err(NpkgError::ArchiveCorrupt(String::from("bad magic"))); }
        let version = u32::from_le_bytes([data[4], data[5], data[6], data[7]]);
        if version > NPKG_VERSION { return Err(NpkgError::ArchiveCorrupt(String::from("unsupported version"))); }
        ...
        if entries_offset >= data.len() || data_offset >= data.len() { return Err(NpkgError::ArchiveCorrupt(String::from("bad offsets"))); }
        Ok(Self { data, version, file_count, entries_offset, data_offset })
    }

    pub fn file_count(&self) -> u32 { self.file_count }

    pub fn archive_version(&self) -> u32 { self.version }
    ...
    pub fn read_file(&self, entry: &ArchiveEntry) -> NpkgResult<Vec<u8>> {
        if entry.entry_type != ENTRY_FILE { return Err(NpkgError::ExtractionFailed(String::from("not a file"))); }
        let start = self.data_offset + entry.data_offset as usize;
        let end = start + entry.size as usize;
        if end > self.data.len() { return Err(NpkgError::ArchiveCorrupt(String::from("data out of bounds"))); }
        let file_data = &self.data[start..end];
        let actual_checksum = crate::crypto::blake3::blake3_hash(file_data);
        if actual_checksum != entry.checksum { return Err(NpkgError::ChecksumMismatch(entry.path.clone())); }
        Ok(file_data.to_vec())
    }
}
