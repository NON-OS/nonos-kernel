...
use super::types::{PackageArchive, ArchiveEntry, ENTRY_SYMLINK};
...
impl<'a> Iterator for ArchiveIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if offset + 4 > data.len() { return Some(Err(NpkgError::ArchiveCorrupt(String::from("truncated entry")))); }
        let path_len = u16::from_le_bytes([data[offset], data[offset + 1]]) as usize;
        let entry_type = data[offset + 2];
        if offset + 4 + path_len > data.len() { return Some(Err(NpkgError::ArchiveCorrupt(String::from("truncated path")))); }
        let path = match core::str::from_utf8(&data[offset + 4..offset + 4 + path_len]) {
            Ok(s) => String::from(s), Err(_) => return Some(Err(NpkgError::ArchiveCorrupt(String::from("invalid path"))))
        };
        let mut pos = offset + 4 + path_len;
        if pos + 48 > data.len() { return Some(Err(NpkgError::ArchiveCorrupt(String::from("truncated metadata")))); }
        let size = u64::from_le_bytes([data[pos], data[pos+1], data[pos+2], data[pos+3], data[pos+4], data[pos+5], data[pos+6], data[pos+7]]);
        pos += 8;
        ...
        pos += 32;
        let data_offset = u64::from_le_bytes([data[pos], data[pos+1], data[pos+2], data[pos+3], data[pos+4], data[pos+5], data[pos+6], data[pos+7]]);
        pos += 8;
        let link_target = if entry_type == ENTRY_SYMLINK {
            let link_len = u16::from_le_bytes([data[pos], data[pos + 1]]) as usize;
            pos += 2;
            if pos + link_len > data.len() { return Some(Err(NpkgError::ArchiveCorrupt(String::from("truncated link")))); }
            match core::str::from_utf8(&data[pos..pos + link_len]) {
                Ok(s) => { pos += link_len; Some(String::from(s)) }
