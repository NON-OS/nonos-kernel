...
pub(crate) fn test_manifest_builder_full() -> TestResult {
    ...
    if manifest.package.meta.name != "testpkg" { return TestResult::Fail; }
    if manifest.package.meta.version.major != 1 { return TestResult::Fail; }
    if manifest.package.meta.description != "A test package" { return TestResult::Fail; }
    if manifest.package.meta.license != "MIT" { return TestResult::Fail; }
    if manifest.package.meta.architecture != Architecture::X86_64 { return TestResult::Fail; }
    if manifest.package.meta.kind != PackageKind::Binary { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_manifest_builder_default_license() -> TestResult {
    ...
    if manifest.package.meta.license != "AGPL-3.0" { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_manifest_builder_with_dependency() -> TestResult {
    ...
    if manifest.package.dependencies.len() != 1 { return TestResult::Fail; }
    if manifest.package.dependencies[0].name != "libfoo" { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_manifest_builder_with_install_script() -> TestResult {
    ...
    if manifest.package.install_script != Some(alloc::string::String::from("mkdir -p /opt/test")) { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_manifest_builder_with_remove_script() -> TestResult {
    ...
    if manifest.package.remove_script != Some(alloc::string::String::from("rm -rf /opt/test")) { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_with_quotes() -> TestResult {
    ...
    if result.is_err() { return TestResult::Fail; }
    let manifest = result.unwrap();
    if manifest.package.meta.name != "my-package" { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_with_arch_shorthand() -> TestResult {
    ...
    if result.is_err() { return TestResult::Fail; }
    let manifest = result.unwrap();
    if manifest.package.meta.architecture != Architecture::Aarch64 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_with_kind() -> TestResult {
    ...
    if result.is_err() { return TestResult::Fail; }
    let manifest = result.unwrap();
    if manifest.package.meta.kind != PackageKind::Library { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_with_type_shorthand() -> TestResult {
    ...
    if result.is_err() { return TestResult::Fail; }
    let manifest = result.unwrap();
    if manifest.package.meta.kind != PackageKind::Driver { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_with_dependencies() -> TestResult {
    ...
    if result.is_err() { return TestResult::Fail; }
    let manifest = result.unwrap();
    if manifest.package.dependencies.len() != 2 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_with_optional_dependency() -> TestResult {
    ...
    let manifest = result.unwrap();
    if manifest.package.dependencies.len() != 1 { return TestResult::Fail; }
    if manifest.package.dependencies[0].kind != DependencyKind::Optional { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_with_conflict() -> TestResult {
    ...
    if result.is_err() { return TestResult::Fail; }
    let manifest = result.unwrap();
    if manifest.package.dependencies[0].kind != DependencyKind::Conflict { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_with_files() -> TestResult {
    ...
    if result.is_err() { return TestResult::Fail; }
    let manifest = result.unwrap();
    if manifest.package.files.len() != 2 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_file_executable() -> TestResult {
    ...
    if result.is_err() { return TestResult::Fail; }
    let manifest = result.unwrap();
    if manifest.package.files[0].permissions.mode != 0o755 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_file_config() -> TestResult {
    ...
    if result.is_err() { return TestResult::Fail; }
    let manifest = result.unwrap();
    if !manifest.package.files[0].is_config { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_file_directory() -> TestResult {
    ...
    if result.is_err() { return TestResult::Fail; }
    let manifest = result.unwrap();
    if !manifest.package.files[0].is_directory { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_with_install_section() -> TestResult {
    ...
    if result.is_err() { return TestResult::Fail; }
    let manifest = result.unwrap();
    if manifest.package.install_script.is_none() { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_parse_manifest_with_remove_section() -> TestResult {
    ...
    if result.is_err() { return TestResult::Fail; }
    let manifest = result.unwrap();
    if manifest.package.remove_script.is_none() { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_serialize_manifest_simple() -> TestResult {
    ...
    if !text.contains("name = \"test\"") { return TestResult::Fail; }
    if !text.contains("version = \"1.0.0\"") { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_serialize_then_parse() -> TestResult {
    ...
    if original.package.meta.name != parsed.package.meta.name { return TestResult::Fail; }
    if original.package.meta.version != parsed.package.meta.version { return TestResult::Fail; }
    if original.package.meta.license != parsed.package.meta.license { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_manifest_new() -> TestResult {
    ...
    if manifest.package.meta.name != "test" { return TestResult::Fail; }
    if !manifest.raw_bytes().is_empty() { return TestResult::Fail; }
    TestResult::Pass
}
