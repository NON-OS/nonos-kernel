...
impl NvmeDriver {
    ...
    pub fn set_timeout(timeout_spins: u32) -> Result<(), NvmeError> {
        get_controller().ok_or(NvmeError::ControllerNotInitialized)?.set_timeout(timeout_spins); Ok(())
    }
    pub fn set_rate_limit(limit_per_sec: u32) -> Result<(), NvmeError> {
        get_controller().ok_or(NvmeError::ControllerNotInitialized)?.set_rate_limit(limit_per_sec); Ok(())
    }
    ...
    pub fn reset_stats() -> Result<(), NvmeError> {
        get_controller().ok_or(NvmeError::ControllerNotInitialized)?.reset_stats(); Ok(())
    }
    ...
    pub fn get_namespace_info() -> Result<NamespaceInfo, NvmeError> {
        ...
        Ok(NamespaceInfo { nsid: ns.nsid(), block_count: ns.block_count(), block_size: ns.block_size(), capacity_bytes: ns.capacity_bytes() })
    }
