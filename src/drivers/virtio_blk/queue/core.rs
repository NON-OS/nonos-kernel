...
use core::ptr;
use super::constants::{VQ_REGION_SIZE, DATA_BUF_SIZE};
use super::types::{VirtqueueRegionBuf, BlkDataBuf};
...
impl BlkQueue {
    pub(crate) fn new() -> Result<Self, &'static str> {
        ...
        if vq_base & 0xFFF != 0 { return Err("virtio-blk: VQ not aligned"); }
        if buf_base & 0xFFF != 0 { return Err("virtio-blk: buffer not aligned"); }
        unsafe {
            ptr::write_bytes(vq_base as *mut u8, 0, VQ_REGION_SIZE);
            ptr::write_bytes(buf_base as *mut u8, 0, DATA_BUF_SIZE);
        }
        Ok(Self { vq_base, buf_base, last_used_idx: 0, next_avail_idx: 0, next_desc_idx: 0, notify_port: 0, notify_mmio: 0 })
    }
