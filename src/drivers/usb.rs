...
impl UsbDevice {
    ...
    pub fn read_sample_data(&self, size: usize) -> Result<Vec<u8>, &'static str> {
        ...
        let controller = controllers.iter_mut()
...
impl XhciController {
    ...
    fn start_controller(&mut self) -> Result<(), &'static str> {
        ...
        while timeout > 0 {
            ...
            if (usbsts & 0x01) == 0 { // HCHalted bit cleared
                break;
            }
            crate::arch::x86_64::delay::delay_ms(1);
            timeout -= 1;
    ...
    fn enumerate_port(&mut self, port_num: u8) -> Result<UsbDevice, &'static str> {
        ...
        let portsc = unsafe { 
            read_volatile((self.operational_regs + port_offset) as *const u32) 
        };
    ...
    fn reset_port(&mut self, port_num: u8) -> Result<(), &'static str> {
        ...
        while timeout > 0 {
            ...
            if (portsc & 0x10) == 0 { // Reset bit cleared
                break;
            }
            crate::arch::x86_64::delay::delay_ms(1);
            timeout -= 1;
    ...
    fn control_transfer(&mut self, slot_id: u8, setup: &[u8], data: &mut [u8]) -> Result<(), &'static str> {
    ...
    fn allocate_dcbaa(&self) -> Result<usize, &'static str> {
        ...
        let dcbaa = crate::memory::alloc::allocate_kernel_pages(
            (dcbaa_size + 0xfff) / 0x1000
    ...
    fn setup_address_device_input_context(&mut self, slot_id: u8, port_num: u8) -> Result<usize, &'static str> {
        ...
        let input_context = crate::memory::alloc::allocate_kernel_pages(
            (input_context_size + 0xfff) / 0x1000
        ...
        unsafe {
            ...
            *ep0_context.add(4) = 0x00080040; // Average TRB Length = 8, Max Packet Size = 64
        }
    ...
    fn handle_port_status_change(&mut self, port_id: u8) {
        ...
        if (portsc & 0x02) != 0 { // Connect Status Change
            if (portsc & 0x01) != 0 { // Device connected
    ...
    pub fn has_device_on_port(&self, port: u8) -> bool {
        ...
        let portsc = unsafe {
            read_volatile((self.operational_regs + port_offset as usize) as *const u32)
        };
    ...
    pub fn bulk_transfer_out(&mut self, device_addr: u8, endpoint: u8, data: &[u8]) -> Result<(), &'static str> {
    ...
    pub fn bulk_transfer_in(&mut self, device_addr: u8, endpoint: u8, buffer: &mut [u8]) -> Result<(), &'static str> {
        // Find device slot for this address
        let slot_id = self.find_slot_for_address(device_addr)?;
...
impl CommandRing {
    fn new() -> Result<Self, &'static str> {
        ...
        let trbs = crate::memory::alloc::allocate_kernel_pages(
            (ring_bytes + 0xfff) / 0x1000
        ).map_err(|_| "Failed to allocate command ring")?.as_u64() as *mut Trb;
...
impl EventRing {
    fn new() -> Result<Self, &'static str> {
        ...
        let segment_trbs = crate::memory::alloc::allocate_kernel_pages(
            (segment_bytes + 0xfff) / 0x1000
        ).map_err(|_| "Failed to allocate event ring segment")?.as_u64() as *mut Trb;
        
        // Allocate segment table
        let erst = crate::memory::alloc::allocate_kernel_pages(1)
            .map_err(|_| "Failed to allocate ERST")?.as_u64() as *mut EventRingSegmentTable;
        ...
        let segments = crate::memory::alloc::allocate_kernel_pages(1)
            .map_err(|_| "Failed to allocate event ring segments")?.as_u64() as *mut EventRingSegment;
