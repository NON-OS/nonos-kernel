...
//!
//! High-performance graphics driver with NONOS cryptographic frame buffer
//! protection
...
#[derive(Debug, Clone)]
pub enum GpuCommand {
    ClearSurface { surface_id: u32, color: u32 },
    BlitSurface { src_surface: u32, dst_surface: u32, src_rect: Rectangle, dst_rect: Rectangle },
    DrawRectangle { surface_id: u32, rect: Rectangle, color: u32 },
    DrawTriangle { surface_id: u32, vertices: [Point; 3], color: u32 },
    SetPixel { surface_id: u32, x: u32, y: u32, color: u32 },
    CopyBuffer { src_addr: u64, dst_addr: u64, size: usize },
    ExecuteShader { shader_id: u32, uniforms: Vec<f32>, vertices: Vec<f32> },
}
...
impl GpuDriver {
    ...
    pub fn init(&mut self) -> Result<(), &'static str> {
        ...
        crate::log::logger::log_critical(&format!(
            "GPU: Initialized {:04X}:{:04X} at {}x{}",
            self.vendor_id, self.device_id, default_mode.width, default_mode.height
        ));
    ...
    fn execute_command(&mut self, command: GpuCommand) -> Result<(), &'static str> {
        match command {
            GpuCommand::ClearSurface { surface_id, color } => self.clear_surface(surface_id, color),
