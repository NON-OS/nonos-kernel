...
impl XhciController {
    ...
    pub fn init(&mut self) -> Result<(), &'static str> {
        unsafe {
            ...
            while timeout > 0 {
                let usbsts = (*self.op_regs).usbsts;
                if (usbsts & (1 << 11)) == 0 { // Controller Not Ready
                    break;
                }
                timeout -= 1;
            }
            ...
            crate::log::logger::log_critical(&format!("xHCI: Controller initialized with {} slots, {} ports", 
    ...
    fn scan_ports(&mut self) {
        unsafe {
            for port in 1..=self.max_ports {
                ...
                if (portsc & (1 << 0)) != 0 { // Current Connect Status
                    crate::log::logger::log_critical(&format!("xHCI: Device detected on port {}", port));
                    ...
                    while timeout > 0 {
                        let portsc = (*self.op_regs).portsc[(port - 1) as usize];
                        if (portsc & (1 << 4)) == 0 { // Port Reset Clear
                            break;
                        }
                        timeout -= 1;
                    }
    ...
    pub fn transfer_data(&mut self, slot_id: u32, endpoint: u8, data: &[u8], direction: TransferDirection) -> Result<usize, &'static str> {
        ...
        if self.security_enabled {
            ...
            let allowed = whitelist.iter().any(|(vid, pid)| {
    ...
    /// Build transfer TRB
    fn build_transfer_trb(&mut self, _slot_id: u32, _endpoint: u8, _data: &[u8], _direction: TransferDirection) -> Result<(), &'static str> {
    ...
    pub fn handle_interrupt(&mut self) {
        ...
        unsafe {
            ...
            let usbsts = (*self.op_regs).usbsts;
            if (usbsts & (1 << 3)) != 0 { // Event Interrupt
                self.process_events();
    ...
    pub fn remove_from_whitelist(&mut self, vendor_id: u16, product_id: u16) {
        self.device_whitelist.write().retain(|(vid, pid)| {
