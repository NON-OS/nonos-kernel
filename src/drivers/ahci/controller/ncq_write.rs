...
use super::super::constants::{PORT_SACT, PORT_CI};
use super::{encryption, validation, io, ncq};
use super::helpers::RegisterAccess;
use super::ncq_wait::wait_ncq_complete;

pub(crate) fn ncq_write_sectors<T: RegisterAccess>(
    ctrl: &T, ports: &RwLock<BTreeMap<u32, AhciDevice>>, port_dma: &Mutex<BTreeMap<u32, PortDma>>,
    validation_failures: &AtomicU64, write_ops: &AtomicU64, bytes_written: &AtomicU64, errors: &AtomicU64,
    port_resets: &AtomicU64, encryption_enabled: &AtomicBool, aes_cipher: &Mutex<Option<Aes256>>,
    encryption_iv: &[u8; 16], command_timeout: u32, port: u32, lba: u64, count: u16, buffer_va: u64,
) -> Result<(), AhciError> {
    let supports_ncq = ports.read().get(&port).map(|d| d.supports_ncq).unwrap_or(false);
    if !supports_ncq {
        return io::write_sectors(ctrl, ports, port_dma, validation_failures, write_ops, bytes_written,
            errors, port_resets, encryption_enabled, aes_cipher, encryption_iv, command_timeout,
            port, lba, count, buffer_va);
    }
    if !ports.read().contains_key(&port) { return Err(AhciError::PortNotInitialized); }
