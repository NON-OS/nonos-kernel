...
use alloc::{vec::Vec, collections::VecDeque, format};
...
impl HdAudioController {
    ...
    pub fn new(pci_device: &PciDevice) -> Result<Self, &'static str> {
        ...
        unsafe {
            ...
            let corb_frame = crate::memory::page_allocator::allocate_frame()
                .ok_or("Failed to allocate CORB")?;
            let corb = corb_frame.start_address().as_u64() as *mut u32;
            
            // Allocate RIRB (Response Inbound Ring Buffer) 
            let rirb_frame = crate::memory::page_allocator::allocate_frame()
                .ok_or("Failed to allocate RIRB")?;
    ...
    pub fn init(&mut self) -> Result<(), &'static str> {
        unsafe {
            ...
            crate::log::logger::log_critical(&format!("HD Audio: Initialized with {} input, {} output, {} bidirectional streams", 
        ...
        unsafe {
            ...
            for codec_addr in 0..15 {
                if (statests & (1 << codec_addr)) != 0 {
                    crate::log::logger::log_critical(&format!("HD Audio: Codec found at address {}", codec_addr));
    ...
    fn send_codec_command(&self, codec_addr: u8, node_id: u8, verb: u32, param: u16) -> Result<u64, &'static str> {
        unsafe {
            // Build command
    ...
    /// Create audio stream
    pub fn create_stream(&mut self, stream_type: StreamType, format: AudioFormat) -> Result<u32, &'static str> {
    ...
    pub fn start_stream(&mut self, stream_id: u32) -> Result<(), &'static str> {
        ...
        let descriptor_index = match stream.stream_type {
            ...
            StreamType::Bidirectional => (self.num_output_streams + self.num_input_streams) as u32 + stream_id - 1,
        };
    ...
    /// Record audio
    pub fn start_recording(&mut self, format: AudioFormat, process_id: u64) -> Result<(), &'static str> {
        if self.secure_mode {
            let authorized = self.authorized_processes.read();
            if !authorized.contains(&process_id) {
    ...
    pub fn handle_interrupt(&mut self) {
        ...
        unsafe {
            ...
            if (intsts & (1 << 30)) != 0 { // RIRB interrupt
