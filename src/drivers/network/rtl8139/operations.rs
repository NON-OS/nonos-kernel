...
impl Rtl8139 {
    pub fn poll_rx(&self) {
        ...
        while offset != cbr {
            ...
            unsafe {
                ...
                for i in 0..pkt_len { pkt.push(*base.add((data_offset + i) % (RX_BUF_SIZE - 16))); }
            }
            self.rx_queue.lock().push(pkt);

            offset = (offset + len + 4 + 3) & !3;
            if offset >= RX_BUF_SIZE - 16 { offset -= RX_BUF_SIZE - 16; }
        }
    ...
    pub fn transmit(&self, data: &[u8]) -> Result<(), ()> {
        if !self.initialized.load(Ordering::SeqCst) || data.len() > TX_BUF_SIZE { return Err(()); }
