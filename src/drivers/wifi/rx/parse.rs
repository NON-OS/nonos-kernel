...
use super::constants::{FRAME_TYPE_MGMT, FRAME_TYPE_CTRL, FRAME_TYPE_DATA};
...
impl _RxProcessor {
    pub(super) fn parse_frame(&self, data: &[u8]) -> Option<_RxFrameInfo> {
        ...
        if frame_type == _FrameType::Data && (subtype & 0x08) != 0 { payload_offset += 2; }
        if (frame_control & 0x8000) != 0 { payload_offset += 4; }
        if to_ds && from_ds { payload_offset += 6; }
        let payload_len = if data.len() > payload_offset { data.len() - payload_offset } else { 0 };
        Some(_RxFrameInfo {
            frame_type, subtype, to_ds, from_ds, protected,
            addr1, addr2, addr3, seq_num, payload_offset, payload_len,
        })
    }
}
