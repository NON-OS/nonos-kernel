...
#[derive(Debug, Clone, Copy)]
pub enum PciBar {
    Memory { address: PhysAddr, size: usize, prefetchable: bool, address_64bit: bool },
        ...
        while cap_ptr != 0 && cap_ptr != 0xFF {
            ...
            for i in 0..cap_length {
                if i % 4 == 0 {
                    ...
                    if i + 1 < cap_length {
                        cap_data.push(((dword >> 8) & 0xFF) as u8);
                    }
                    if i + 2 < cap_length {
                        cap_data.push(((dword >> 16) & 0xFF) as u8);
                    }
                    if i + 3 < cap_length {
                        cap_data.push(((dword >> 24) & 0xFF) as u8);
                    }
                }
            }
...
impl PciManager {
    ...
    pub fn get_device_info(&self, bus: u8, device: u8, function: u8) -> Option<(u16, u16, u8)> {
        ...
        devices
            ...
            .map(|dev| (dev.vendor_id, dev.device_id, dev.class_code))
    }
