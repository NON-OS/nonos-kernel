...
impl HdAudioController {
    #[inline] pub fn get_volume(&self) -> Volume { Volume::new(self.volume.load(Ordering::Relaxed)) }
    ...
    pub fn set_volume(&self, volume: u8) -> Result<(), AudioError> {
        ...
        if let (Some(cad), Some(ref paths)) = (self.primary_codec, &self.codec_paths) {
            codec::set_volume(self, &self.corb, &self.rirb, self.corb_entries, self.rirb_entries, cad, paths, vol)?;
        }
        Ok(())
    }

    #[inline] pub fn is_muted(&self) -> bool { self.muted.load(Ordering::Relaxed) }

    pub fn set_mute(&self, mute: bool) -> Result<(), AudioError> {
        self.muted.store(mute, Ordering::Relaxed);
        if let (Some(cad), Some(ref paths)) = (self.primary_codec, &self.codec_paths) {
            codec::set_mute(self, &self.corb, &self.rirb, self.corb_entries, self.rirb_entries, cad, paths, mute)?;
        }
        Ok(())
    }
