...
pub(crate) fn discover_codec<T: RegisterAccess>(ctrl: &T, corb: &DmaRegion, rirb: &DmaRegion,
    corb_entries: usize, rirb_entries: usize, cad: u8) -> Result<CodecInfo, AudioError> {
    let vendor_device = get_parameter(ctrl, corb, rirb, corb_entries, rirb_entries, cad, 0, PARAM_VENDOR_ID)?;
    let vendor_id = (vendor_device >> 16) as u16;
    let device_id = (vendor_device & 0xFFFF) as u16;
    if vendor_id == 0x0000 || vendor_id == 0xFFFF { return Err(AudioError::NoCodecPresent); }
    let revision_id = get_parameter(ctrl, corb, rirb, corb_entries, rirb_entries, cad, 0, PARAM_REVISION_ID).unwrap_or(0);
    let sub_nodes = get_parameter(ctrl, corb, rirb, corb_entries, rirb_entries, cad, 0, PARAM_SUB_NODE_COUNT).unwrap_or(0);
    ...
    if quirks.has_quirks() { increment_quirks_applied(); }
