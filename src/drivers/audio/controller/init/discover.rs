    ...
    for cad in 0..=15u8 {
        if (codec_mask & (1 << cad)) != 0 {
            match discover_codec(ctrl, corb, rirb, corb_entries, rirb_entries, cad) {
                Ok(info) => {
                    crate::log::logger::log_critical(&alloc::format!("HDA: Codec {} - {} {} (0x{:04X}:0x{:04X})", cad, info.vendor_name(), info.device_name(), info.vendor_id, info.device_id));
                    if info.quirks.has_quirks() {
                        crate::log::logger::log_critical(&alloc::format!("HDA: Applying {} quirks for codec {}", info.quirks.quirk_count(), cad));
                        let _ = apply_codec_quirks(ctrl, corb, rirb, corb_entries, rirb_entries, &info);
                    }
                    match init_codec_path(ctrl, corb, rirb, corb_entries, rirb_entries, &info) {
                        Ok(_) => {} Err(e) => { crate::log::logger::log_critical(&alloc::format!("HDA: Path init failed for codec {}: {:?}", cad, e)); }
                    }
                    codecs.push(info);
                }
                Err(e) => { crate::log::logger::log_critical(&alloc::format!("HDA: Codec {} discovery failed: {:?}", cad, e)); }
            }
        }
    }
    codecs
