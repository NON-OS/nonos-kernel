...
//! HD Audio MMIO register access helpers.
...
#[inline]
pub(super) fn spin_until<F: Fn() -> bool>(cond: F, mut max_spins: u32) -> bool {
    while max_spins > 0 {
        if cond() {
            return true;
        }
...
#[inline]
pub(super) fn spin_while<F: Fn() -> bool>(cond: F, mut max_spins: u32) -> bool {
    while max_spins > 0 {
        if !cond() {
            return true;
        }
