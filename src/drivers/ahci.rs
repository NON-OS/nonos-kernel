...
impl AhciController {
    ...
    fn init_port(&mut self, port: u32) -> Result<(), &'static str> {
        ...
        let fis_area =
            crate::memory::page_allocator::allocate_frame().ok_or("Failed to allocate FIS area")?;
    ...
    fn identify_device(&mut self, port: u32) -> Result<(), &'static str> {
        ...
        // Parse identify data
        let identify_data = unsafe { core::slice::from_raw_parts(buffer_addr as *const u16, 256) };
        ...
        crate::log::logger::log_critical(&format!(
            "AHCI: Found {} sectors device: {}",
            sectors, device.model
        ));
