...
pub fn test_pci_address_creation() -> TestResult {
    ...
    if addr.function != 2 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_pci_address_bdf_conversion() -> TestResult {
    ...
    if restored.bus != addr.bus { return TestResult::Fail; }
    if restored.device != addr.device { return TestResult::Fail; }
    if restored.function != addr.function { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_pci_address_display() -> TestResult {
    ...
    if display != "12:0a.3" { return TestResult::Fail; }
    TestResult::Pass
}
