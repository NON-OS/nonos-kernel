...
pub enum Violation { UnauthorizedCap, MemoryExceeded, CpuExceeded, MemoryAccess, IllegalSyscall }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SandboxState { Ready, Running, Blocked, Exited(i32), Violated(Violation) }
...
impl Sandbox {
    ...
    pub fn free_mem(&mut self, size: u64) { self.mem_used = self.mem_used.saturating_sub(size); }
    pub fn state(&self) -> SandboxState { self.state }
    pub fn set_state(&mut self, s: SandboxState) { self.state = s; }
    pub fn addr_space(&self) -> Arc<AddressSpace> { self.addr_space.clone() }
    pub fn terminate(&mut self, code: i32) { self.state = SandboxState::Exited(code); }
    pub fn caps(&self) -> u64 { self.caps }
    pub fn entry(&self) -> u64 { self.entry }
    pub fn mem_used(&self) -> u64 { self.mem_used }
