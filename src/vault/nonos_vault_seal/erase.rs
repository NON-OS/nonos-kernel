...
impl VaultSealStore {
    pub fn secure_erase_sealed(&self, backend: Option<SealPolicy>) {
        ...
        for entry in sealed.iter_mut() {
            for b in entry.sealed_data.iter_mut() { unsafe { core::ptr::write_volatile(b, 0) }; }
        ...
        if let Some(ref policy) = backend {
            match policy {
                SealPolicy::UEFI => {
                    let _ = uefi_set_variable("NONOS_VAULT_SECRET", &Guid::GLOBAL_VARIABLE, VariableAttributes::NON_VOLATILE | VariableAttributes::RUNTIME_ACCESS, &[]);
                }
                SealPolicy::Disk => { let _ = NonosFilesystem::new().delete_file("nonos_vault.sealed"); }
                SealPolicy::Custom(_) | SealPolicy::RAMOnly => {}
            }
        }
        let audit = VaultAuditEvent { timestamp: crate::time::timestamp_millis(), event: "secure_erase_sealed".into(), context: backend.map(|b| format!("{:?}", b)), status: Some("success".into()) };
