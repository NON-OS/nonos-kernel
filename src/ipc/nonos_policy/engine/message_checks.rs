...
impl IpcPolicy {
    pub(super) fn check_message_policy(&self, env: &IpcEnvelope, from: &str, to: &str, policy: &ModulePolicy) -> bool {
        if env.data.len() > policy.max_message_size && !policy.has_capability(IpcCapability::LargeMessages) {
            self.record_violation(PolicyViolation::MessageTooLarge {
                size: env.data.len(), limit: policy.max_message_size,
            });
        ...
        if !policy.allowed_destinations.is_empty() && !policy.allowed_destinations.iter().any(|d| d == to) {
            self.record_violation(PolicyViolation::DestinationBlocked {
                from: String::from(from), to: String::from(to),
            });
            self.stats.messages_denied.fetch_add(1, Ordering::Relaxed);
        ...
        if to.starts_with("kernel") && !policy.has_capability(IpcCapability::KernelAccess) {
            self.record_violation(PolicyViolation::MissingCapability {
                module: String::from(from), capability: IpcCapability::KernelAccess,
            });
            self.stats.messages_denied.fetch_add(1, Ordering::Relaxed);
            return false;
        }
    ...
    fn check_security_and_rate(&self, env: &IpcEnvelope, from: &str, _to: &str, policy: &ModulePolicy) -> bool {
        ...
        if env.sec_level == SecurityLevel::None &&
           (!self.allow_unsigned || !policy.has_capability(IpcCapability::AllowUnsigned)) {
            self.record_violation(PolicyViolation::SecurityLevelInsufficient {
                required: SecurityLevel::Signed, actual: SecurityLevel::None,
            });
            self.stats.messages_denied.fetch_add(1, Ordering::Relaxed);
            return false;
        }
        if !self.check_rate_limit(from, policy) {
            self.record_violation(PolicyViolation::RateLimitExceeded {
                module: String::from(from), limit: policy.rate_limit_per_sec,
            });
            self.stats.messages_denied.fetch_add(1, Ordering::Relaxed);
            return false;
        }
