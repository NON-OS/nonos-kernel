...
impl NonosIPCManager {
    pub fn send_message(&self, sender_id: u64, channel_id: u64, recipient_id: u64, message_type: NonosMessageType, payload: Vec<u8>) -> Result<u64, IpcManagerError> {
        if payload.len() > MAX_PAYLOAD_SIZE { return Err(IpcManagerError::PayloadTooLarge { size: payload.len(), max: MAX_PAYLOAD_SIZE }); }
    ...
    pub fn receive_message(&self, receiver_id: u64, channel_id: u64) -> Result<Option<NonosIPCMessage>, IpcManagerError> {
        ...
        if result.is_some() { self.stats.messages_received.fetch_add(1, Ordering::Relaxed); }
        Ok(result)
    }

    pub fn peek_message(&self, receiver_id: u64, channel_id: u64) -> Result<Option<NonosIPCMessage>, IpcManagerError> {
