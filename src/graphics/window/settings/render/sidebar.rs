...
pub(super) fn draw_sidebar(x: u32, y: u32, h: u32, current_page: u8) {
    ...
    let items: [(&[u8], u32); 5] = [
        (b"Privacy", 0xFF8B5CF6),
        (b"Network", 0xFF3B82F6),
        (b"Appearance", 0xFFF59E0B),
        (b"System", 0xFF10B981),
        (b"Power", 0xFFEF4444),
    ];
    for (i, (name, accent)) in items.iter().enumerate() {
        let ty = y + 56 + (i as u32) * 44;
        let is_sel = current_page == i as u8;
        if is_sel {
            fill_rounded_rect(x + 8, ty, SIDEBAR_WIDTH - 16, 36, 6, BG_SELECTED);
            fill_rect(x + 8, ty, 3, 36, *accent);
        }
        let text_col = if is_sel { TEXT_PRIMARY } else { TEXT_DIM };
        draw_string(x + 20, ty + 12, name, text_col);
    }
}
