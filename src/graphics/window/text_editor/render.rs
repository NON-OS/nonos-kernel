...
use super::state::*;
use super::render_ui::{draw_file_picker, draw_toolbar, draw_line_numbers, draw_status_bar};
...
pub(super) fn draw(x: u32, y: u32, w: u32, h: u32) {
    if picker_is_active() { draw_file_picker(x, y, w, h); return; }
    let tab_offset = if tabs_enabled() { draw_tabs(x, y, w); TAB_BAR_HEIGHT } else { 0 };
    let content_y = y + tab_offset;
    let content_h = h - tab_offset;
    draw_toolbar(x, content_y, w);
    draw_line_numbers(x, content_y + TOOLBAR_HEIGHT, content_h - TOOLBAR_HEIGHT - STATUS_BAR_HEIGHT);
