...
use super::tabs_state::{MAX_TABS, TABS, ACTIVE_TAB, TAB_COUNT, TabBuffer};
...
pub fn new_tab() -> bool {
    ...
    super::tabs_sync::save_current_to_tab();
    unsafe { TABS[count] = TabBuffer::new(); TABS[count].active = true; }
...
pub fn close_tab(idx: usize) -> bool {
    let count = TAB_COUNT.load(Ordering::Relaxed);
    if count <= 1 || idx >= count { return false; }
    unsafe {
        for i in idx..count-1 { TABS[i] = core::mem::replace(&mut TABS[i+1], TabBuffer::new()); }
        TABS[count-1].active = false;
    }
    TAB_COUNT.store(count - 1, Ordering::Relaxed);
    let active = ACTIVE_TAB.load(Ordering::Relaxed);
    if active >= count - 1 { ACTIVE_TAB.store(count - 2, Ordering::Relaxed); }
    super::tabs_sync::load_tab_to_current();
    true
}

pub fn get_tab_name(idx: usize) -> &'static [u8] {
    unsafe { if idx < MAX_TABS && TABS[idx].path_len > 0 { &TABS[idx].path[..TABS[idx].path_len] } else { b"untitled" } }
}

pub fn is_tab_modified(idx: usize) -> bool { unsafe { idx < MAX_TABS && TABS[idx].modified } }
