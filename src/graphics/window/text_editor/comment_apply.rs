...
pub(super) fn add_comment(start: usize, prefix: &[u8]) -> bool {
    ...
    if len + prefix.len() >= BUFFER_SIZE { return false; }
    unsafe {
        for i in (start..len).rev() { EDITOR_BUFFER[i + prefix.len()] = EDITOR_BUFFER[i]; }
        for (i, &ch) in prefix.iter().enumerate() { EDITOR_BUFFER[start + i] = ch; }
    }
...
pub(super) fn remove_comment(start: usize, prefix: &[u8]) -> bool {
    ...
    unsafe {
        for i in pos + prefix.len()..len { EDITOR_BUFFER[i - prefix.len()] = EDITOR_BUFFER[i]; }
        for i in len - prefix.len()..len { EDITOR_BUFFER[i] = 0; }
    }
    EDITOR_LEN.store(len - prefix.len(), Ordering::Relaxed);
    let cursor = EDITOR_CURSOR.load(Ordering::Relaxed);
    if cursor >= prefix.len() { EDITOR_CURSOR.store(cursor - prefix.len(), Ordering::Relaxed); }
    EDITOR_MODIFIED.store(true, Ordering::Relaxed);
    true
}
