...
pub(super) fn undo() -> bool {
    let entry = match pop_undo() { Some(e) => e, None => return false };
    match entry.op_type {
        UndoOpType::Insert => { if !undo_insert(&entry) { return false; } push_redo(entry); }
        UndoOpType::Delete => { if !undo_delete(&entry) { return false; } push_redo(entry); }
