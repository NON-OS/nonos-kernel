...
pub(super) fn ensure_visible(max_lines: usize) {
    ...
    unsafe { for i in 0..cursor.min(len) { if EDITOR_BUFFER[i] == b'\n' { cursor_line += 1; } } }
    if cursor_line < scroll_y { EDITOR_SCROLL_Y.store(cursor_line, Ordering::Relaxed); }
    else if cursor_line >= scroll_y + max_lines { EDITOR_SCROLL_Y.store(cursor_line - max_lines + 1, Ordering::Relaxed); }
}
