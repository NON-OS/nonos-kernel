...
pub(crate) fn handle_click(rx: u32, ry: u32) -> bool {
    if ry < 30 && rx < 70 { set_view(VIEW_LIST); return true; }
    if ry >= 350 && rx < 400 { set_input_focused(true); return true; }
    if ry >= 350 && rx >= 400 { send_message(); return true; }
    false
}
...
pub(crate) fn handle_key(ch: u8) {
    ...
    let len = input_len();
    if ch == 8 && len > 0 { set_input_len(len - 1); }
    else if ch == 13 { send_message(); }
