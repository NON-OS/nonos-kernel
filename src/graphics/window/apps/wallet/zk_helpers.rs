...
pub(super) fn generate_blinding_factor() -> [u8; 32] {
    ...
    let t = tick_count();
    let seed = [(t >> 56) as u8, (t >> 48) as u8, (t >> 40) as u8, (t >> 32) as u8, (t >> 24) as u8, (t >> 16) as u8, (t >> 8) as u8, t as u8,
        0xDE, 0xAD, 0xBE, 0xEF, 0xCA, 0xFE, 0xBA, 0xBE, 0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10];
    blake3_hash(&seed)
}
...
pub(super) fn bytes_to_field_input(b: &[u8]) -> [u8; 32] {
    let mut r = [0u8; 32]; let l = b.len().min(32); r[..l].copy_from_slice(&b[..l]); r
}
