...
use super::state::{STATUS_MSG, STATUS_LEN, STATUS_SUCCESS, CACHED_BLOCK};
use super::render::{COLOR_BORDER, COLOR_TEXT_DIM, COLOR_TEXT_WHITE, COLOR_GREEN, COLOR_RED};

pub(super) fn draw_status_bar(x: u32, y: u32, w: u32) {
    for gy in 0..32u32 { let s = 28 - (gy / 4) as u8; fill_rect(x, y + gy, w, 1, 0xFF000000 | ((s as u32) << 16) | ((s as u32) << 8) | (s as u32)); }
    ...
    if block_num > 0 {
        ...
        block_str[0] = b'B'; block_str[1] = b'l'; block_str[2] = b'o'; block_str[3] = b'c'; block_str[4] = b'k'; block_str[5] = b':'; block_str[6] = b' ';
...
fn format_u64(buf: &mut [u8], n: u64) -> usize {
    ...
    let mut val = n; let mut digits = [0u8; 20]; let mut dc = 0;
