...
fn collect_accounts() -> Vec<AccountDisplay> {
    ...
    state.accounts.iter().map(|a| {
        ...
        AccountDisplay {
            index: a.index, name: a.name, name_len: a.name_len,
            addr_short: truncate_address(&a.address_hex()),
            eth, eth_frac: wei / 1_000_000_000_000_000,
            nox, nox_frac: nox_w / 1_000_000_000_000_000,
...
fn draw_premium_account_card(x: u32, y: u32, w: u32, h: u32, acc: &AccountDisplay) {
    ...
    if acc.active { fill_rect(x, y + 16, 3, h - 32, COLOR_ACCENT); }
