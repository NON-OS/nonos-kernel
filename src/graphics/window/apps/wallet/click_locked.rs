...
use crate::crypto::{generate_secure_key, blake3_derive_key, blake3_hash};
use super::state::*;
...
pub(super) fn handle_locked_click(x: u32, y: u32, w: u32, h: u32) -> bool {
    let (cx, cy) = (w / 2, h / 2);
    if x >= cx.saturating_sub(130) && x <= cx + 130 && y >= cy && y <= cy + 32 { PASSWORD_FOCUSED.store(true, Ordering::SeqCst); return true; }
    if x >= cx.saturating_sub(130) && x <= cx.saturating_sub(5) && y >= cy + 55 && y <= cy + 99 { try_unlock(); return true; }
    if x >= cx + 5 && x <= cx + 130 && y >= cy + 55 && y <= cy + 99 { generate_new_wallet(); return true; }
...
fn generate_new_wallet() {
    ...
    if pwd_len > 0 { combined[32..32 + pwd_len.min(32)].copy_from_slice(&pwd[..pwd_len.min(32)]); }
    ...
    for b in combined.iter_mut() { unsafe { core::ptr::write_volatile(b, 0) }; }
    drop(pwd);
    match init_wallet(key) {
        Ok(()) => { clear_pwd(); set_status(if pwd_len > 0 { b"Wallet created with password" } else { b"New wallet created" }, true); }
...
pub(super) fn try_unlock() {
    ...
    if len == 0 { set_status(b"Enter a master key", false); return; }
    let key = blake3_hash(&pwd[..len]);
    drop(pwd);
    match init_wallet(key) { Ok(()) => { clear_pwd(); } Err(e) => { set_status(e.as_bytes(), false); } }
}
