...
use super::{state::WALLET_STATE, network::chain_id, rpc, transaction_parse::{derive_signing_key, build_erc20_transfer_data}, transaction_sign::build_and_sign_contract_tx};
...
pub fn send_nox_to(to: &[u8; 20], amount: u128) -> Result<[u8; 32], &'static [u8]> {
    if !rpc::is_rpc_available() { return Err(b"No network"); }
    let s = WALLET_STATE.lock();
    if !s.unlocked { return Err(b"Wallet locked"); }
    let mk = s.master_key.ok_or(b"No master key" as &[u8])?;
    let (from, idx) = s.get_active_account().map(|a| (a.address, a.index)).ok_or(b"No account" as &[u8])?;
    drop(s);
