...
pub(super) fn draw_stealth_view(x: u32, y: u32, w: u32, h: u32) {
    ...
    let stealth_active = { let state = WALLET_STATE.lock(); state.stealth_keypair.is_some() };
    if stealth_active { draw_string(x + 36, y + 110, b"[Active] Keys derived from master seed", COLOR_GREEN); }
    else { draw_string(x + 36, y + 110, b"Unlock wallet to enable stealth keys", COLOR_TEXT_DIM); }
    ...
    if zk_init { let mut s = *b"ZK Engine Ready (X/Y circuits)"; s[18] = b'0' + circuits_ready; s[20] = b'0' + total_circuits; draw_string(x + 36, y + 215, &s, COLOR_GREEN); }
    else { draw_string(x + 36, y + 215, b"ZK Engine: Not initialized", COLOR_YELLOW); }
    ...
    if stealth_active {
        ...
        let encoded = { let state = WALLET_STATE.lock(); state.stealth_keypair.as_ref().map(|s| s.meta_address().encode()) };
        if let Some(enc) = encoded { draw_string(x + 28, y + 378, &enc[..60.min(enc.len())], COLOR_TEXT_WHITE); if enc.len() > 60 { draw_string(x + 28, y + 395, &enc[60..], COLOR_TEXT_WHITE); }}
    }
    if h > 450 { fill_rect(x + 20, y + h - 85, w - 40, 70, COLOR_CARD); draw_string(x + 36, y + h - 70, b"Generate ZK Proof for Transaction", COLOR_TEXT_WHITE); draw_string(x + 36, y + h - 50, b"Prove balance sufficiency without revealing amount", COLOR_TEXT_DIM); fill_rect(x + w - 160, y + h - 65, 120, 30, COLOR_GREEN); draw_string(x + w - 145, y + h - 57, b"Create Proof", 0xFF0D1117); }
}
...
pub(super) fn draw_settings_view(x: u32, y: u32, w: u32, h: u32) {
    ...
    if show_key { let key_hex = { let state = WALLET_STATE.lock(); state.get_active_account().map(|a| a.private_key_hex()) }; if let Some(key) = key_hex { draw_string(x + 36, export_y + 30, key.as_bytes(), COLOR_TEXT_WHITE); } else { draw_string(x + 36, export_y + 30, b"No active account", COLOR_TEXT_DIM); } draw_string(x + 36, export_y + 50, b"[Click to hide]", COLOR_TEXT_DIM); }
    else { draw_string(x + 36, export_y + 30, b"Click to reveal (keep secure!)", COLOR_TEXT_DIM); }
