...
use super::render::{COLOR_ACCENT, COLOR_TEXT_WHITE, COLOR_BORDER, COLOR_BG, COLOR_GREEN};

pub(super) fn draw_locked_view(x: u32, y: u32, w: u32, h: u32) {
    let cx = x + w / 2; let cy = y + h / 2;
    for gy in 0..h { let s = ((gy as f32 / h as f32) * 30.0) as u8; fill_rect(x, y + gy, w, 1, 0xFF000000 | ((s as u32) << 16) | ((s as u32) << 8) | (s as u32)); }
    ...
    for i in 0..pwd_len.min(30) { fill_rect(cx - 118 + (i as u32 * 8), cy + 13, 6, 6, COLOR_TEXT_WHITE); }
    if focused { fill_rect(cx - 118 + (pwd_len.min(30) as u32 * 8), cy + 8, 2, 16, COLOR_ACCENT); }
...
pub(super) fn draw_rounded_card(x: u32, y: u32, w: u32, h: u32, color: u32) {
    let r = 16u32;
    for shadow in 0..8u32 { fill_rect(x + r + shadow, y + shadow + 4, w - 2 * r, h, ((40 - shadow * 4) << 24) | 0x000000); }
...
fn draw_password_field(x: u32, y: u32, w: u32, focused: bool) {
    let r = 10u32; let border = if focused { COLOR_ACCENT } else { COLOR_BORDER }; let bg = 0xFF1C1C1E;
    fill_rect(x + r, y - 1, w - 2 * r, 34, border); fill_rect(x - 1, y + r - 1, w + 2, 32 - 2 * r + 2, border);
    fill_rect(x + r, y, w - 2 * r, 32, bg); fill_rect(x, y + r, w, 32 - 2 * r, bg);
}
