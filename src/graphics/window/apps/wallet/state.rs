...
pub(crate) struct WalletState {
    ...
    pub active_account: usize, pub stealth_keypair: Option<StealthKeyPair>, pub pending_transactions: Vec<Transaction>,
}
impl WalletState {
    pub(crate) const fn new() -> Self { Self { unlocked: false, master_key: None, accounts: Vec::new(), active_account: 0, stealth_keypair: None, pending_transactions: Vec::new() } }
    pub(crate) fn lock(&mut self) {
        if let Some(ref mut k) = self.master_key { for b in k.iter_mut() { *b = 0; } } self.master_key = None;
        for a in self.accounts.iter_mut() { for b in a.secret_key.iter_mut() { *b = 0; } }
        self.accounts.clear(); self.active_account = 0; self.stealth_keypair = None; self.pending_transactions.clear(); self.unlocked = false;
    }
    pub(crate) fn get_active_account(&self) -> Option<&WalletAccount> { self.accounts.get(self.active_account) }
    pub(crate) fn set_active_account(&mut self, i: usize) { if i < self.accounts.len() { self.active_account = i; } }
    pub(crate) fn total_balance(&self) -> u128 { self.accounts.iter().map(|a| a.balance).sum() }
    pub(crate) fn total_nox_balance(&self) -> u128 { self.accounts.iter().map(|a| a.nox_balance).sum() }
}
...
pub(crate) fn set_view(v: WalletView) { CURRENT_VIEW.store(v as u8, Ordering::SeqCst); }
pub(crate) fn get_view() -> WalletView { WalletView::from(CURRENT_VIEW.load(Ordering::SeqCst)) }
pub(crate) fn set_status(msg: &[u8], ok: bool) { let mut s = STATUS_MSG.lock(); let l = msg.len().min(63); s[..l].copy_from_slice(&msg[..l]); STATUS_LEN.store(l, Ordering::SeqCst); STATUS_SUCCESS.store(ok, Ordering::SeqCst); }
