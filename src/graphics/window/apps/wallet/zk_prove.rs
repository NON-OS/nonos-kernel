...
pub(crate) fn prove_balance_ownership(balance: u128, secret_key: &[u8; 32], address: &[u8; ADDRESS_LEN]) -> Result<WalletZKProof, ZKError> {
    ...
    let witness = vec![FieldElement::from_bytes_array(&commitment), FieldElement::from_bytes_array(&bytes_to_field_input(address)), FieldElement::from_u128(balance), FieldElement::from_bytes_array(&blinding), FieldElement::from_bytes_array(secret_key)];
    let public_inputs = vec![FieldElement::from_bytes_array(&commitment), FieldElement::from_bytes_array(&bytes_to_field_input(address))];
