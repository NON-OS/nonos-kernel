...
pub(super) fn fetch_staking_state() -> Result<(), &'static str> {
    ...
    if let Ok(result) = rpc::eth_call(&ZSP_MAINNET, &nft_data) {
        if result.len() >= 32 { STAKING_STATE.lock().nft_count = parse_u256_to_u32(&result[0..32]).min(255) as u8; }
    }
    ...
    if let Ok(result) = rpc::eth_call(&STAKING_MAINNET, &call_data) {
        if result.len() >= 160 { let mut s = STAKING_STATE.lock(); s.staked_amount = parse_u256_to_u128(&result[0..32]); s.weighted_amount = parse_u256_to_u128(&result[32..64]); s.pending_rewards = parse_u256_to_u128(&result[96..128]); s.boost = parse_u256_to_u32(&result[128..160]); }
    }
    ...
    if let Ok(result) = rpc::eth_call(&STAKING_MAINNET, &pool_data) {
        if result.len() >= 128 { let mut s = STAKING_STATE.lock(); s.total_pool_staked = parse_u256_to_u128(&result[0..32]); s.total_weighted = parse_u256_to_u128(&result[32..64]); s.emission_rate = parse_u256_to_u128(&result[64..96]); s.current_apy = parse_u256_to_u32(&result[96..128]); s.genesis_started = true; }
    }
    let mut allow_data = [0u8; 68]; allow_data[0..4].copy_from_slice(&SIG_ALLOWANCE); allow_data[16..36].copy_from_slice(&addr); allow_data[48..68].copy_from_slice(&STAKING_MAINNET);
    if let Ok(result) = rpc::eth_call(&NOX_MAINNET, &allow_data) {
        if result.len() >= 32 { STAKING_STATE.lock().allowance = parse_u256_to_u128(&result[0..32]); }
    }
    Ok(())
}
...
pub(super) fn approve_nox() -> Result<[u8; 32], &'static str> {
    ...
    drop(s);
    let mut data = [0u8; 68]; data[0..4].copy_from_slice(&SIG_APPROVE); data[16..36].copy_from_slice(&STAKING_MAINNET);
    for i in 4..36 { data[32 + i] = 0xff; }
    ...
    let tx = build_and_sign_contract_tx(&NOX_MAINNET, 0, &data, nonce, gp, 60000, 1, &sk).map_err(|_| "Build tx failed")?;
