...
pub(crate) fn draw_calculator(x: u32, y: u32, w: u32, h: u32) {
    ...
    if scientific::is_scientific_mode() {
        super::render_scientific::draw_scientific_panel(x + PADDING, y + DISPLAY_H + PADDING + 8, w - PADDING * 2, 150);
        draw_buttons(x, y + 150, w, h - 150);
    } else { draw_buttons(x, y, w, h); }
    if history::is_visible() { super::render_history::draw_history_panel(x + w + 8, y, 200, h); }
}
...
fn draw_expression(x: u32, y: u32, w: u32) {
    ...
    let op_ch = match expr_op { 1 => b'+', 2 => b'-', 3 => 0xD7, 4 => 0xF7, _ => b' ' };
