...
use crate::graphics::design_system::colors::{CALC_DISPLAY_BG, TEXT_SECONDARY};
...
fn draw_expression(x: u32, y: u32, w: u32) {
    ...
    if !CALC_NEW_INPUT.load(Ordering::Relaxed) || CALC_OPERATOR.load(Ordering::Relaxed) == 0 {
        expr_x = draw_number_small(expr_x, expr_y, current, TEXT_SECONDARY);
        expr_x -= 16;
    }

    let op_ch = match expr_op { 1 => b'+', 2 => b'-', 3 => 0xD7, 4 => 0xF7, _ => b' ' };
    crate::graphics::font::draw_char(expr_x, expr_y, op_ch, TEXT_SECONDARY);
    draw_number_small(expr_x - 16, expr_y, expr_val, TEXT_SECONDARY);
}
