...
use super::super::state::{get_path, get_current_source, FILE_ENTRIES, FILE_ENTRY_COUNT, FM_SELECTED_ITEM};
...
pub fn rename_selected(new_name: &str) -> FmResult {
    if new_name.is_empty() { return FmResult::InvalidName; }
    let source = get_current_source();
    let max_len = match source { FileSource::Ramfs => 63, FileSource::Fat32(_) => 11 };
    if new_name.len() > max_len { return FmResult::InvalidName; }
...
fn rename_ramfs(old_name: &str, new_name: &str) -> FmResult {
    ...
    if !path.ends_with('/') { old_path.push('/'); }
    old_path.push_str(old_name);
    let mut new_path = String::new();
    new_path.push_str(path);
    if !path.ends_with('/') { new_path.push('/'); }
