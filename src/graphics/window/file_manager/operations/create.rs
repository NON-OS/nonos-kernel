...
use super::super::state::{get_path, get_current_source, FM_CREATING_FOLDER, FM_CREATING_FILE};
use super::super::listing::{refresh_listing, get_fat32_dir_cluster};
use super::super::block_io::{block_read, block_write};

pub fn create_folder(name: &str) -> FmResult {
    if name.is_empty() { return FmResult::InvalidName; }
    let source = get_current_source();
    let max_len = match source { FileSource::Ramfs => 63, FileSource::Fat32(_) => 11 };
    if name.len() > max_len { return FmResult::InvalidName; }
...
fn create_folder_fat32(fs_id: u8, name: &str) -> FmResult {
    ...
    let new_cluster = match fat32::allocate_cluster_chain(&fs, 1, block_read, block_write) {
        Ok(Some(c)) => c, Ok(None) => return FmResult::NoSpace, Err(_) => return FmResult::IoError,
    };
    if let Err(e) = init_fat32_dir(&fs, new_cluster, dir_cluster) { return e; }
...
fn init_fat32_dir(fs: &fat32::Fat32, new_cluster: u32, parent: u32) -> Result<(), FmResult> {
    ...
    if block_write(fs.device_id, sector as u64, &buf).is_err() { return Err(FmResult::IoError); }
    Ok(())
}
...
fn write_fat32_entry(fs: &fat32::Fat32, dir_cluster: u32, entry: &fat32::DirEntry) -> FmResult {
    match fat32::find_free_dir_slot(fs, dir_cluster, block_read) {
        Ok(Some((_, slot_sector, slot_offset))) => {
            let mut buf = [0u8; 512];
            if block_read(fs.device_id, slot_sector as u64, &mut buf).is_err() { return FmResult::IoError; }
            unsafe {
                let eb = core::slice::from_raw_parts(entry as *const _ as *const u8, 32);
                buf[slot_offset..slot_offset + 32].copy_from_slice(eb);
            }
            if block_write(fs.device_id, slot_sector as u64, &buf).is_err() { return FmResult::IoError; }
            refresh_listing();
            FmResult::Ok
        }
        _ => FmResult::NoSpace,
