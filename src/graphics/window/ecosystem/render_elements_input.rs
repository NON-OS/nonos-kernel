...
use super::render_helpers::{COLOR_ACCENT, COLOR_WARNING, COLOR_INPUT_BG, COLOR_INPUT_BORDER};
...
pub fn blit_image_data(x: u32, y: u32, data: &ImageData, max_width: u32, clip_bottom: u32) {
    ...
    for py in 0..data.height {
        let screen_y = y + py;
        if screen_y >= clip_bottom { break; }
        for px in 0..draw_w {
            let idx = (py * data.width + px) as usize;
            if idx < data.pixels.len() {
                let color = data.pixels[idx];
                if color & 0xFF000000 != 0 { put_pixel(x + px, screen_y, color); }
            }
        }
    }
}
