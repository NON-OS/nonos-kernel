...
pub fn draw_render_element(base_x: u32, line_y: u32, elem: &RenderElement, max_width: u32, clip_bottom: u32) {
    let ex = base_x + elem.x;
    if ex >= base_x + max_width { return; }
    match &elem.content {
...
fn draw_text(ex: u32, line_y: u32, text: &str, style: &crate::apps::ecosystem::browser::engine::TextStyle, max_width: u32, base_x: u32, clip_bottom: u32) {
    let fg = style.color.unwrap_or(if style.heading_level > 0 { COLOR_HEADING } else if style.bold { COLOR_TEXT_BRIGHT } else { COLOR_TEXT });
    ...
    for &ch in text.as_bytes() {
        if cx + 8 > base_x + max_width { break; }
        draw_char(cx + italic_offset, line_y, ch, fg);
        cx += 8;
    }
    if style.underline && cx > ex {
        let uy = line_y + 15;
        if uy < clip_bottom { fill_rect(ex, uy, cx - ex, 1, fg); }
    }
}
...
fn draw_link(ex: u32, line_y: u32, text: &str, max_width: u32, base_x: u32, clip_bottom: u32) {
    ...
    if cx > ex {
        let uy = line_y + 15;
        if uy < clip_bottom { fill_rect(ex, uy, cx - ex, 1, COLOR_LINK); }
    }
}
