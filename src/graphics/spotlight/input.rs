...
use super::state::{close, get_query, get_selected, is_open, pop_char, push_char, select_next, select_prev};
use crate::graphics::window;

pub fn handle_key(key: u8, ctrl: bool) -> bool {
    if !is_open() { return false; }
    match key {
        0x1B => { close(); return true; }
        0x0D => { execute_selected(); close(); return true; }
        0x08 | 0x7F => { pop_char(); return true; }
        0x26 => { select_prev(); return true; }
        0x28 => { select_next(result_count()); return true; }
        c if c >= 0x20 && c < 0x7F && !ctrl => { push_char(c); return true; }
        _ => {}
    }
    false
}
...
pub fn handle_click(mx: i32, my: i32, sw: u32) -> bool {
    ...
    let input_h = 56u32;
    let result_h = 44u32;
    if mx < x as i32 || mx >= (x + width) as i32 { close(); return true; }
    if my < y as i32 { close(); return true; }
...
fn execute_selected() {
    ...
    if let Some(act) = action {
        match act {
            ResultAction::OpenApp(wtype) => { window::open(wtype); }
