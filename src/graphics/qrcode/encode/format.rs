...
pub(super) fn place_format_info(qr: &mut QrCode) {
    ...
    for i in 0..6 { qr.modules[8][i] = (format_bits >> i) & 1 == 1; }
    qr.modules[8][7] = (format_bits >> 6) & 1 == 1;
    qr.modules[8][8] = (format_bits >> 7) & 1 == 1;
    qr.modules[7][8] = (format_bits >> 8) & 1 == 1;
    for i in 0..6 { qr.modules[5 - i][8] = (format_bits >> (9 + i)) & 1 == 1; }
    for i in 0..7 { qr.modules[QR_SIZE - 1 - i][8] = (format_bits >> i) & 1 == 1; }
    qr.modules[QR_SIZE - 8][8] = true;
    for i in 0..8 { qr.modules[8][QR_SIZE - 8 + i] = (format_bits >> (7 + i)) & 1 == 1; }
}
