...
fn gf_multiply(a: u8, b: u8) -> u8 {
    if a == 0 || b == 0 { return 0; }
    let log_a = GF_LOG[a as usize]; let log_b = GF_LOG[b as usize];
    let log_result = (log_a as u16 + log_b as u16) % 255;
    GF_EXP[log_result as usize]
}

pub(super) fn add_error_correction(data: &[u8]) -> Vec<u8> {
    let ec_codewords = 10;
    let generator: [u8; 11] = [1, 216, 194, 159, 111, 199, 94, 95, 113, 157, 193];
    let mut result = data.to_vec(); result.resize(data.len() + ec_codewords, 0);
    for i in 0..data.len() {
        let coef = result[i];
        if coef != 0 { for j in 0..generator.len() { result[i + j] ^= gf_multiply(generator[j], coef); } }
    }
