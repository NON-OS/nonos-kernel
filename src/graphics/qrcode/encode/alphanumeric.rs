...
fn alphanumeric_value(c: u8) -> Option<u8> { ALPHANUMERIC_CHARS.iter().position(|&x| x == c).map(|p| p as u8) }

pub(super) fn encode_alphanumeric(data: &[u8]) -> Option<Vec<u8>> {
    let mut bits = Vec::new();
    bits.extend_from_slice(&[false, false, true, false]);
    let len = data.len();
    for i in (0..9).rev() { bits.push((len >> i) & 1 == 1); }
    let mut i = 0;
    while i < data.len() {
        if i + 1 < data.len() {
            ...
            for j in (0..11).rev() { bits.push((combined >> j) & 1 == 1); }
            i += 2;
        } else {
            let v = alphanumeric_value(data[i].to_ascii_uppercase())?;
            for j in (0..6).rev() { bits.push((v >> j) & 1 == 1); }
            i += 1;
        }
    }
    bits.extend_from_slice(&[false, false, false, false]);
    while bits.len() % 8 != 0 { bits.push(false); }
    let data_codewords = 34;
    let pad_patterns = [0b11101100u8, 0b00010001u8];
    let mut pad_idx = 0;
    while bits.len() < data_codewords * 8 {
        for j in (0..8).rev() { bits.push((pad_patterns[pad_idx] >> j) & 1 == 1); }
        pad_idx = 1 - pad_idx;
    }
    let mut bytes = Vec::with_capacity(bits.len() / 8);
    for chunk in bits.chunks(8) {
        let mut byte = 0u8;
        for (i, &bit) in chunk.iter().enumerate() { if bit { byte |= 1 << (7 - i); } }
        bytes.push(byte);
    }
    Some(bytes)
}
