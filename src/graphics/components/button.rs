...
use crate::graphics::design_system::{colors, spacing, borders, shadows};
...
pub struct ButtonState { pub hovered: bool, pub pressed: bool, pub disabled: bool }
...
pub fn draw_button(x: u32, y: u32, label: &[u8], variant: ButtonVariant, size: ButtonSize, state: ButtonState) {
    ...
    let (bg, text_color) = match (variant, state.disabled, state.pressed, state.hovered) {
        ...
        (ButtonVariant::Primary, _, _, true) => (colors::BUTTON_PRIMARY_HOVER, colors::TEXT_INVERSE),
        (ButtonVariant::Primary, _, _, _) => (colors::BUTTON_PRIMARY_BG, colors::BUTTON_PRIMARY_TEXT),
        (ButtonVariant::Secondary, _, true, _) => (colors::BG_ACTIVE, colors::TEXT_PRIMARY),
        (ButtonVariant::Secondary, _, _, true) => (colors::BUTTON_SECONDARY_HOVER, colors::TEXT_PRIMARY),
        (ButtonVariant::Secondary, _, _, _) => (colors::BUTTON_SECONDARY_BG, colors::BUTTON_SECONDARY_TEXT),
    ...
    if bg != 0 { primitives::rounded_rect(x, y, w, h, radius, bg); }
    let text_y = y + (h - 16) / 2;
    text::draw_centered(x, text_y, w, label, text_color);
}

pub fn button_hit_test(x: u32, y: u32, label_len: usize, size: ButtonSize, click_x: i32, click_y: i32) -> bool {
