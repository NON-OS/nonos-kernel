...
pub enum TabStyle { Underline, Pill, Glass }
...
pub fn draw_tab_bar(x: u32, y: u32, w: u32, tabs: &[&[u8]], selected: usize, style: TabStyle) {
    ...
    if style == TabStyle::Underline { fill_rect(x, y + TAB_HEIGHT - 1, w, 1, colors::BORDER_DEFAULT); }
}
...
fn draw_tab_underline(x: u32, y: u32, w: u32, label: &[u8], selected: bool) {
    ...
    if selected { fill_rect(x, y + TAB_HEIGHT - 2, w, 2, colors::ACCENT); }
}

fn draw_tab_pill(x: u32, y: u32, w: u32, label: &[u8], selected: bool) {
    if selected { rounded_rect_blend(x, y + 4, w, TAB_HEIGHT - 8, borders::RADIUS_MD, colors::ACCENT); }
...
fn draw_tab_glass(x: u32, y: u32, w: u32, label: &[u8], selected: bool) {
    ...
    if selected { rounded_rect_blend(x, y + 4, w, TAB_HEIGHT - 8, borders::RADIUS_MD, bg); }
    let text_color = if selected { colors::ACCENT } else { colors::TEXT_SECONDARY };
    let text_x = x + (w - label.len() as u32 * 8) / 2;
    draw_text(text_x, y + 12, label, text_color);
}
...
fn calc_tab_widths(tabs: &[&[u8]], total_w: u32) -> [u32; 16] {
    ...
    for i in 0..count { widths[i] = per_tab; }
    widths
}

pub fn tab_hit_test(x: u32, tabs: &[&[u8]], total_w: u32, click_x: i32, click_y: i32, bar_y: u32) -> i32 {
    if click_y < bar_y as i32 || click_y >= (bar_y + TAB_HEIGHT) as i32 { return -1; }
    let widths = calc_tab_widths(tabs, total_w);
    let mut tx = x as i32;
    for i in 0..tabs.len().min(16) {
        if click_x >= tx && click_x < tx + widths[i] as i32 { return i as i32; }
