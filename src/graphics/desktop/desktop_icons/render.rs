...
use super::super::constants::{MENU_BAR_HEIGHT, DOCK_HEIGHT};
use super::state::{DesktopIcon, ICON_COUNT, ICONS, ICON_POSITIONS, DRAGGING_ICON, IS_DRAGGING, SELECTED_ICON};
...
pub(super) fn get_icon_position(i: usize, w: u32) -> (u32, u32) {
    unsafe {
        ...
        if px >= 0 && py >= 0 { return (px as u32, py as u32); }
    }
    let cols = ((w - ICON_START_X - 20) / ICON_SPACING).max(1) as usize;
    let col = i % cols;
    let row = i / cols;
    (ICON_START_X + (col as u32) * ICON_SPACING, MENU_BAR_HEIGHT + ICON_START_Y + (row as u32) * ICON_SPACING)
}
...
pub(crate) fn draw(w: u32, h: u32) {
    ...
    for i in 0..cnt {
        ...
        unsafe { draw_icon(x, y, &ICONS[i], i == sel); }
    }
...
fn draw_icon(x: u32, y: u32, icon: &DesktopIcon, selected: bool) {
    if selected { fill_rect(x, y - 4, ICON_SIZE, ICON_SIZE + 24, 0x403B82F6); }
