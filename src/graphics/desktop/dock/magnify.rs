...
use super::state::{
    get_hover_position, is_magnification_enabled, set_hover_index, set_hover_position,
};
...
pub(super) fn update_hover_position(mx: i32, my: i32, dock_x: u32, dock_y: u32, dock_h: u32) {
    if !is_magnification_enabled() {
        return;
    }
    let in_y = my >= dock_y as i32 && my < (dock_y + dock_h) as i32;
    let in_x =
        mx >= dock_x as i32 && mx < (dock_x + DOCK_ICON_COUNT as u32 * ICON_SPACING + 28) as i32;
    if !in_y || !in_x {
    ...
    if (idx as usize) < DOCK_ICON_COUNT {
        set_hover_index(Some(idx));
    } else {
        set_hover_index(None);
    }
}
...
pub(super) fn get_magnified_size(icon_index: u32, dock_x: u32) -> u32 {
    ...
    let center =
        dock_x as i32 + 14 + (icon_index as i32 * ICON_SPACING as i32) + (BASE_SIZE as i32 / 2);
