...
pub(crate) fn test_zk_identity_empty() -> TestResult {
    ...
    if identity.created_epoch != 0 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_zk_identity_generate_unique() -> TestResult {
    ...
    if id1.commitment == id2.commitment { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_zk_identity_short_id_hex() -> TestResult {
    ...
    identity.id[0] = 0xAB;
    let short = identity.short_id();
    if short[0] != b'a' { return TestResult::Fail; }
    if short[1] != b'b' { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_privacy_state_create_identity() -> TestResult {
    ...
    if state.identity_count != 1 { return TestResult::Fail; }
    if state.active_identity != 0 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_privacy_state_create_identity_multiple() -> TestResult {
    ...
    if state.create_identity(102) != Some(2) { return TestResult::Fail; }
    if state.identity_count != 3 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_privacy_state_create_identity_max() -> TestResult {
    ...
    if state.identity_count != MAX_IDENTITIES { return TestResult::Fail; }
    if state.create_identity(100).is_some() { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_privacy_state_switch_identity() -> TestResult {
    ...
    if !state.switch_identity(1) { return TestResult::Fail; }
    if state.active_identity != 1 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_privacy_state_deactivate_identity() -> TestResult {
    ...
    if state.identities[0].active { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_privacy_state_get_active_some() -> TestResult {
    ...
    if active.is_none() { return TestResult::Fail; }
    if !active.unwrap().active { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_privacy_state_active_count_all() -> TestResult {
    ...
    if state.active_count() != 3 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_privacy_state_active_count_partial() -> TestResult {
    ...
    state.create_identity(102);
    state.deactivate_identity(1);
    if state.active_count() != 2 { return TestResult::Fail; }
    TestResult::Pass
}
