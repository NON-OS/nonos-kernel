...
///
/// Implements the ChaCha20 stream cipher as a CSPRNG, providing
/// cryptographically secure random bytes with forward secrecy on reseed.
pub struct ChaChaRng {
    /// 256-bit key material.
    key: [u8; 32],
    /// ChaCha20 state (constants + key + counter + nonce).
    state: [u32; 16],
    /// Output buffer for generated bytes.
    output: [u8; 64],
...
/// Maximum blocks before automatic reseed is recommended.
pub const RESEED_INTERVAL: u64 = 1 << 20; // ~1 million blocks
...
impl ChaChaRng {
    ...
    ///
    /// This securely erases the old state before applying the new seed,
    ...
    /// Check if reseed is recommended based on blocks generated.
    pub fn needs_reseed(&self) -> bool {
        self.blocks_generated >= RESEED_INTERVAL
    }

    /// Get the number of blocks generated since last seed.
