...
pub fn get_entropy64_secure() -> Result<u64, EntropyError> {
    if virtio_rng::is_available() { let mut buf = [0u8; 8]; if virtio_rng::fill_random(&mut buf).is_ok() { return Ok(u64::from_le_bytes(buf)); } }
    if let Some(v) = try_rdseed64() { return Ok(v); }
    if let Some(v) = try_rdrand64() { return Ok(v); }
    if tpm::is_tpm_available() { if let Ok(bytes) = tpm::get_random_bytes(8) { if bytes.len() >= 8 { let mut buf = [0u8; 8]; buf.copy_from_slice(&bytes[..8]); return Ok(u64::from_le_bytes(buf)); } } }
    Err(EntropyError::HardwareFailure)
}

pub fn get_entropy64() -> u64 {
    if let Ok(v) = get_entropy64_secure() { return v; }
...
pub(super) fn emergency_entropy_mix() -> u64 {
    ...
    for _ in 0..counter.wrapping_rem(16).wrapping_add(1) { core::hint::spin_loop(); }
    let tsc2 = read_tsc();
