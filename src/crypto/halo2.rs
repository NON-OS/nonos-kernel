...
impl Halo2Circuit {
    ...
    pub fn assign_advice(&mut self, col: usize, row: usize, value: Fp) -> Result<(), &'static str> {
        if col >= self.advice_columns.len() || row >= self.num_rows {
            return Err("Invalid column or row");
        }
        self.advice_columns[col][row] = value;
        Ok(())
    }
    
    pub fn assign_fixed(&mut self, col: usize, row: usize, value: Fp) -> Result<(), &'static str> {
...
impl Halo2Prover {
    ...
    pub fn prove(&self, circuit: &Halo2Circuit) -> Result<Halo2Proof, &'static str> {
...
pub fn generate_halo2_proof(circuit_data: &[u8], witness: &[u8]) -> Result<(Vec<u8>, Vec<u8>), &'static str> {
    ...
    for (i, chunk) in witness.chunks(8).enumerate() {
        ...
        circuit.assign_advice(advice_col, i, value).ok();
        circuit.assign_fixed(fixed_col, i, Fp::one()).ok();
    }
...
pub fn verify_halo2_proof(statement: &[u8], proof: &[u8], vk: &[u8]) -> Result<bool, &'static str> {
