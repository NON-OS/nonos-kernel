...
impl ChaCha20State {
    ...
    fn encrypt_in_place(&mut self, data: &mut [u8]) {
        ...
        while offset < data.len() {
            let block = self.generate_block();
            let block_bytes =
                unsafe { core::slice::from_raw_parts(block.as_ptr() as *const u8, 64) };
...
impl KeyExchange {
    ...
    pub fn derive_aead_keys(shared_secret: &[u8], handshake_hash: &[u8]) -> ([u8; 32], [u8; 32]) {
        ...
        let key_material =
            Hkdf::derive(b"tor-aead-key-derivation", shared_secret, handshake_hash, 64);
