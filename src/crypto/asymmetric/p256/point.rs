...
use super::{FieldElement, Scalar, P256_GX, P256_GY, P256_A, P256_B};
...
impl AffinePoint {
    ...
    pub fn from_compressed(bytes: &[u8; 33]) -> Option<Self> {
        ...
        let ax = FieldElement(P256_A).mul(&x);
...
impl ProjectivePoint {
    ...
    pub fn add(&self, other: &Self) -> Self {
        ...
        // If h == 0 and s_diff == 0, return double
        result = Self::ct_select(h_is_zero & s_diff_is_zero, &double_result, &result);
        // If h == 0 and s_diff != 0, return identity
    ...
    // SECURITY: Montgomery ladder - constant-time scalar multiplication
    // Both branches compute the same operations regardless of bit value
    pub fn mul(&self, scalar: &Scalar) -> Self {
        ...
        for i in (0..256).rev() {
            ...
            // Always compute both sum and doubles
            let sum = r0.add(&r1);
            let r0_double = r0.double();
            let r1_double = r1.double();

            // Select based on bit value (constant-time)
