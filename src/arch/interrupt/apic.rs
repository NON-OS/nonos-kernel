...
fn cpuid(leaf: u32, sub: u32) -> (u32,u32,u32,u32) {
    ...
    unsafe { core::arch::asm!("push %rbx; cpuid; mov %ebx, %esi; pop %rbx", inlateout("eax")a, out("esi")b, inlateout("ecx")c, lateout("edx")d, options(nostack, preserves_flags, att_syntax)); }
    (a,b,c,d)
}
...
// Hook for precise calibration (HPET/PIT). to be filled later in arch/time/*.
#[allow(unused)]
...
// crude spin-wait in ~TSC cycles; we replace with arch::time once ready
