...
pub unsafe fn enable_aeoi() {
    ...
    // Instead, we only recommend AEOI if initialized with it; otherwise skip it in prod.
...
/// Spurious IRQ handlers (IRQ7, IRQ15). Call these in IDT stubs if temporarily run with PIC enabled. They read ISR to confirm spurious.
...
unsafe fn current_offsets() -> Option<(u8,u8)> {
    // Not reliably readable; return None unless we track it in a static.
    None
}
