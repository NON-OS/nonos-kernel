...
impl PciDevice {
    pub fn check_and_clear_errors(&self) -> Option<u16> {
        ...
        let error_bits = stat & (status::MASTER_DATA_PARITY_ERROR | status::SIGNALED_TARGET_ABORT
            | status::RECEIVED_TARGET_ABORT | status::RECEIVED_MASTER_ABORT
            | status::SIGNALED_SYSTEM_ERROR | status::DETECTED_PARITY_ERROR);
        if error_bits != 0 {
