...
static TRAP_COUNTS: [AtomicU64; 32] = [const { AtomicU64::new(0) }; 32];
...
lazy_static! {
    static ref IDT: InterruptDescriptorTable = {
        ...
        unsafe {
            idt.double_fault
                .set_handler_addr(VirtAddr::new(df_handler as *const () as u64))
        ...
        unsafe {
            idt.machine_check
                .set_handler_addr(VirtAddr::new(mc_handler as *const () as u64))
...
/// Load the IDT  for diagnostics and isolation.
pub fn init() {
    IDT.load();
...
extern "x86-interrupt" fn df_handler(stack: InterruptStackFrame, _code: u64) {
    df_handler_impl(stack, _code)
}

fn df_handler_impl(stack: InterruptStackFrame, _code: u64) -> ! {
    enter_panic_mode();
...
extern "x86-interrupt" fn mc_handler(stack: InterruptStackFrame) {
    mc_handler_impl(stack)
}

fn mc_handler_impl(stack: InterruptStackFrame) -> ! {
    enter_panic_mode();
...
/// Handler modification detection —  false for now.
