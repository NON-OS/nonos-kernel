...
/// Request C0 power state (full performance) - ADVANCED IMPLEMENTATION
pub fn request_power_state_c0() {
    unsafe {
        // Enable turbo boost and all CPU features
        ...
        // Set maximum performance P-state
        let msr_perf_ctl = 0x199;
        wrmsr(msr_perf_ctl, 0x0); // P0 state (maximum performance)
        
        crate::log_debug!("CPU power state: C0 (full performance with turbo boost)");
    }
}

/// Request C1 power state (halt with reduced power) - ADVANCED IMPLEMENTATION
pub fn request_power_state_c1() {
    unsafe {
        // Disable turbo boost for power savings
        let msr_misc_enable = 0x1A0;
        let current_value = rdmsr(msr_misc_enable);
        let new_value = current_value | (1 << 38); // Set turbo disable bit
        wrmsr(msr_misc_enable, new_value);
        
        // Set reduced performance P-state
        let msr_perf_ctl = 0x199;
        wrmsr(msr_perf_ctl, 0x800); // P8 state (reduced performance)
        
        crate::log_debug!("CPU power state: C1 (reduced power, turbo disabled)");
    }
}

/// Request C2 power state (stop clock) - ADVANCED IMPLEMENTATION  
pub fn request_power_state_c2() {
    unsafe {
        // Further reduce performance and disable features
        let msr_perf_ctl = 0x199;
        wrmsr(msr_perf_ctl, 0xF00); // P15 state (minimum performance)
