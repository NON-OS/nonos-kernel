...
/// Initialize CPU detection and characterization (runs only once).
...
/// Detect CPU count using CPUID and APIC IDs.
fn detect_cpu_count() {
    let mut cpu_count = 1;
...
/// Scan APIC IDs to count processors (uses CPUID leaf 1).
...
/// Detect CPU features using CPUID (leaves 1 and 7).
...
/// Detect CPU topology using CPUID (leaves 1, 4, and 0xB).
...
/// Detect features for a specific CPU (assumes uniform features).
fn detect_features_for_cpu(_cpu_id: usize) -> CpuFeatures {
    get_cpu_features().unwrap_or(CpuFeatures {
        has_avx: false, has_avx2: false, has_sse41: false, has_sse42: false,
        has_aes_ni: false, has_rdrand: false, has_rdseed: false, has_fsgsbase: false,
        has_smep: false, has_smap: false, has_cet: false, has_pku: false,
        has_la57: false, has_tsx: false,
    })
}

/// Detect CPU caches using CPUID leaf 4.
fn detect_cpu_caches() {
    // No global state needed; per-CPU cache info is queried at init.
}

/// Detect cache info for specific CPU (uses CPUID leaf 4).
fn detect_cache_for_cpu(_cpu_id: usize) -> CacheInfo {
    unsafe {
...
/// Measure TSC frequency (PIT calibration).
fn measure_tsc_frequency() {
    // Real calibration is performed in get_cpu_frequency().
}

/// Measure TSC frequency for a specific CPU.
fn measure_tsc_for_cpu(_cpu_id: usize) -> u64 {
    get_cpu_frequency()
}

/// Estimate CPU frequency in MHz.
fn estimate_cpu_frequency_mhz() -> u32 {
    (get_cpu_frequency() / 1_000_000) as u32
}

/// Get current CPU ID (for SMP, use APIC ID).
pub fn current_cpu_id() -> usize {
    // APIC ID logic for SMP can be added here.
    0
}
...
/// Check if a CPU feature is supported (by name).
...
/// Initialize CPU-specific optimizations (enable HW features).
...
/// Get CPU cache statistics for CPU 0.
...
/// Real CPU frequency detection using CPUID and PIT calibration.
...
/// Request C0 power state (full performance, turbo boost).
pub fn request_power_state_c0() {
    unsafe {
        let msr_misc_enable = 0x1A0;
        let current_value = rdmsr(msr_misc_enable);
        let new_value = current_value & !(1 << 38); // Clear turbo disable bit
        wrmsr(msr_misc_enable, new_value);
        let msr_perf_ctl = 0x199;
        wrmsr(msr_perf_ctl, 0x0); // P0 state
        crate::log_debug!("CPU power state: C0 (full performance with turbo boost)");
    }
}

/// Request C1 power state (reduced power, turbo disabled).
pub fn request_power_state_c1() {
    unsafe {
        let msr_misc_enable = 0x1A0;
        let current_value = rdmsr(msr_misc_enable);
        let new_value = current_value | (1 << 38); // Set turbo disable bit
        wrmsr(msr_misc_enable, new_value);
        let msr_perf_ctl = 0x199;
        wrmsr(msr_perf_ctl, 0x800); // P8 state
        crate::log_debug!("CPU power state: C1 (reduced power, turbo disabled)");
    }
}

/// Request C2 power state (minimal performance).
pub fn request_power_state_c2() {
    unsafe {
        let msr_perf_ctl = 0x199;
        wrmsr(msr_perf_ctl, 0xF00); // P15 state
        crate::log_debug!("CPU power state: C2 (minimal performance)");
    }
}
