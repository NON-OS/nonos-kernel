...
pub fn init() {
    // Early GDT initialization would be done by bootloader or early boot
    // This is a placeholder for library compatibility
}
...
fn harden_crs() {
    unsafe {
        ...
        // CR4: UMIP/SMEP/SMAP if present (commented out - flags not available in this x86_64 crate version)
...
pub unsafe fn cet_enable_stub() {
    ...
    // We keep disabled until we have per-thread alloc + context switch integration.
}
...
pub unsafe fn entry_swapgs_if_needed(rflags: RFlags) {
    ...
    // Here we assume caller checked CPL; and keep utility for future.
