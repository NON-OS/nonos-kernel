...
impl PortManager {
    pub const fn new() -> Self {
        ...
        Self { initialized: AtomicBool::new(false), reserved_ranges: RwLock::new([NONE; MAX_RESERVED_RANGES]) }
    }
    ...
    pub fn reserve_range(&self, range: PortRange) -> Result<(), PortError> {
        ...
        for existing in ranges.iter().flatten() {
            if existing.overlaps(&range) { return Err(PortError::PortReserved { port: range.start() }); }
        }
        for slot in ranges.iter_mut() {
            if slot.is_none() { *slot = Some(range); return Ok(()); }
        }
        Err(PortError::InvalidRange { start: range.start(), end: range.end() })
    }
    ...
    pub fn is_reserved(&self, port: u16) -> bool {
        ...
        for range in ranges.iter().flatten() { if range.contains(port) { return true; } }
        false
    }

    pub fn stats(&self) -> &'static PortStats { &PORT_STATS }
    pub fn total_ops(&self) -> u64 { PORT_STATS.total_ops() }
}

impl Default for PortManager { fn default() -> Self { Self::new() } }
