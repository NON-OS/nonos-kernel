...
use crate::arch::x86_64::acpi::tables::{SdtHeader, SIG_XSDT};
...
pub fn parse_xsdt(registry: &mut TableRegistry, addr: u64) -> AcpiResult<()> {
    unsafe {
        ...
        if header.signature != SIG_XSDT { return Err(AcpiError::InvalidXsdtSignature); }
        if !header.validate_checksum(addr as *const u8) { return Err(AcpiError::XsdtChecksumFailed); }
