...
unsafe fn init_interrupts_early() -> bool {
    ...
    // extern "x86-interrupt" fn double_fault_wrapper(stack_frame: InterruptStackFrame, error_code: u64) {
    //     crate::interrupts::handlers::double_fault_handler(stack_frame, error_code);
    ...
    // SIMPLE_IDT.double_fault.set_handler_fn(double_fault_wrapper).set_stack_index(0);
...
unsafe fn init_memory() {
    ...
    // Step 2: Initialize physical memory allocator (now safe to access higher-half statics)
...
unsafe fn init_kernel_address_space() {
    ...
    let level4_table = (level4_table_frame.start_address().as_u64() + phys_offset) as *mut PageTable;
