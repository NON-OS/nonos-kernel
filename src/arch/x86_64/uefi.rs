...
impl UefiManager {
    ...
    pub fn set_variable(
        ...
        if let Some(rt_services) = self.runtime_services {
            unsafe {
                ...
                crate::log::logger::log_debug!(
                    "UEFI variable data prepared: {} bytes",
                    var_data.len()
                );
            }
        }
        ...
        let var =
            UefiVariable { name: String::from(name), guid: *guid, attributes, data: data.to_vec() };
    ...
    pub fn reset_system(&self, reset_type: ResetType) -> Result<(), &'static str> {
        if let Some(rt) = self.runtime_services {
            ...
            crate::log::logger::log_info!(
                "{}",
                &format!("UEFI system reset requested: {:?}", reset_type)
            );
            Ok(())
        } else {
