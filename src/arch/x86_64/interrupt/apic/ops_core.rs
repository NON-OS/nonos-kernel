...
pub fn set_tpr(priority: u8) {
    ...
    } else { mmio_w32(LAPIC_TPR, priority as u32); }
}

pub fn get_tpr() -> u8 { CURRENT_TPR.load(Ordering::Acquire) }
...
pub fn version() -> u32 {
    ...
    } else { mmio_r32(LAPIC_VER) & 0xFF }
}

pub fn max_lvt() -> u8 {
    if X2APIC_MODE.load(Ordering::Acquire) {
        ((rdmsr(0x803) >> 16) & 0xFF) as u8
    } else { ((mmio_r32(LAPIC_VER) >> 16) & 0xFF) as u8 }
}
