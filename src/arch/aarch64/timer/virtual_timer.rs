...
impl VirtualTimer {
    ...
    pub fn set_relative(ticks: u64) {
        let current = super::generic::virtual_count();
        Self::set_compare_value(current + ticks);
        Self::enable();
    }
}

pub fn set_virtual_timer(ticks: u64) {
    VirtualTimer::set_relative(ticks);
}

pub fn clear_virtual_timer() {
    VirtualTimer::disable();
}

pub fn virtual_timer_pending() -> bool {
    VirtualTimer::is_pending()
}
