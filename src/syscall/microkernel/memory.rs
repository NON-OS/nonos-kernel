...
use crate::memory::paging::{PagePermissions, map_page, unmap_page};
...
pub fn sys_mmap(addr: u64, length: usize, prot: u32, _flags: u32) -> i64 {
    ...
    if prot & PROT_EXEC != 0 { perms = perms | PagePermissions::EXECUTE; }
    let base = if addr != 0 {
        VirtAddr::new(addr)
    } else {
    ...
    for i in 0..pages {
        ...
        if map_page(va, frame, perms).is_err() { return E_NOMEM; }
    }
    base.as_u64() as i64
}

pub fn sys_munmap(addr: u64, length: usize) -> i64 {
    if addr == 0 || length == 0 { return E_INVAL; }
    if addr % PAGE_SIZE as u64 != 0 { return E_INVAL; }
    let pages = (length + PAGE_SIZE - 1) / PAGE_SIZE;
    for i in 0..pages {
