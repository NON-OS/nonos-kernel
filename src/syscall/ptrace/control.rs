...
use super::state::{get_tracer, set_options, set_singlestep, set_syscall_entry, clear_traced};
...
pub fn do_cont(pid: u32, signal: u32) -> Result<(), i32> {
    ...
    if signal != 0 {
        let _ = crate::syscall::signals::send_signal_to_process(pid, signal as i32);
    }
    Ok(())
}
