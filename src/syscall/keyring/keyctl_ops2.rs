...
pub fn keyctl_describe(key_id: KeySerial, buf: u64, buflen: usize) -> SyscallResult {
    ...
    if buf != 0 && buflen > 0 {
        ...
        if write_user_bytes(buf, &bytes[..to_copy]).is_err() { return errno(14); }
    }
    SyscallResult { value: bytes.len() as i64 + 1, capability_consumed: false, audit_required: false }
}
...
pub fn keyctl_link(key_id: KeySerial, keyring_id: KeySerial) -> SyscallResult {
    if get_key(key_id).is_none() { return errno(2); }
...
pub fn keyctl_search(keyring_id: KeySerial, type_ptr: u64, desc_ptr: u64, dest: KeySerial) -> SyscallResult {
    let type_str = match read_user_string(type_ptr, 32) { Ok(s) => s, Err(_) => return errno(14) };
    let description = match read_user_string(desc_ptr, 256) { Ok(s) => s, Err(_) => return errno(14) };
    match search_keyring(keyring_id, &type_str, &description) {
        Some(serial) => {
            if dest != 0 { get_key_mut(dest, |kr| link_key(kr, serial)); }
            SyscallResult { value: serial as i64, capability_consumed: false, audit_required: false }
        }
        None => errno(126),
    }
}
...
pub fn keyctl_read(key_id: KeySerial, buf: u64, buflen: usize) -> SyscallResult {
    ...
    if buf != 0 && buflen > 0 {
        ...
        if write_user_bytes(buf, &key.payload[..to_copy]).is_err() { return errno(14); }
    }
    SyscallResult { value: key.payload.len() as i64, capability_consumed: false, audit_required: false }
}
