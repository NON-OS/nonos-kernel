...
pub unsafe fn user_slice_mut<'a>(addr: u64, len: usize) -> Result<&'a mut [u8], &'static str> { unsafe {
    validate_user_region(addr, len, Access::Write)?;
    Ok(core::slice::from_raw_parts_mut(addr as *mut u8, len))
}}

pub unsafe fn user_slice<'a>(addr: u64, len: usize) -> Result<&'a [u8], &'static str> { unsafe {
