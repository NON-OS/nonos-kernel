...
pub fn sys_process_vm_readv(
    ...
    if validate_iovec_access(&local, true).is_err() { return -14; }
...
fn readv_same_space(local: &[IoVec], remote: &[IoVec]) -> i64 {
    ...
    for riov in remote {
        ...
        while remaining > 0 && local_idx < local.len() {
            ...
            if crate::usercopy::copy_from_user(remote_addr as u64, &mut buf).is_err() { return total_read as i64; }
            if crate::usercopy::copy_to_user(local_addr, &buf).is_err() { return total_read as i64; }
            total_read += to_copy;
            local_off += to_copy;
            remote_addr += to_copy;
            remaining -= to_copy;
    ...
    for riov in remote {
        ...
        while remaining > 0 && local_idx < local.len() {
            ...
            match copy_from_remote(cr3, remote_addr, &mut buf) {
                Ok(n) => {
                    if crate::usercopy::copy_to_user(local_addr, &buf[..n]).is_err() { return total_read as i64; }
                    total_read += n; local_off += n; remote_addr += n; remaining -= n;
                }
