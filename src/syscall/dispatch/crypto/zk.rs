...
pub fn handle_crypto_zk_prove(
    proof_type: u64, secret_ptr: u64, public_ptr: u64, proof_out_ptr: u64,
) -> SyscallResult {
    if let Err(e) = require_capability(Capability::Crypto) { return e; }
    if secret_ptr == 0 || public_ptr == 0 || proof_out_ptr == 0 { return errno(22); }
    match proof_type {
        1 => {
            let mut secret_arr = [0u8; 32];
            let mut public_arr = [0u8; 32];
            if copy_from_user(secret_ptr, &mut secret_arr).is_err() { return errno(14); }
            if copy_from_user(public_ptr, &mut public_arr).is_err() { return errno(14); }
            match crate::crypto::zk_kernel::syscall_zk_prove_schnorr(&secret_arr, &public_arr) {
                Ok(proof) => {
                    if copy_to_user(proof_out_ptr, &proof).is_err() { return errno(14); }
        ...
        6 => {
            ...
            if copy_from_user(secret_ptr, &mut value_arr).is_err() { return errno(14); }
            match crate::crypto::zk_kernel::syscall_zk_commit(&value_arr) {
                Ok((commitment, blinding)) => {
                    ...
                    if copy_to_user(proof_out_ptr, &proof_out).is_err() { return errno(14); }
                    SyscallResult { value: 64, capability_consumed: false, audit_required: true }
                }
                Err(_) => errno(5),
            }
...
pub fn handle_crypto_zk_verify(
    proof_type: u64, proof_ptr: u64, proof_len: u64, public_input_ptr: u64,
) -> SyscallResult {
    if let Err(e) = require_capability(Capability::Crypto) { return e; }
    if proof_ptr == 0 || public_input_ptr == 0 || proof_len == 0 { return errno(22); }
    let public_input_len = match proof_type { 1 => 32, 2 => 32, 3 => 0, 6 => 64, _ => return errno(22) };
    let mut proof_data = alloc::vec![0u8; proof_len as usize];
    if copy_from_user(proof_ptr, &mut proof_data).is_err() { return errno(14); }
    let public_input = if public_input_len > 0 {
        let mut buf = alloc::vec![0u8; public_input_len];
        if copy_from_user(public_input_ptr, &mut buf).is_err() { return errno(14); }
        buf
    } else {
        alloc::vec![]
    };
