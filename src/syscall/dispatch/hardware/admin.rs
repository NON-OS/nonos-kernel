...
pub fn handle_admin_reboot() -> SyscallResult {
    ...
    if !proc.capability_token().grants(crate::capabilities::Capability::Admin) { return errno(1); }
    crate::log::info!("ADMIN: System reboot requested by pid {}", proc.pid);
    crate::security::secure_wipe_all_memory();
    unsafe {
        core::arch::asm!("cli", "out 0x64, al", in("al") 0xFEu8, options(nostack, nomem));
    }
...
pub fn handle_admin_shutdown() -> SyscallResult {
    ...
    crate::security::secure_wipe_all_memory();
    unsafe {
        core::arch::asm!("cli", "out dx, ax", in("dx") 0x604u16, in("ax") 0x2000u16, options(nostack, nomem));
        core::arch::asm!("hlt");
    }
    loop { unsafe { core::arch::asm!("hlt"); } }
}
...
pub fn handle_admin_mod_load(name_ptr: u64, name_len: u64, code_ptr: u64, code_len: u64, sig_ptr: u64) -> SyscallResult {
    let Some(proc) = crate::process::current_process() else { return errno(1); };
    if !proc.capability_token().grants(crate::capabilities::Capability::Admin) { return errno(1); }
    if name_ptr == 0 || code_ptr == 0 || sig_ptr == 0 { return errno(22); }
    if name_len == 0 || name_len > 256 || code_len == 0 || code_len > 16 * 1024 * 1024 { return errno(22); }
    let mut name_buf = alloc::vec![0u8; name_len as usize];
    if copy_from_user(name_ptr, &mut name_buf).is_err() { return errno(14); }
    let name = match core::str::from_utf8(&name_buf) { Ok(s) => s, Err(_) => return errno(22) };
    let mut code = alloc::vec![0u8; code_len as usize];
    if copy_from_user(code_ptr, &mut code).is_err() { return errno(14); }
    let mut sig = [0u8; 64];
    if copy_from_user(sig_ptr, &mut sig).is_err() { return errno(14); }
    use crate::modules::nonos_module_loader::{NONOS_MODULE_LOADER, NonosModuleType};
    match NONOS_MODULE_LOADER.load_module(name, NonosModuleType::Application, code, &sig) {
        Ok(module_id) => {
        ...
        Err(e) => { crate::log_warn!("ADMIN: Module load failed: {}", e); errno(22) }
    }
}
...
pub fn handle_admin_cap_grant(target_pid: u32, caps_bits: u64, _ttl_ms: u64) -> SyscallResult {
    ...
    let Some(pcb) = PROCESS_TABLE.find_by_pid(target_pid) else { return errno(3); };
    if caps_bits == 0 { return errno(22); }
