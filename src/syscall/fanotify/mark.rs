...
impl FanotifyMark {
    ...
    pub fn matches(&self, path: &str, event_mask: u64) -> bool {
        if (self.mask & event_mask) == 0 { return false; }
        match &self.path {
...
pub fn sys_fanotify_mark(fanotify_fd: i32, flags: u32, mask: u64, dirfd: i32, pathname: usize) -> i64 {
    ...
    if (flags & super::FAN_MARK_ADD) != 0 {
        ...
        marks.retain(|m| {
            if let (Some(p1), Some(p2)) = (&m.path, &path) { p1 != p2 } else { true }
        });
    }
    0
}
