...
#[test]
fn test_in_all_events_combined() {
...
#[test]
fn test_in_onlydir_constant() {
    assert_eq!(IN_ONLYDIR, 0x01000000);
}

#[test]
fn test_in_dont_follow_constant() {
    assert_eq!(IN_DONT_FOLLOW, 0x02000000);
}

#[test]
fn test_in_excl_unlink_constant() {
    assert_eq!(IN_EXCL_UNLINK, 0x04000000);
}

#[test]
fn test_in_mask_create_constant() {
    assert_eq!(IN_MASK_CREATE, 0x10000000);
}

#[test]
fn test_in_mask_add_constant() {
    assert_eq!(IN_MASK_ADD, 0x20000000);
}

#[test]
fn test_in_isdir_constant() {
    assert_eq!(IN_ISDIR, 0x40000000);
}

#[test]
fn test_in_oneshot_constant() {
    assert_eq!(IN_ONESHOT, 0x80000000);
}

#[test]
fn test_in_unmount_constant() {
    assert_eq!(IN_UNMOUNT, 0x00002000);
}

#[test]
fn test_in_q_overflow_constant() {
    assert_eq!(IN_Q_OVERFLOW, 0x00004000);
}

#[test]
fn test_in_ignored_constant() {
    assert_eq!(IN_IGNORED, 0x00008000);
}

#[test]
fn test_inotify_einval_constant() {
    assert_eq!(EINVAL, 22);
}

#[test]
fn test_inotify_enomem_constant() {
    assert_eq!(ENOMEM, 12);
}

#[test]
fn test_inotify_ebadf_constant() {
    assert_eq!(EBADF, 9);
}

#[test]
fn test_inotify_enoent_constant() {
    assert_eq!(ENOENT, 2);
}

#[test]
fn test_inotify_eagain_constant() {
    assert_eq!(EAGAIN, 11);
}

#[test]
fn test_inotify_enotdir_constant() {
    assert_eq!(ENOTDIR, 20);
}

#[test]
fn test_inotify_eexist_constant() {
    assert_eq!(EEXIST, 17);
}

#[test]
fn test_max_inotify_instances() {
    assert_eq!(MAX_INOTIFY_INSTANCES, 128);
}

#[test]
fn test_max_watches_per_instance() {
    assert_eq!(MAX_WATCHES_PER_INSTANCE, 8192);
}

#[test]
fn test_max_queued_events() {
    assert_eq!(MAX_QUEUED_EVENTS, 16384);
}

#[test]
fn test_inotify_event_new_no_name() {
...
#[test]
fn test_inotify_event_new_with_name() {
...
#[test]
fn test_inotify_event_name_len_alignment() {
    let ev = InotifyEvent::new(1, IN_CREATE, 0, Some("a"));
    assert_eq!(ev.len % 4, 0);
}

#[test]
fn test_inotify_event_name_len_alignment_longer() {
    let ev = InotifyEvent::new(1, IN_CREATE, 0, Some("test"));
    assert_eq!(ev.len % 4, 0);
}

#[test]
fn test_inotify_event_total_size_no_name() {
    let ev = InotifyEvent::new(1, IN_ACCESS, 0, None);
...
#[test]
fn test_inotify_event_total_size_with_name() {
...
#[test]
fn test_inotify_event_cookie_for_move() {
...
#[test]
fn test_inotify_init_success_returns_fd() {
    let fd = 5i64;
...
#[test]
fn test_inotify_init1_invalid_flags_returns_einval() {
    let result = SyscallResult::error(22);
    assert_eq!(result.errno(), Some(22));
}

#[test]
fn test_inotify_init1_success_returns_fd() {
    let fd = 6i64;
    let result = SyscallResult::success(fd);
...
#[test]
fn test_inotify_init_max_instances_returns_enomem() {
    let result = SyscallResult::error(12);
    assert_eq!(result.errno(), Some(12));
}

#[test]
fn test_inotify_add_watch_null_path_returns_einval() {
    let result = SyscallResult::error(22);
    assert_eq!(result.errno(), Some(22));
}

#[test]
fn test_inotify_add_watch_invalid_fd_returns_ebadf() {
    let result = SyscallResult::error(9);
    assert_eq!(result.errno(), Some(9));
}

#[test]
fn test_inotify_add_watch_success_returns_wd() {
    let wd = 1i64;
...
#[test]
fn test_inotify_add_watch_path_not_found_returns_enoent() {
    let result = SyscallResult::error(2);
    assert_eq!(result.errno(), Some(2));
}

#[test]
fn test_inotify_add_watch_not_directory_returns_enotdir() {
    let result = SyscallResult::error(20);
    assert_eq!(result.errno(), Some(20));
}

#[test]
fn test_inotify_add_watch_max_watches_returns_enomem() {
    let result = SyscallResult::error(12);
    assert_eq!(result.errno(), Some(12));
}

#[test]
fn test_inotify_rm_watch_invalid_fd_returns_ebadf() {
    let result = SyscallResult::error(9);
    assert_eq!(result.errno(), Some(9));
}

#[test]
fn test_inotify_rm_watch_invalid_wd_returns_einval() {
    let result = SyscallResult::error(22);
    assert_eq!(result.errno(), Some(22));
}

#[test]
fn test_inotify_rm_watch_success_returns_zero() {
    let result = SyscallResult::success(0);
    assert_eq!(result.value, 0);
}

#[test]
fn test_inotify_read_would_block_returns_eagain() {
    let result = SyscallResult::error(11);
    assert_eq!(result.errno(), Some(11));
}

#[test]
fn test_inotify_read_success_returns_bytes() {
    let bytes = 32i64;
