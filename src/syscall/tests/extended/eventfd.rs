...
#[test]
fn test_efd_cloexec_constant() {
    assert_eq!(EFD_CLOEXEC, 0x80000);
}

#[test]
fn test_efd_nonblock_constant() {
    assert_eq!(EFD_NONBLOCK, 0x800);
}

#[test]
fn test_efd_semaphore_constant() {
    assert_eq!(EFD_SEMAPHORE, 0x1);
}

#[test]
fn test_eventfd_einval_constant() {
    assert_eq!(EINVAL, 22);
}

#[test]
fn test_eventfd_eagain_constant() {
    assert_eq!(EAGAIN, 11);
}

#[test]
fn test_eventfd_enomem_constant() {
    assert_eq!(ENOMEM, 12);
}

#[test]
fn test_eventfd_ebadf_constant() {
    assert_eq!(EBADF, 9);
}

#[test]
fn test_eventfd_max_constant() {
    assert_eq!(EVENTFD_MAX, u64::MAX - 1);
}

#[test]
fn test_max_eventfd_instances_constant() {
    assert_eq!(MAX_EVENTFD_INSTANCES, 1024);
}

#[test]
fn test_eventfd_instance_new() {
...
#[test]
fn test_eventfd_instance_with_initval() {
    let inst = EventFdInstance::new(1, 100, 0);
    assert_eq!(inst.get_counter(), 100);
}

#[test]
fn test_eventfd_instance_is_semaphore() {
    let inst = EventFdInstance::new(1, 0, EFD_SEMAPHORE);
    assert!(inst.is_semaphore());
}

#[test]
fn test_eventfd_instance_not_semaphore() {
    let inst = EventFdInstance::new(1, 0, 0);
    assert!(!inst.is_semaphore());
}

#[test]
fn test_eventfd_instance_is_nonblock() {
    let inst = EventFdInstance::new(1, 0, EFD_NONBLOCK);
    assert!(inst.is_nonblock());
}

#[test]
fn test_eventfd_instance_not_nonblock() {
    let inst = EventFdInstance::new(1, 0, 0);
    assert!(!inst.is_nonblock());
}

#[test]
fn test_eventfd_instance_with_both_flags() {
...
#[test]
fn test_eventfd_instance_with_all_flags() {
...
#[test]
fn test_eventfd_invalid_flags_returns_einval() {
    let result = SyscallResult::error(22);
    assert_eq!(result.errno(), Some(22));
}

#[test]
fn test_eventfd_success_returns_fd() {
    let fd = 5000i64;
...
#[test]
fn test_eventfd2_success_returns_fd() {
    let fd = 5001i64;
    let result = SyscallResult::success(fd);
...
#[test]
fn test_eventfd_write_adds_to_counter() {
...
#[test]
fn test_eventfd_counter_max_boundary() {
    let max = EVENTFD_MAX;
    let at_max = max;
    let _ = at_max;
...
#[test]
fn test_eventfd_counter_overflow_check() {
...
#[test]
fn test_eventfd_counter_no_overflow() {
    let current: u64 = 100;
    let value: u64 = 50;
    let would_overflow = current > EVENTFD_MAX - value;
    assert!(!would_overflow);
...
#[test]
fn test_eventfd_initval_max() {
    let initval: u32 = u32::MAX;
    assert_eq!(initval, u32::MAX);
}

#[test]
fn test_eventfd_initval_zero() {
    let initval: u32 = 0;
    assert_eq!(initval, 0);
}

#[test]
fn test_eventfd_close_success() {
    let result = Ok::<(), i32>(());
    assert!(result.is_ok());
}

#[test]
fn test_eventfd_close_invalid_fd_returns_ebadf() {
    let result = Err::<(), i32>(EBADF);
    assert_eq!(result.err(), Some(9));
}

#[test]
fn test_eventfd_ops_read_buffer_too_small() {
    let count: usize = 4;
    assert!(count < 8);
    let result = SyscallResult::error(22);
    assert_eq!(result.errno(), Some(22));
...
#[test]
fn test_eventfd_ops_write_buffer_too_small() {
    let count: usize = 4;
    assert!(count < 8);
    let result = SyscallResult::error(22);
    assert_eq!(result.errno(), Some(22));
...
#[test]
fn test_eventfd_ops_read_invalid_fd() {
    let result = SyscallResult::error(9);
    assert_eq!(result.errno(), Some(9));
}

#[test]
fn test_eventfd_ops_write_invalid_fd() {
    let result = SyscallResult::error(9);
    assert_eq!(result.errno(), Some(9));
}

#[test]
fn test_eventfd_ops_read_success() {
    let bytes = 8usize;
...
#[test]
fn test_eventfd_ops_write_success() {
    let bytes = 8usize;
    let result = SyscallResult::success(bytes as i64);
    assert_eq!(result.value, 8);
}
...
#[test]
fn test_eventfd_is_readable_nonzero_counter() {
    let counter: u64 = 1;
...
#[test]
fn test_eventfd_is_readable_zero_counter() {
    let counter: u64 = 0;
    let readable = counter > 0;
    assert!(!readable);
}
...
#[test]
fn test_eventfd_is_writable_below_max() {
    let counter: u64 = 100;
...
#[test]
fn test_eventfd_is_writable_at_max() {
...
#[test]
fn test_eventfd_count_empty() {
    let count: usize = 0;
    assert_eq!(count, 0);
}

#[test]
fn test_eventfd_count_nonzero() {
    let count: usize = 5;
    assert!(count > 0);
}

#[test]
fn test_eventfd_stats_fields() {
    let active = 10usize;
...
#[test]
fn test_eventfd_u64_to_le_bytes() {
...
#[test]
fn test_eventfd_u64_from_le_bytes() {
