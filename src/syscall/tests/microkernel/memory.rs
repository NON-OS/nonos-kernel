...
#[test]
fn test_e_inval_constant() {
    assert_eq!(E_INVAL, -22);
}

#[test]
fn test_e_nomem_constant() {
    assert_eq!(E_NOMEM, -12);
}

#[test]
fn test_e_perm_constant() {
    assert_eq!(E_PERM, -1);
}
...
#[test]
fn test_user_mmap_base_constant() {
    assert_eq!(USER_MMAP_BASE, 0x4000_0000);
}

#[test]
fn test_user_space_max_constant() {
    assert_eq!(USER_SPACE_MAX, 0x7FFF_FFFF_FFFF);
}

#[test]
fn test_max_mmap_size_constant() {
...
#[test]
fn test_map_private_constant() {
    assert_eq!(MAP_PRIVATE, 0x02);
}

#[test]
fn test_map_anonymous_constant() {
    assert_eq!(MAP_ANONYMOUS, 0x20);
}

#[test]
fn test_sys_mmap_zero_length_returns_einval() {
    let result = E_INVAL;
    assert_eq!(result, -22);
}

#[test]
fn test_sys_mmap_length_exceeds_max_returns_einval() {
...
#[test]
fn test_sys_mmap_invalid_addr_returns_eperm() {
    let result = E_PERM;
    assert_eq!(result, -1);
}

#[test]
fn test_sys_mmap_success_returns_address() {
    let addr: i64 = USER_MMAP_BASE as i64;
    assert!(addr > 0);
}

#[test]
fn test_sys_mmap_no_memory_returns_enomem() {
    let result = E_NOMEM;
    assert_eq!(result, -12);
}

#[test]
fn test_sys_mmap_prot_read_permission() {
    let prot = PROT_READ;
    assert_eq!(prot & PROT_READ, PROT_READ);
}

#[test]
fn test_sys_mmap_prot_write_permission() {
    let prot = PROT_WRITE;
    assert_eq!(prot & PROT_WRITE, PROT_WRITE);
}

#[test]
fn test_sys_mmap_prot_exec_permission() {
    let prot = PROT_EXEC;
    assert_eq!(prot & PROT_EXEC, PROT_EXEC);
}

#[test]
fn test_sys_mmap_prot_read_write() {
    let prot = PROT_READ | PROT_WRITE;
    assert_eq!(prot, 0x3);
}

#[test]
fn test_sys_mmap_prot_read_exec() {
    let prot = PROT_READ | PROT_EXEC;
    assert_eq!(prot, 0x5);
}

#[test]
fn test_sys_mmap_addr_zero_auto_allocate() {
    let addr: u64 = 0;
    assert_eq!(addr, 0);
}

#[test]
fn test_sys_mmap_addr_nonzero_fixed() {
    let addr: u64 = 0x5000_0000;
    assert!(addr > 0);
}

#[test]
fn test_sys_munmap_zero_addr_returns_einval() {
    let result = E_INVAL;
    assert_eq!(result, -22);
}

#[test]
fn test_sys_munmap_zero_length_returns_einval() {
    let result = E_INVAL;
    assert_eq!(result, -22);
}

#[test]
fn test_sys_munmap_unaligned_addr_returns_einval() {
    let result = E_INVAL;
    assert_eq!(result, -22);
}

#[test]
fn test_sys_munmap_success_returns_zero() {
    let result: i64 = 0;
    assert_eq!(result, 0);
}
...
#[test]
fn test_is_user_space_invalid_addr() {
...
#[test]
fn test_is_user_space_overflow() {
...
#[test]
fn test_next_user_va_initial() {
    let initial = USER_MMAP_BASE;
    assert_eq!(initial, 0x4000_0000);
}

#[test]
fn test_next_user_va_increment() {
...
#[test]
fn test_next_user_va_overflow_check() {
