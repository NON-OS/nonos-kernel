...
use core::arch::asm;
use super::types::{UserDesc, GdtEntry64, GDT_ENTRY_TLS_MIN, GDT_ENTRIES};
...
pub fn get_ds() -> u16 {
    let ds: u16;
    unsafe { asm!("mov {0:x}, ds", out(reg) ds, options(nomem, nostack, preserves_flags)); }
    ds
}
...
pub fn get_gs() -> u16 {
    let gs: u16;
    unsafe { asm!("mov {0:x}, gs", out(reg) gs, options(nomem, nostack, preserves_flags)); }
    gs
}

pub fn get_fs() -> u16 {
    let fs: u16;
    unsafe { asm!("mov {0:x}, fs", out(reg) fs, options(nomem, nostack, preserves_flags)); }
    fs
}
