...
use crate::process::scheduler::{self as policy, SCHED_NORMAL, SCHED_FIFO, SCHED_RR, SCHED_BATCH, SCHED_IDLE, SCHED_DEADLINE, SCHED_PRIORITY_MIN, SCHED_PRIORITY_MAX};
...
pub fn handle_sched_setparam(pid: i32, param: u64) -> SyscallResult {
    if param == 0 { return errno(14); }
    let target_pid = match resolve_pid(pid) { Some(p) => p, None => return errno(3) };
    if !can_modify_process(target_pid) { return errno(1); }
    let priority: i32 = match read_user_value(param) { Ok(v) => v, Err(_) => return errno(14) };
    let current_policy = policy::get_policy(target_pid);
    match current_policy {
        SCHED_FIFO | SCHED_RR => {
            if priority < SCHED_PRIORITY_MIN || priority > SCHED_PRIORITY_MAX { return errno(22); }
        }
        _ => { if priority != 0 { return errno(22); } }
    }
    if policy::set_priority(target_pid, priority).is_err() { return errno(22); }
    ok(0)
}
...
pub fn handle_sched_getparam(pid: i32, param: u64) -> SyscallResult {
    ...
    if write_user_value(param, &priority).is_err() { return errno(14); }
    ok(0)
}
...
pub fn handle_sched_setscheduler(pid: i32, sched_policy: i32, param: u64) -> SyscallResult {
    ...
    let priority: i32 = match read_user_value(param) { Ok(v) => v, Err(_) => return errno(14) };
    if policy::set_policy(target_pid, sched_policy, priority).is_err() { return errno(22); }
