...
pub fn handle_mremap(old_addr: u64, old_size: u64, new_size: u64, flags: u64) -> SyscallResult {
    ...
    if (flags & MREMAP_MAYMOVE) == 0 { return errno(12); }
    ...
    let new_addr = match proc.mmap(None, new_size as usize, page_flags) {
        Ok(addr) => addr, Err(_) => return errno(12)
    };
    let mut buf = alloc::vec![0u8; old_size as usize];
    if copy_from_user(old_addr, &mut buf).is_err() { return errno(14); }
    if copy_to_user(new_addr.as_u64(), &buf).is_err() { return errno(14); }
    let _ = proc.munmap(VirtAddr::new(old_addr), old_size as usize);
    SyscallResult { value: new_addr.as_u64() as i64, capability_consumed: false, audit_required: true }
}
