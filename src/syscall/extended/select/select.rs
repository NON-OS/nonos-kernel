...
use super::types::{FdSet, Timeval, Timespec, FD_SETSIZE, EINVAL, EINTR};
use super::check::{is_fd_readable, is_fd_writable, has_fd_exception};
...
fn write_fdset_opt(ptr: u64, fds: &Option<FdSet>) {
    if ptr != 0 { if let Some(ref fds) = fds { let _ = write_user_value(ptr, fds); } }
}

fn do_select(
    nfds: i32, readfds: &mut Option<FdSet>, writefds: &mut Option<FdSet>,
    exceptfds: &mut Option<FdSet>, timeout_ms: Option<u64>,
) -> SyscallResult {
    ...
    loop {
        ...
        for fd in 0..nfds {
            if let Some(ref fds) = readfds {
                if fds.isset(fd) && is_fd_readable(fd) { result_read.set(fd); ready_count += 1; }
            }
            if let Some(ref fds) = writefds {
                if fds.isset(fd) && is_fd_writable(fd) { result_write.set(fd); ready_count += 1; }
            }
            if let Some(ref fds) = exceptfds {
                if fds.isset(fd) && has_fd_exception(fd) { result_except.set(fd); ready_count += 1; }
            }
        }
        if ready_count > 0 {
            if readfds.is_some() { *readfds = Some(result_read); }
            if writefds.is_some() { *writefds = Some(result_write); }
            if exceptfds.is_some() { *exceptfds = Some(result_except); }
...
fn zero_fdsets(readfds: &mut Option<FdSet>, writefds: &mut Option<FdSet>, exceptfds: &mut Option<FdSet>) {
    if let Some(ref mut fds) = readfds { fds.zero(); }
    if let Some(ref mut fds) = writefds { fds.zero(); }
    if let Some(ref mut fds) = exceptfds { fds.zero(); }
}
