...
impl EventFdInstance {
    ...
    pub fn is_semaphore(&self) -> bool { (self.flags & EFD_SEMAPHORE) != 0 }
    pub fn is_nonblock(&self) -> bool { (self.flags & EFD_NONBLOCK) != 0 }
    pub fn get_counter(&self) -> u64 { self.counter.load(Ordering::Acquire) }
    pub fn read(&self) -> Result<u64, i32> {
        loop {
            let current = self.counter.load(Ordering::Acquire);
            if current == 0 {
                if self.is_nonblock() { return Err(EAGAIN); }
                core::hint::spin_loop();
                continue;
            }
            if self.is_semaphore() {
                if self.counter.compare_exchange(current, current - 1, Ordering::AcqRel, Ordering::Acquire).is_ok() { return Ok(1); }
            } else {
                if self.counter.compare_exchange(current, 0, Ordering::AcqRel, Ordering::Acquire).is_ok() { return Ok(current); }
            }
        }
    }
    pub fn write(&self, value: u64) -> Result<(), i32> {
        if value == u64::MAX { return Err(EINVAL); }
        loop {
            let current = self.counter.load(Ordering::Acquire);
            ...
            if self.counter.compare_exchange(current, current + value, Ordering::AcqRel, Ordering::Acquire).is_ok() { return Ok(()); }
        }
    }
}
...
pub fn handle_eventfd2(initval: u32, flags: i32) -> SyscallResult {
    ...
    if EVENTFD_INSTANCES.lock().len() >= MAX_EVENTFD_INSTANCES { return errno(ENOMEM); }
