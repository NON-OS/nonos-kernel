...
pub fn handle_semctl(semid: i32, semnum: i32, cmd: i32, arg: u64) -> SyscallResult {
    ...
    match cmd {
        IPC_RMID => { if sets.remove(&semid).is_some() { ok(0) } else { errno(22) } }
...
fn handle_sem_stat(sets: &alloc::collections::BTreeMap<i32, super::sem_types::SemaphoreSet>, semid: i32, arg: u64) -> SyscallResult {
    ...
    if let Some(set) = sets.get(&semid) {
        ...
        if copy_to_user(arg, &buf).is_err() { return errno(14); }
        ok(0)
    } else { errno(22) }
}

fn handle_getval(sets: &alloc::collections::BTreeMap<i32, super::sem_types::SemaphoreSet>, semid: i32, semnum: i32) -> SyscallResult {
    if let Some(set) = sets.get(&semid) {
        if semnum < 0 || semnum as usize >= set.nsems { return errno(22); }
        ok(set.sems[semnum as usize].value as i64)
    } else { errno(22) }
}

fn handle_setval(sets: &mut alloc::collections::BTreeMap<i32, super::sem_types::SemaphoreSet>, semid: i32, semnum: i32, arg: u64) -> SyscallResult {
...
fn handle_getpid(sets: &alloc::collections::BTreeMap<i32, super::sem_types::SemaphoreSet>, semid: i32, semnum: i32) -> SyscallResult {
    if let Some(set) = sets.get(&semid) {
        if semnum < 0 || semnum as usize >= set.nsems { return errno(22); }
...
fn handle_getncnt(sets: &alloc::collections::BTreeMap<i32, super::sem_types::SemaphoreSet>, semid: i32, semnum: i32) -> SyscallResult {
    if let Some(set) = sets.get(&semid) {
        if semnum < 0 || semnum as usize >= set.nsems { return errno(22); }
...
fn handle_getzcnt(sets: &alloc::collections::BTreeMap<i32, super::sem_types::SemaphoreSet>, semid: i32, semnum: i32) -> SyscallResult {
    if let Some(set) = sets.get(&semid) {
        if semnum < 0 || semnum as usize >= set.nsems { return errno(22); }
...
fn handle_getall(sets: &alloc::collections::BTreeMap<i32, super::sem_types::SemaphoreSet>, semid: i32, arg: u64) -> SyscallResult {
    if arg == 0 { return errno(14); }
    if let Some(set) = sets.get(&semid) {
        ...
        if copy_to_user(arg, &bytes).is_err() { return errno(14); }
        ok(0)
    } else { errno(22) }
}

fn handle_setall(sets: &mut alloc::collections::BTreeMap<i32, super::sem_types::SemaphoreSet>, semid: i32, arg: u64) -> SyscallResult {
    if arg == 0 { return errno(14); }
    if let Some(set) = sets.get_mut(&semid) {
