...
use super::constants::*;
use super::shm_types::{ok, SHM_SEGMENTS, SHM_ATTACHMENTS};

pub fn handle_shmat(shmid: i32, shmaddr: u64, shmflg: i32) -> SyscallResult {
    let mut segments = SHM_SEGMENTS.lock();
    let segment = match segments.get_mut(&shmid) { Some(s) => s, None => return errno(22) };
    if segment.marked_for_removal { return errno(43); }
    let attach_addr = if shmaddr == 0 {
        let pcb = match crate::process::current_process() { Some(p) => p, None => return errno(14) };
        let flags = x86_64::structures::paging::PageTableFlags::WRITABLE;
        match pcb.mmap(None, segment.size, flags) { Ok(va) => va.as_u64(), Err(_) => return errno(12) }
    } else {
        if (shmflg & SHM_RND) != 0 { shmaddr & !0xFFF }
        else if (shmaddr & 0xFFF) != 0 { return errno(22); }
        else { shmaddr }
    };
    if copy_to_user(attach_addr, &segment.data).is_err() { return errno(14); }
...
pub fn handle_shmdt(shmaddr: u64) -> SyscallResult {
    if shmaddr == 0 || (shmaddr & 0xFFF) != 0 { return errno(22); }
    let pid = crate::process::current_pid().unwrap_or(0);
    let shmid = {
        let mut attachments = SHM_ATTACHMENTS.lock();
        match attachments.remove(&(pid, shmaddr)) { Some(id) => id, None => return errno(22) }
    };
    let mut segments = SHM_SEGMENTS.lock();
    if let Some(segment) = segments.get_mut(&shmid) {
        ...
        segment.lpid = pid;
        if segment.marked_for_removal && segment.nattch == 0 { segments.remove(&shmid); }
    }
