...
pub fn handle_epoll_create1(flags: i32) -> SyscallResult {
    if flags != 0 && flags != EPOLL_CLOEXEC { return errno(EINVAL); }
    let id = NEXT_EPOLL_ID.fetch_add(1, Ordering::SeqCst);
    let mut instances = EPOLL_INSTANCES.lock();
    if instances.len() >= MAX_EPOLL_INSTANCES { return errno(ENOMEM); }
...
pub fn handle_epoll_ctl(epfd: i32, op: i32, fd: i32, event_ptr: u64) -> SyscallResult {
    let epoll_id = match get_epoll_id(epfd) { Some(id) => id, None => return errno(EBADF) };
    if epfd == fd { return errno(EINVAL); }
    if get_fd_info(fd).is_none() { return errno(EBADF); }
    let event = if op != EPOLL_CTL_DEL {
        if event_ptr == 0 { return errno(EFAULT); }
        let ev: EpollEvent = match read_user_value(event_ptr) { Ok(v) => v, Err(_) => return errno(EFAULT) };
        Some(ev)
    } else { None };
    let mut instances = EPOLL_INSTANCES.lock();
    let instance = match instances.get_mut(&epoll_id) { Some(i) => i, None => return errno(EBADF) };
    let result = match op {
        EPOLL_CTL_ADD => match event { Some(ev) => instance.add(fd, ev.events, ev.data), None => Err(EFAULT) },
        EPOLL_CTL_MOD => match event { Some(ev) => instance.modify(fd, ev.events, ev.data), None => Err(EFAULT) },
        EPOLL_CTL_DEL => instance.delete(fd),
        _ => Err(EINVAL),
    };
    match result { Ok(()) => SyscallResult::success(0), Err(e) => errno(e) }
}
...
pub fn handle_epoll_pwait(
    epfd: i32, events_ptr: u64, maxevents: i32, timeout: i32, _sigmask: u64, _sigsetsize: u64,
) -> SyscallResult {
    if maxevents <= 0 { return errno(EINVAL); }
    if events_ptr == 0 { return errno(EFAULT); }
    let epoll_id = match get_epoll_id(epfd) { Some(id) => id, None => return errno(EBADF) };
    ...
    loop {
        ...
        if timeout == 0 { return SyscallResult::success(0); }
        if crate::time::timestamp_millis().saturating_sub(start_time) >= timeout_ms { return SyscallResult::success(0); }
