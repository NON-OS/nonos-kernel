...
pub fn handle_link(oldpath: u64, newpath: u64) -> SyscallResult {
    if oldpath == 0 || newpath == 0 { return errno(14); }
    let old_str = match read_user_string(oldpath, 4096) { Ok(s) => s, Err(_) => return errno(14) };
    let new_str = match read_user_string(newpath, 4096) { Ok(s) => s, Err(_) => return errno(14) };
...
pub fn handle_symlink(target: u64, linkpath: u64) -> SyscallResult {
    if target == 0 || linkpath == 0 { return errno(14); }
    let target_str = match read_user_string(target, 4096) { Ok(s) => s, Err(_) => return errno(14) };
    let link_str = match read_user_string(linkpath, 4096) { Ok(s) => s, Err(_) => return errno(14) };
...
pub fn handle_readlinkat(dirfd: i32, pathname: u64, buf: u64, bufsiz: u64) -> SyscallResult {
    if pathname == 0 || buf == 0 { return errno(14); }
    let path = match read_user_string(pathname, 4096) { Ok(s) => s, Err(_) => return errno(14) };
