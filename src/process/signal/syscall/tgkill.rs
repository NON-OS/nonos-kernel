...
use crate::process::with_process;

const EINVAL: i64 = -22;
const EPERM: i64 = -1;
const ESRCH: i64 = -3;

pub fn sys_tgkill(tgid: u64, tid: u64, sig: u64) -> i64 {
    if sig > SIGRTMAX as u64 {
        return EINVAL;
    }
    let target = tid as u32;
    let group = tgid as u32;
    let target_tgid = match with_process(target, |pcb| pcb.thread_group_id()) {
        Some(t) => t,
        None => return ESRCH,
    };
    if target_tgid != group {
        return ESRCH;
    }
