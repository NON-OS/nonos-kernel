...
impl ProcessScheduler {
    ...
    fn calculate_time_slice(&self, process: &Process) -> u32 {
        match process.priority {
            ...
            Priority::Normal(_) => {
                ...
                let nr_running =
                    self.cpu_queues.iter().map(|q| q.cfs_nr_running).sum::<u32>().max(1);
    ...
    pub fn load_balance(&mut self) {
        ...
        if max_load > min_load * 125 / 100 {
            // 25% imbalance threshold
    ...
    fn migrate_tasks(&mut self, from_cpu: u32, to_cpu: u32) {
        if from_cpu >= self.cpu_queues.len() as u32
            || to_cpu >= self.cpu_queues.len() as u32
            || from_cpu == to_cpu
        {
            return;
        }
...
pub fn schedule_current_cpu() -> Option<ProcessId> {
    let current_cpu = crate::arch::x86_64::cpu::current_cpu_id();

    if let Ok(mut sched_guard) = get_scheduler() {
        if let Some(ref mut scheduler) = *sched_guard {
            return scheduler.schedule(current_cpu as u32);
        }
    }
