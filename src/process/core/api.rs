...
#[inline] pub fn current_pid() -> Option<Pid> { match CURRENT_PID.load(Ordering::SeqCst) { 0 => None, v => Some(v) } }
#[inline] pub fn current_process() -> Option<Arc<ProcessControlBlock>> { PROCESS_TABLE.find_by_pid(current_pid()?) }
...
pub fn get_process_stats() -> ProcessManagementStats {
    ...
    for p in list {
        match *p.state.lock() {
            ProcessState::Running => s.running += 1, ProcessState::Sleeping => s.sleeping += 1,
            ProcessState::Stopped => s.stopped += 1, _ => {}
        }
    }
    s
}
