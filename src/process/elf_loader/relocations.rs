...
pub fn apply_relocations(data: &[u8], header: &Elf64Header, base_addr: u64) -> Result<(), ElfError> {
    ...
    if dyn_offset + dyn_size > data.len() { return Err(ElfError::InvalidProgramHeader); }
    ...
    let mut rela_addr = 0u64; let mut rela_size = 0u64; let mut rela_ent = 0u64;
    let mut symtab = 0u64; let mut strtab = 0u64;
    let mut jmprel = 0u64; let mut pltrelsz = 0u64;
    ...
    if rela_addr != 0 && rela_size != 0 {
        if rela_ent != size_of::<Elf64Rela>() as u64 { return Err(ElfError::RelocationFailed); }
...
fn apply_rela_section(base: u64, rela_addr: u64, rela_size: u64, symtab: u64, strtab: u64) -> Result<(), ElfError> {
    ...
    for i in 0..num_relas {
        ...
        match rel_type {
            ...
            R_X86_64_PC32 => {
                let sym_val = resolve_symbol(base, symtab, strtab, sym_idx)?;
                let value = sym_val.wrapping_add(rela.r_addend as u64).wrapping_sub(rela.r_offset + base);
            ...
            R_X86_64_COPY => {
                ...
                if sym.st_size > 0 {
                    unsafe { core::ptr::copy_nonoverlapping(sym_val as *const u8, target as *mut u8, sym.st_size as usize) };
                }
            }
            _ => {}
        }
