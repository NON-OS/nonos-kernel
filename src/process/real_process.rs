...
use alloc::{vec, vec::Vec, collections::BTreeMap, string::{String, ToString}, sync::Arc};
...
impl ProcessControlBlock {
    ...
    pub fn mmap(&self, addr: Option<VirtAddr>, size: usize, flags: PageTableFlags) -> Result<VirtAddr, &'static str> {
        ...
        if memory.page_table.as_u64() != 0 {
            unsafe {
                x86_64::registers::control::Cr3::write(
                    x86_64::structures::paging::PhysFrame::from_start_address(memory.page_table).unwrap(),
...
impl ProcessTable {
    ...
    /// Create new process
    pub fn create_process(&self, name: String, parent_pid: Pid) -> Result<Arc<ProcessControlBlock>, &'static str> {
    ...
    pub fn get_processes_by_state(&self, state: ProcessState) -> Vec<Arc<ProcessControlBlock>> {
        let processes = self.processes.read();
        processes.values()
            .filter(|proc| proc.get_state() == state)
            .cloned()
            .collect()
    }
    ...
    pub fn fork(&self) -> Result<Pid, &'static str> {
        ...
        let child_process = ProcessControlBlock::new(
            child_pid,
            current.pid,
            current.name.clone()
        );
        
        // Copy process state (fields in Arc cannot be directly assigned, need to be set during creation or use interior mutability)
    ...
    pub fn exec(&self, path: &str, argv: Vec<String>, envp: Vec<String>) -> Result<(), &'static str> {
        let current = self.current_process().ok_or("No current process")?;
...
/// Create new process
pub fn create_process(name: String, parent_pid: Pid) -> Result<Arc<ProcessControlBlock>, &'static str> {
...
pub mod syscalls {
    ...
    pub fn sys_exec(path: *const u8, argv: *const *const u8, envp: *const *const u8) -> Result<i32, &'static str> {
    ...
    pub fn sys_kill(pid: i32, signal: i32) -> Result<i32, &'static str> {
        ...
        let target_process = get_process_table().get_process(pid as u32)
