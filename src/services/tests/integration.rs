...
pub fn test_request_encode_then_parse() -> TestResult {
    ...
    if parsed.is_none() { return TestResult::Fail; }
    let req = parsed.unwrap();
    if req.seq != original.seq { return TestResult::Fail; }
    if req.op != original.op { return TestResult::Fail; }
    if req.payload != original.payload { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_request_encode_parse_open() -> TestResult {
    ...
    if parsed.is_none() { return TestResult::Fail; }
    let req = parsed.unwrap();
    if req.op != ServiceOp::Open { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_request_encode_parse_close() -> TestResult {
    ...
    if parsed.is_none() { return TestResult::Fail; }
    let req = parsed.unwrap();
    if req.op != ServiceOp::Close { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_request_encode_parse_read() -> TestResult {
    ...
    if parsed.is_none() { return TestResult::Fail; }
    let req = parsed.unwrap();
    if req.op != ServiceOp::Read { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_request_encode_parse_ioctl() -> TestResult {
    ...
    if parsed.is_none() { return TestResult::Fail; }
    let req = parsed.unwrap();
    if req.op != ServiceOp::Ioctl { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_request_encode_parse_query() -> TestResult {
    ...
    if parsed.is_none() { return TestResult::Fail; }
    let req = parsed.unwrap();
    if req.op != ServiceOp::Query { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_request_encode_parse_subscribe() -> TestResult {
    ...
    if parsed.is_none() { return TestResult::Fail; }
    let req = parsed.unwrap();
    if req.op != ServiceOp::Subscribe { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_request_encode_parse_unsubscribe() -> TestResult {
    ...
    if parsed.is_none() { return TestResult::Fail; }
    let req = parsed.unwrap();
    if req.op != ServiceOp::Unsubscribe { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_request_encode_parse_preserves_seq() -> TestResult {
    for seq in [0u32, 1, 100, 1000, u32::MAX / 2, u32::MAX] {
        ...
        if parsed.seq != seq { return TestResult::Fail; }
    }
    TestResult::Pass
}
...
pub fn test_request_encode_parse_preserves_flags() -> TestResult {
    ...
    let encoded = original.encode();
    let parsed = parse_request(&encoded).unwrap();
    if parsed.flags != 0x1234 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_request_encode_parse_preserves_payload() -> TestResult {
    ...
    let encoded = original.encode();
    let parsed = parse_request(&encoded).unwrap();
    if parsed.payload != payload { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_response_err_then_encode() -> TestResult {
    ...
    if status != -500 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_cap_check_with_service_cap() -> TestResult {
    ...
    if cap.has(CAP_CRYPTO) { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_service_message_roundtrip_request() -> TestResult {
    ...
    match msg {
        ServiceMessage::Request(r) => {
            if r.seq != req.seq { return TestResult::Fail; }
            if r.op != req.op { return TestResult::Fail; }
            if r.payload != req.payload { return TestResult::Fail; }
        }
...
pub fn test_service_message_roundtrip_response() -> TestResult {
    ...
    match msg {
        ServiceMessage::Response(r) => {
            if r.seq != resp.seq { return TestResult::Fail; }
            if r.status != resp.status { return TestResult::Fail; }
            if r.payload != resp.payload { return TestResult::Fail; }
        }
        _ => return TestResult::Fail,
    }
    TestResult::Pass
...
pub fn test_all_ops_can_be_encoded_and_parsed() -> TestResult {
    ...
    for (i, op) in ops.iter().enumerate() {
        ...
        let parsed = parse_request(&encoded);
        if parsed.is_none() { return TestResult::Fail; }
        if parsed.unwrap().op != *op { return TestResult::Fail; }
    }
    TestResult::Pass
}
...
pub fn test_client_error_from_response_status() -> TestResult {
    ...
    match err {
        ClientError::RemoteError(code) => {
            if code != -403 { return TestResult::Fail; }
        }
        _ => return TestResult::Fail,
    }
    TestResult::Pass
...
pub fn test_cap_bits_do_not_overlap() -> TestResult {
    ...
    for i in 0..all_caps.len() {
        for j in (i + 1)..all_caps.len() {
            if all_caps[i] & all_caps[j] != 0 { return TestResult::Fail; }
        }
    }
    TestResult::Pass
}
...
pub fn test_service_cap_combined_check() -> TestResult {
    ...
    if partial.has(required) { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_error_types_are_distinct() -> TestResult {
    ...
    if !cap_debug.contains("NoCap") { return TestResult::Fail; }
    if !client_debug.contains("NotFound") { return TestResult::Fail; }
    if !server_debug.contains("RegistrationFailed") { return TestResult::Fail; }
    if !reg_debug.contains("Full") { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_service_endpoint_with_all_fields() -> TestResult {
    ...
    if ep.name != "test_service" { return TestResult::Fail; }
    if ep.port != 8080 { return TestResult::Fail; }
    if ep.pid != 42 { return TestResult::Fail; }
    if ep.caps_required != CAP_VFS | CAP_NET { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_service_response_status_codes() -> TestResult {
    ...
    if err_cap.status != -403 { return TestResult::Fail; }

    let err_not_found = ServiceResponse::err(3, -404);
    if err_not_found.status != -404 { return TestResult::Fail; }

    let err_internal = ServiceResponse::err(4, -500);
    if err_internal.status != -500 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_encode_decode_consistency() -> TestResult {
    ...
    if decoded.seq != original_req.seq { return TestResult::Fail; }
    if decoded.op != original_req.op { return TestResult::Fail; }
    if decoded.payload != original_req.payload { return TestResult::Fail; }
    ...
    if seq != original_resp.seq { return TestResult::Fail; }
    if status != original_resp.status { return TestResult::Fail; }
    if &resp_encoded[8..] != &original_resp.payload[..] { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_cap_admin_is_highest_bit() -> TestResult {
    ...
    if CAP_ADMIN <= CAP_SHELL { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_service_cap_no_expiry_means_never_expires() -> TestResult {
    ...
    if cap.is_expired(u64::MAX - 1) { return TestResult::Fail; }
    TestResult::Pass
}
