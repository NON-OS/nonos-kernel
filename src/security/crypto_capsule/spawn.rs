...
pub fn spawn_crypto_capsule() -> Result<(), SpawnError> {
    ...
    let entry = load_elf_into_capsule_as(CRYPTO_ELF, pid)?;

    // Capsule needs IPC for mk_ipc_*, Memory for heap, Crypto so the
    // current proxy ops (and any future kernel-crypto fallback during
    // bring-up) work. CAP_CRYPTO is the user-facing gate, not the
    // capsule's own bit.
...
fn load_elf_into_capsule_as(elf: &'static [u8], pid: u32) -> Result<u64, SpawnError> {
    ...
    let load = load_elf_executable(elf).map_err(|err| {
        crate::sys::serial::println(b"[CRYPTO-DEBUG] load_elf_executable error:");
