...
use super::engine::{is_initialized, derive_region_key, generate_nonce, aes_gcm_encrypt, aes_gcm_decrypt, rotate_master_key};
use super::region::{get_region, update_region, register_region, unregister_region};

pub fn encrypt_region(start: u64, size: usize) -> Result<(), EncryptionError> {
    if !is_initialized() { return Err(EncryptionError::NotInitialized); }
    let mut region = get_region(start).ok_or(EncryptionError::RegionNotFound)?;
    if region.encrypted { return Ok(()); }
    if region.size != size { return Err(EncryptionError::SizeMismatch); }
...
pub fn rotate_keys() {
    ...
    crate::security::monitoring::audit::log_security_event(
        "memencrypt", crate::security::monitoring::audit::AuditSeverity::Info,
        alloc::format!("Memory encryption master key rotated"), None, None, None);
}
