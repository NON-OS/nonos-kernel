...
impl TrustedHashDatabase {
    ...
    pub fn verify_hash(&self, name: &str, data: &[u8]) -> bool {
        ...
        if let Some(trusted_hash) = self.get_hash(name) {
            ...
            if !is_valid {
                ...
                crate::log::logger::log_info!("{}", &format!("Hash verification failed for: {}", name));
            } else {
    ...
    pub fn get_statistics(&self) -> HashDbStats {
        HashDbStats {
            ...
            critical_hashes: AtomicU64::new(self.statistics.critical_hashes.load(Ordering::Relaxed)),
            verifications_performed: AtomicU64::new(self.statistics.verifications_performed.load(Ordering::Relaxed)),
            verification_failures: AtomicU64::new(self.statistics.verification_failures.load(Ordering::Relaxed)),
    ...
    pub fn maintenance(&self) {
        ...
        if removed_count > 0 {
            ...
            crate::log::logger::log_info!("{}", &format!("Removed {} old hash entries", removed_count));
        }
    }
}
...
pub fn verify_integrity() {
    ...
    if failures.is_empty() {
        crate::log::logger::log_info!("Integrity verification passed: {}/{} checks successful", 
            verifications, verifications);
    } else {
        crate::log::logger::log_err!("Integrity verification FAILED: {} critical failures detected", 
            failures.len());
...
fn verify_kernel_text_section() -> bool {
    ...
    unsafe {
        ...
        if text_size > 0 {
            ...
            if !matches_hash {
                crate::log::logger::log_err!("Kernel text hash mismatch: expected {:?}, got {:?}", 
                    KERNEL_TEXT_HASH, computed_hash);
            }
...
fn verify_idt_integrity() -> bool {
    ...
    let idt_bytes = unsafe {
        core::slice::from_raw_parts(idt_ptr.base.as_ptr::<u8>(), idt_size)
    };
...
fn verify_gdt_integrity() -> bool {
    ...
    // For now, just check if computation succeeds - in real system would compare to known hash
...
fn verify_modules_integrity() -> bool {
    ...
    for module in loaded_modules {
        ...
        let module_bytes = unsafe {
            core::slice::from_raw_parts(
                module.base_address as *const u8,
                module.size
            )
        };
...
fn verify_drivers_integrity() -> bool {
    ...
    for driver in critical_drivers {
        ...
        let driver_bytes = unsafe {
            core::slice::from_raw_parts(
                driver.base_address as *const u8,
                driver.size
            )
        };
