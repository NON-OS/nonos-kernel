...
impl FirmwareDatabase {
    ...
    fn analyze_firmware_heuristics(&self, vendor: &str, version: &str) -> bool {
        ...
        let vendor_trusted = known_good_vendors
            .iter()
        ...
        let suspicious_patterns = ["1.0.0.0", "0.0.0.1", "99.99.99", "1337", "hack", "crack"];

        let version_suspicious =
            suspicious_patterns.iter().any(|&pattern| version.to_lowercase().contains(pattern));
    ...
    pub fn add_firmware_entry(&self, entry: FirmwareEntry) {
        ...
        let is_suspicious =
            matches!(entry.trust_level, TrustLevel::Suspicious | TrustLevel::Malicious);
    ...
    pub fn get_firmware_vulnerabilities(
        &self,
        vendor: &str,
        version: &str,
    ) -> Vec<FirmwareVulnerability> {
        ...
        vulnerabilities
            ...
            .filter(|vuln| {
                vuln.affected_versions.iter().any(|affected| {
                    affected == &target_version || version_matches_pattern(version, affected)
                })
            })
            .cloned()
            .collect()
    ...
    pub fn update_database(
        ...
        vulnerabilities: Vec<FirmwareVulnerability>,
    ) {
    ...
    pub fn get_statistics(&self) -> FirmwareDbStats {
        FirmwareDbStats {
            ...
            trusted_entries: AtomicU64::new(
                self.statistics.trusted_entries.load(Ordering::Relaxed),
            ),
            suspicious_entries: AtomicU64::new(
                self.statistics.suspicious_entries.load(Ordering::Relaxed),
            ),
            total_vulnerabilities: AtomicU64::new(
                self.statistics.total_vulnerabilities.load(Ordering::Relaxed),
            ),
            queries_performed: AtomicU64::new(
                self.statistics.queries_performed.load(Ordering::Relaxed),
            ),
    ...
    pub fn maintenance(&self) {
        ...
        if entries_removed > 0 {
            ...
            crate::log::logger::log_info!(
                "{}",
                &format!("Firmware database maintenance: removed {} old entries", entries_removed)
            );
        }
    }
}
...
fn load_initial_firmware_data() {
    ...
    let vulnerabilities = vec![FirmwareVulnerability {
