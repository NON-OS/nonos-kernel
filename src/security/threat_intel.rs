...
use alloc::{vec, vec::Vec, collections::{BTreeMap, BTreeSet}, string::String, format};
...
impl ThreatIntelligence {
    ...
    pub fn is_malicious(&self, ip: [u8; 4]) -> bool {
        ...
        if let Some(info) = self.ip_database.read().get(&packed_ip) {
            let is_threat = matches!(info.reputation, ReputationLevel::Malicious | ReputationLevel::Blacklisted);
            if is_threat {
                self.statistics.threats_detected.fetch_add(1, Ordering::Relaxed);
            }
            return is_threat;
    ...
    pub fn update_database(&self, entries: Vec<IpInfo>) {
        ...
        crate::log::logger::log_info!("{}", &format!("Threat intelligence database updated with {} new entries", new_entries));
    }
    ...
    pub fn get_stats(&self) -> ThreatStats {
        ThreatStats {
            ...
            threats_detected: AtomicU64::new(self.statistics.threats_detected.load(Ordering::Relaxed)),
            false_positives: AtomicU64::new(self.statistics.false_positives.load(Ordering::Relaxed)),
            database_entries: AtomicU64::new(self.statistics.database_entries.load(Ordering::Relaxed)),
    ...
    pub fn maintenance(&self) {
        ...
        if removed > 0 {
            ...
            crate::log::logger::log_info!("{}", &format!("Removed {} old threat intelligence entries", removed));
        }
    }
}
...
pub fn update_threat_score(ip: [u8; 4], score_delta: i32) {
    let packed_ip = ((ip[0] as u32) << 24) | ((ip[1] as u32) << 16) | ((ip[2] as u32) << 8) | (ip[3] as u32);
    ...
    crate::log::logger::log_info!("Updated threat score for {:?}: {} -> {}", 
        ip, entry.threat_score.saturating_sub(score_delta.abs() as u8), entry.threat_score);
}