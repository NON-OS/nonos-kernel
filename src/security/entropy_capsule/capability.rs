// NONOS Operating System
// Copyright (C) 2026 NONOS Contributors
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program. If not, see <https://www.gnu.org/licenses/>.

use super::error::EntropyCapsuleError;
use crate::services::caps::{has_capability, CAP_ADMIN, CAP_ENTROPY};

// Read path: GetRandom + GetStats. Caller must hold CAP_ENTROPY. The
// pid is read from the kernel's process accounting, never from a
// caller-supplied payload.
pub(super) fn gate_read() -> Result<u32, EntropyCapsuleError> {
    let pid = match crate::process::current_pid() {
        Some(p) => p,
        None => return Err(EntropyCapsuleError::NoCallerPid),
    };
    if !has_capability(pid, CAP_ENTROPY) {
        return Err(EntropyCapsuleError::AccessDenied);
    }
    Ok(pid)
}

// Reseed path. Until a dedicated `CAP_ENTROPY_ADMIN` bit ships, reseed
// requires `CAP_ADMIN`. TODO(M1-3): split into a dedicated admin bit
// once the cap allocation table grows.
pub(super) fn gate_reseed() -> Result<u32, EntropyCapsuleError> {
    let pid = match crate::process::current_pid() {
        Some(p) => p,
        None => return Err(EntropyCapsuleError::NoCallerPid),
    };
    if !has_capability(pid, CAP_ADMIN) {
        return Err(EntropyCapsuleError::AccessDenied);
    }
    Ok(pid)
}
