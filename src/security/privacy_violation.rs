...
impl PrivacyViolationDetector {
    ...
    fn create_violation(
        ...
        PrivacyViolation {
            ...
            remediation_required: matches!(
                rule.severity,
                ViolationSeverity::Critical | ViolationSeverity::Catastrophic
            ),
    ...
    pub fn get_violations_by_severity(
        ...
        self.detected_violations.iter().filter(|v| v.severity == severity).collect()
    }

    pub fn get_violations_by_compliance(
        &self,
    ...
    pub fn generate_compliance_report(&self, compliance_type: ComplianceType) -> ComplianceReport {
        ...
        for violation in &violations {
            ...
            if let Some(framework) =
                self.compliance_frameworks.iter().find(|f| f.framework_type == compliance_type)
            {
    ...
    pub fn get_statistics(&self) -> PrivacyStatistics {
        ...
        let auto_blocked = self.detected_violations.iter().filter(|v| v.auto_blocked).count();
        ...
        PrivacyStatistics {
            ...
            compliance_frameworks_enabled: self
...
pub fn scan_for_privacy_violations(
    data: &[u8],
    source_process: u32,
    ...
    unsafe {
        if let Some(ref mut detector) = PRIVACY_VIOLATION_DETECTOR {
            let location = ViolationLocation {
                ...
                file_path: if matches!(location_type, LocationType::FileSystem) {
                    location_info.into()
                } else {
                    "".into()
                },
                network_destination: if matches!(location_type, LocationType::Network) {
                    location_info.into()
                } else {
                    "".into()
                },
...
pub fn get_privacy_statistics() -> Option<PrivacyStatistics> {
    unsafe { PRIVACY_VIOLATION_DETECTOR.as_ref().map(|d| d.get_statistics()) }
}
...
pub fn generate_gdpr_report() -> Option<ComplianceReport> {
    unsafe {
        PRIVACY_VIOLATION_DETECTOR
            .as_ref()
            .map(|d| d.generate_compliance_report(ComplianceType::GDPR))
    }
}
...
pub fn generate_ccpa_report() -> Option<ComplianceReport> {
    unsafe {
        PRIVACY_VIOLATION_DETECTOR
            .as_ref()
            .map(|d| d.generate_compliance_report(ComplianceType::CCPA))
    }
}
        ...
        if let Some(ref mut detector) = PRIVACY_VIOLATION_DETECTOR {
            ...
            if total_violations > 0 {
                crate::log::logger::log_warn!(
                    "Privacy violation check completed: {} violations found ({} critical)",
                    total_violations,
                    critical_violations
                );
                ...
                crate::log::logger::log_warn!(
                    "GDPR impact: {} violations, potential fine: ${}",
                    gdpr_report.total_violations,
                    gdpr_report.total_potential_fine
                );
                crate::log::logger::log_warn!(
                    "CCPA impact: {} violations, potential fine: ${}",
                    ccpa_report.total_violations,
                    ccpa_report.total_potential_fine
                );
                crate::log::logger::log_warn!(
                    "HIPAA impact: {} violations, potential fine: ${}",
                    hipaa_report.total_violations,
                    hipaa_report.total_potential_fine
                );

                // Trigger incident response for critical violations
                if critical_violations > 0 {
                    let violation_list = vec![format!(
                ...
                crate::log::logger::log_info!(
                    "Privacy violation check completed: No violations detected"
                );
            }
        } else {
...
fn scan_memory_for_violations(detector: &mut PrivacyViolationDetector) -> usize {
    ...
    if !heap_regions.is_empty() {
        for allocation in heap_regions {
            ...
            for violation in violations {
                crate::log::logger::log_warn!(
                    "Memory privacy violation at 0x{:x}",
    ...
    if let Some(net_stack) = crate::network::get_network_stack() {
        // 1. Scan RX queue for privacy violations in incoming packets
        {
            ...
            for packet in rx_queue.iter() {
                ...
                for violation in &violations {
                    crate::log::logger::log_warn!(
                        "Network privacy violation in RX packet from {}",
        ...
        // 2. Scan TX queue for outbound privacy violations
        {
            ...
            for packet in tx_queue.iter() {
                ...
                for violation in &violations {
                    crate::log::logger::log_warn!(
                        "Network privacy violation in TX packet to {}",
                    ...
                    for packet in rx_buffer.iter() {
                        let location = ViolationLocation {
                            ...
                            network_destination: format!(
                                ...
                                socket
                                    .remote_address
                                    .map_or("unknown".to_string(), |addr| format!("{:?}", addr)),
                                socket.remote_port
                            ),
                            memory_address: 0,
                        };
                        ...
                        for violation in &violations {
                            crate::log::logger::log_warn!(
                                "Socket privacy violation on handle {:?} to {}:{}",
                                handle,
                                socket
                                    .remote_address
                                    .map_or("unknown".to_string(), |addr| format!("{:?}", addr)),
        ...
        // 4. Access interfaces properly using .read() instead of .interfaces()
        {
            ...
            for (interface_id, interface) in interfaces.iter() {
                ...
                if stats.rx_packets.load(core::sync::atomic::Ordering::Relaxed) > 1000000 {
                    crate::log::logger::log_warn!(
                        "High packet volume on interface {}: potential data exfiltration",
                        interface_id
                    );
                    violation_count += 1;
                }
...
fn scan_filesystem_for_violations(detector: &mut PrivacyViolationDetector) -> usize {
    ...
    let sensitive_paths =
        ["/tmp", "/var/log", "/home", "/root", "/etc", "/var/cache", "/var/spool", "/var/mail"];
        ...
        if let Ok(entries) = crate::filesystem::read_directory(path) {
            for entry in entries.iter().take(50) {
                ...
                if let Ok(file_data) = crate::filesystem::read_file(&entry.path) {
                    let violations = detector.scan_file_operations(&entry.path, &file_data, 0xAAAA);
                    violation_count += violations.len();

                    for violation in violations {
                        crate::log::logger::log_warn!(
                            "Filesystem privacy violation: {:?} in {}",
...
fn scan_processes_for_violations(detector: &mut PrivacyViolationDetector) -> usize {
    ...
    for process in crate::process::get_all_processes() {
        ...
        if let Some(cmdline) = process.command_line() {
            ...
            for violation in violations {
                crate::log::logger::log_warn!(
                    "Process cmdline violation: {:?} in process {}",
                    violation.violation_type,
                    process.name()
                );
            }
    ...
    if let Some(_usb_manager) = crate::drivers::usb::get_usb_manager() {
        for device in crate::drivers::usb::get_connected_devices() {
            ...
            if device.is_storage_device() {
                if let Ok(device_contents) = device.read_sample_data(1024 * 1024) {
                    // Sample 1MB blocks
                    let location = ViolationLocation {
                    ...
                    for violation in violations {
                        crate::log::logger::log_warn!(
                            "USB device violation: {:?} on device {}",
                            violation.violation_type,
                            device.device_path()
                        );
                    }
