...
pub(crate) fn test_trusted_key_struct_fields() -> TestResult {
    ...
    if key.name != "test.key" { return TestResult::Fail; }
    if key.key != vec![1, 2, 3, 4] { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_trusted_key_empty_key_data() -> TestResult {
    ...
    if !key.key.is_empty() { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_trusted_key_32_byte_key() -> TestResult {
    ...
    if key.key.len() != 32 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_trusted_key_name_with_dots() -> TestResult {
    ...
    if !key.name.contains('.') { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_verify_integrity_with_mismatched_hash() -> TestResult {
    ...
    if verify_integrity("mismatch_test", &check_hash) { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_trusted_key_db_add_and_get() -> TestResult {
    ...
    if result.unwrap() != vec![1, 2, 3, 4] { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_add_trusted_key_overwrites() -> TestResult {
    ...
    if result != Some(vec![4, 5, 6]) { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_add_trusted_key_large_data() -> TestResult {
    ...
    if result.unwrap().len() != 1024 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_trusted_key_db_multiple_keys() -> TestResult {
    ...
    if get_trusted_key("multi_key_1") != Some(vec![1]) { return TestResult::Fail; }
    if get_trusted_key("multi_key_2") != Some(vec![2]) { return TestResult::Fail; }
    if get_trusted_key("multi_key_3") != Some(vec![3]) { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_trusted_hash_32_bytes() -> TestResult {
    ...
    if retrieved != Some(hash) { return TestResult::Fail; }
    TestResult::Pass
}
...
pub(crate) fn test_trusted_key_debug_format() -> TestResult {
    ...
    if !debug_str.contains("debug_key") { return TestResult::Fail; }
    TestResult::Pass
}
