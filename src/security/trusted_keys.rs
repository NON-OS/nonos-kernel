...
impl TrustedKeysManager {
    ...
    pub fn get_keys_by_usage(&self, usage: KeyUsage) -> Vec<TrustedKey> {
        ...
        keys.values()
            .filter(|key| {
                key.metadata.usage.contains(&usage) && 
                self.is_key_trusted(&key.metadata.id)
            })
            .cloned()
            .collect()
    }
    ...
    pub fn get_all_trusted_keys(&self) -> Vec<TrustedKey> {
        let keys = self.keys.read();
        keys.values()
            .filter(|key| self.is_key_trusted(&key.metadata.id))
            .cloned()
            .collect()
    }
...
pub fn verify_signature(signature_data: &[u8]) -> bool {
    if signature_data.len() < 4 { return false; }
    
    // Parse signature format: [sig_len:4][signature:sig_len][pubkey_len:4][pubkey:pubkey_len][hash:32]
    let sig_len = u32::from_le_bytes([signature_data[0], signature_data[1], signature_data[2], signature_data[3]]) as usize;
    if signature_data.len() < 4 + sig_len + 4 { return false; }
    ...
    if signature_data.len() < pubkey_len_offset + 4 + pubkey_len + 32 { return false; }
    
    let pubkey_data = &signature_data[pubkey_len_offset + 4..pubkey_len_offset + 4 + pubkey_len];
    let message_hash = &signature_data[pubkey_len_offset + 4 + pubkey_len..pubkey_len_offset + 4 + pubkey_len + 32];
    
    // Parse RSA public key: [n_len:4][n:n_len][e:4]
    if pubkey_data.len() < 8 { return false; }
    let n_len = u32::from_le_bytes([pubkey_data[0], pubkey_data[1], pubkey_data[2], pubkey_data[3]]) as usize;
    if pubkey_data.len() < 4 + n_len + 4 { return false; }
...
fn verify_pkcs1_sha256_padding(padded_data: &[u8], expected_hash: &[u8]) -> bool {
    if padded_data.len() < 51 { return false; }
    
    // PKCS#1 v1.5 format: 0x00 || 0x01 || PS || 0x00 || DigestInfo
    if padded_data[0] != 0x00 || padded_data[1] != 0x01 { return false; }
    ...
    for i in 2..padded_data.len() - 51 {
        ...
        if padded_data[i] != 0xFF { return false; } // Invalid padding byte
    }
    ...
    if padded_data.len() < sep_pos + 1 + sha256_digest_info.len() + 32 { return false; }
    
    // Verify DigestInfo
    let digest_info_start = sep_pos + 1;
    if &padded_data[digest_info_start..digest_info_start + sha256_digest_info.len()] != &sha256_digest_info {
        return false;
    }
...
fn big_multiply(a: &BigInteger, b: &BigInteger) -> BigInteger {
    ...
    for (i, &a_limb) in a.limbs.iter().enumerate() {
        ...
        for (j, &b_limb) in b.limbs.iter().enumerate() {
            let product = (a_limb as u128) * (b_limb as u128) + (result_limbs[i + j] as u128) + (carry as u128);
...
fn big_mod_reduce(dividend: &BigInteger, divisor: &BigInteger) -> BigInteger {
    ...
    while remainder.len() >= divisor_limbs.len() {
        ...
        let quotient_digit = if remainder.len() > divisor_limbs.len() {
            remainder_high / divisor_high
        } else {
            1
        };
