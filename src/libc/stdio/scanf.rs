...
pub unsafe extern "C" fn scanf(fmt: *const u8, mut args: ...) -> i32 {
    vfscanf_impl(super::file::stdin(), fmt, args.as_va_list())
}

#[no_mangle]
pub unsafe extern "C" fn fscanf(stream: *mut FILE, fmt: *const u8, mut args: ...) -> i32 {
    vfscanf_impl(stream, fmt, args.as_va_list())
}

#[no_mangle]
pub unsafe extern "C" fn sscanf(s: *const u8, fmt: *const u8, mut args: ...) -> i32 {
    vsscanf_impl(s, fmt, args.as_va_list())
}

#[no_mangle]
pub unsafe extern "C" fn vfscanf(stream: *mut FILE, fmt: *const u8, mut args: VaListImpl) -> i32 {
    vfscanf_impl(stream, fmt, args.as_va_list())
}

#[no_mangle]
pub unsafe extern "C" fn vsscanf(s: *const u8, fmt: *const u8, mut args: VaListImpl) -> i32 {
    vsscanf_impl(s, fmt, args.as_va_list())
}

unsafe fn vfscanf_impl(stream: *mut FILE, fmt: *const u8, mut args: core::ffi::VaList<'_, '_>) -> i32 {
    if stream.is_null() || fmt.is_null() { return -1; }
    let mut buf = [0u8; 1024];
    let n = crate::libc::unistd::read((*stream).fd, buf.as_mut_ptr(), buf.len());
    if n <= 0 { return -1; }
    parse_input(&buf[..n as usize], fmt, &mut args)
}

unsafe fn vsscanf_impl(s: *const u8, fmt: *const u8, mut args: core::ffi::VaList<'_, '_>) -> i32 {
    if s.is_null() || fmt.is_null() { return -1; }
    let mut len = 0;
    while ptr::read(s.add(len)) != 0 && len < 4096 { len += 1; }
    let slice = core::slice::from_raw_parts(s, len);
    parse_input(slice, fmt, &mut args)
}

unsafe fn parse_input(input: &[u8], fmt: *const u8, args: &mut core::ffi::VaList<'_, '_>) -> i32 {
    let mut count = 0i32;
    let mut ipos = 0usize;
    let mut fpos = 0usize;
