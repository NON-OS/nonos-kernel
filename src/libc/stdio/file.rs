...
static mut STDIN_FILE: FILE = FILE { fd: 0, flags: _IOREAD | _IOLBF, buf: ptr::null_mut(), buf_size: 0, buf_pos: 0, buf_len: 0, error: 0, eof: false };
static mut STDOUT_FILE: FILE = FILE { fd: 1, flags: _IOWRITE | _IOLBF, buf: ptr::null_mut(), buf_size: 0, buf_pos: 0, buf_len: 0, error: 0, eof: false };
static mut STDERR_FILE: FILE = FILE { fd: 2, flags: _IOWRITE | _IONBF, buf: ptr::null_mut(), buf_size: 0, buf_pos: 0, buf_len: 0, error: 0, eof: false };

#[no_mangle]
pub unsafe extern "C" fn stdin() -> *mut FILE { &raw mut STDIN_FILE }
#[no_mangle]
pub unsafe extern "C" fn stdout() -> *mut FILE { &raw mut STDOUT_FILE }
#[no_mangle]
pub unsafe extern "C" fn stderr() -> *mut FILE { &raw mut STDERR_FILE }
...
pub unsafe extern "C" fn fflush(stream: *mut FILE) -> i32 {
    ...
    if (f.flags & _IOWRITE) != 0 && f.buf_pos > 0 && !f.buf.is_null() {
        ...
        if written < 0 { f.error = 1; return EOF; }
        f.buf_pos = 0;
    }
    0
}
...
pub unsafe extern "C" fn setvbuf(stream: *mut FILE, buf: *mut u8, mode: i32, size: usize) -> i32 {
    ...
    f.flags |= match mode { 0 => _IOFBF, 1 => _IOLBF, 2 => _IONBF, _ => return -1 };
    if !buf.is_null() { f.buf = buf; f.buf_size = size; }
    0
}
