...
pub const CLOCK_REALTIME: i32 = 0; pub const CLOCK_MONOTONIC: i32 = 1;
pub const CLOCK_PROCESS_CPUTIME_ID: i32 = 2; pub const CLOCK_THREAD_CPUTIME_ID: i32 = 3;

#[repr(C)] #[derive(Clone, Copy, Default)] pub struct Timespec { pub tv_sec: i64, pub tv_nsec: i64 }
#[repr(C)] #[derive(Clone, Copy, Default)] pub struct Timeval { pub tv_sec: i64, pub tv_usec: i64 }
#[repr(C)] #[derive(Clone, Copy, Default)] pub struct Tm {
    pub tm_sec: i32, pub tm_min: i32, pub tm_hour: i32, pub tm_mday: i32,
    pub tm_mon: i32, pub tm_year: i32, pub tm_wday: i32, pub tm_yday: i32, pub tm_isdst: i32,
}
...
pub unsafe extern "C" fn time(tloc: *mut i64) -> i64 {
    ...
    if !tloc.is_null() { ptr::write(tloc, ts.tv_sec); }
    ts.tv_sec
}
...
pub unsafe extern "C" fn gmtime(timep: *const i64) -> *mut Tm {
    ...
    while days < 0 { year -= 1; days += if is_leap(year) { 366 } else { 365 }; }
    TM_BUFFER.tm_year = year - 1900;
    TM_BUFFER.tm_yday = days;
    let leap = is_leap(year);
    let mut mon = 0;
    while mon < 12 { let d = if mon == 1 && leap { 29 } else { DAYS_PER_MONTH[mon] }; if days < d { break; } days -= d; mon += 1; }
...
#[no_mangle]
pub unsafe extern "C" fn localtime(timep: *const i64) -> *mut Tm { gmtime(timep) }
...
pub unsafe extern "C" fn mktime(tm: *mut Tm) -> i64 {
    ...
    for m in 0..t.tm_mon { days += if m == 1 && is_leap(y) { 29 } else { DAYS_PER_MONTH[m as usize] }; }
    days += t.tm_mday - 1;
...
pub unsafe extern "C" fn strftime(s: *mut u8, max: usize, fmt: *const u8, tm: *const Tm) -> usize {
    if s.is_null() || max == 0 || fmt.is_null() || tm.is_null() { return 0; }
    let t = &*tm;
    let mut pos = 0;
    let mut i = 0;
    while ptr::read(fmt.add(i)) != 0 && pos < max - 1 {
        ...
        if c == b'%' {
            ...
            match ptr::read(fmt.add(i)) {
                b'Y' => { let n = write_num(&mut *ptr::slice_from_raw_parts_mut(s.add(pos), max - pos), t.tm_year + 1900, 4); pos += n; }
                b'm' => { let n = write_num(&mut *ptr::slice_from_raw_parts_mut(s.add(pos), max - pos), t.tm_mon + 1, 2); pos += n; }
                b'd' => { let n = write_num(&mut *ptr::slice_from_raw_parts_mut(s.add(pos), max - pos), t.tm_mday, 2); pos += n; }
                b'H' => { let n = write_num(&mut *ptr::slice_from_raw_parts_mut(s.add(pos), max - pos), t.tm_hour, 2); pos += n; }
                b'M' => { let n = write_num(&mut *ptr::slice_from_raw_parts_mut(s.add(pos), max - pos), t.tm_min, 2); pos += n; }
                b'S' => { let n = write_num(&mut *ptr::slice_from_raw_parts_mut(s.add(pos), max - pos), t.tm_sec, 2); pos += n; }
                _ => {}
            }
        } else { ptr::write(s.add(pos), c); pos += 1; }
        i += 1;
    }
