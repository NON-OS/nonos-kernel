...
pub unsafe extern "C" fn memset(dest: *mut u8, c: i32, n: usize) -> *mut u8 {
    if n == 0 || dest.is_null() { return dest; }
    let b = c as u8;
    let d = dest as usize;
    if d % 8 == 0 && n >= 8 {
        ...
        for i in 0..qwords { ptr::write(dp.add(i), pattern); }
        let rem = n % 8;
        let off = qwords * 8;
        for i in 0..rem { ptr::write(dest.add(off + i), b); }
    } else {
        for i in 0..n { ptr::write(dest.add(i), b); }
    }
    dest
}
