...
fn validate_params(threshold: usize, signers: &[u64]) -> Result<(), MultiSigError> {
    if threshold == 0 { return Err(MultiSigError::ZeroThreshold); }
    if signers.is_empty() { return Err(MultiSigError::NoSigners); }
...
pub fn create_multisig_token(
    owner: u64, perms: &[Capability], threshold: usize, authorized_signers: &[u64], ttl_ms: Option<u64>,
    ...
    Ok(MultiSigToken {
        owner_module: owner, permissions: perms.to_vec(), expires_at_ms: expiry,
        nonce: default_nonce(), threshold, authorized_signers: authorized_signers.to_vec(),
        signatures: Vec::with_capacity(threshold),
    })
}

pub fn create_multisig_token_with_nonce(
    owner: u64, perms: &[Capability], threshold: usize, authorized_signers: &[u64], ttl_ms: Option<u64>, nonce: u64,
) -> Result<MultiSigToken, MultiSigError> {
    validate_params(threshold, authorized_signers)?;
    let expiry = ttl_ms.map(|t| crate::time::timestamp_millis().saturating_add(t));
    Ok(MultiSigToken {
        owner_module: owner, permissions: perms.to_vec(), expires_at_ms: expiry,
        nonce, threshold, authorized_signers: authorized_signers.to_vec(),
        signatures: Vec::with_capacity(threshold),
    })
}
