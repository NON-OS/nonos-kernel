...
pub(crate) fn test_resource_quota_display() -> TestResult {
    ...
    if !display.contains("1000B") {
        return TestResult::Fail;
    }
    if !display.contains("50ops") {
        return TestResult::Fail;
    }
    TestResult::Pass
}
...
pub(crate) fn test_resource_quota_display_with_expiry() -> TestResult {
    ...
    let display = alloc::format!("{}", q);
    if !display.contains("exp:5000ms") {
        return TestResult::Fail;
    }
    TestResult::Pass
}
...
pub(crate) fn test_resource_error_insufficient_bytes_as_str() -> TestResult {
    ...
    if err.as_str() != "Insufficient bytes" {
        return TestResult::Fail;
    }
    TestResult::Pass
}
...
pub(crate) fn test_resource_error_insufficient_ops_as_str() -> TestResult {
    ...
    if err.as_str() != "Insufficient operations" {
        return TestResult::Fail;
    }
    TestResult::Pass
}
...
pub(crate) fn test_resource_error_zero_quota_as_str() -> TestResult {
    ...
    if err.as_str() != "Zero quota not allowed" {
        return TestResult::Fail;
    }
    TestResult::Pass
}
...
pub(crate) fn test_resource_error_is_quota_error_false() -> TestResult {
    if ResourceError::TokenExpired.is_quota_error() {
        return TestResult::Fail;
    }
    if ResourceError::ZeroQuota.is_quota_error() {
        return TestResult::Fail;
    }
    if ResourceError::InvalidSignature.is_quota_error() {
        return TestResult::Fail;
    }
    TestResult::Pass
}
...
pub(crate) fn test_resource_error_display_missing_key() -> TestResult {
    ...
    if !display.contains("Signing key") {
        return TestResult::Fail;
    }
    TestResult::Pass
}
...
pub(crate) fn test_resource_error_equality() -> TestResult {
    if ResourceError::ZeroQuota != ResourceError::ZeroQuota {
        return TestResult::Fail;
    }
    if ResourceError::ZeroQuota == ResourceError::TokenExpired {
        return TestResult::Fail;
    }
...
pub(crate) fn test_create_resource_token_zero_quota() -> TestResult {
    ...
    if !matches!(result, Err(ResourceError::ZeroQuota)) {
        return TestResult::Fail;
    }
    TestResult::Pass
}
