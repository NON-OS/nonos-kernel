...
pub(super) fn list_agents() {
    ...
    for (id, name) in agents {
        ...
        let mut buf = [0u8; 64]; let n = fmt_agent_line(id, &name[..len], &mut buf);
...
fn fmt_u32(mut n: u32, buf: &mut [u8]) -> usize {
    ...
    let mut i = 0; let mut tmp = [0u8; 10];
...
pub(super) fn create_agent(name: &str) {
    ...
    let mut buf = [0u8; 32]; let i = fmt_u32(id, &mut buf);
...
pub(super) fn create_preset(name: &str) {
    for (pname, factory) in presets::list_presets() {
        if ci_contains(pname, name.as_bytes()) {
            ...
            let mut buf = [0u8; 32]; let i = fmt_u32(id, &mut buf);
            print_line(b"Created agent from preset", COLOR_GREEN); print_line(&buf[..i], COLOR_WHITE);
            return;
        }
    }
...
fn ci_contains(hay: &[u8], needle: &[u8]) -> bool {
    if needle.is_empty() || hay.len() < needle.len() { return needle.is_empty(); }
