...
pub fn cmd_push(args: &[&str], cwd: &str) -> String {
    ...
    let branch = if args.len() > 1 { args[1] }
    else { current.as_deref().unwrap_or("main") };
    ...
    let head = match refs::head_commit(cwd) {
        Some(h) => h, None => return String::from("error: no commits to push"),
    };
    format!("To {}\n   {}..{} {} -> {}/{}\nPush successful", url, &head[..7], &head[..7], branch, remote, branch)
}
