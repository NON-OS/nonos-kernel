...
pub fn store_blob(repo: &str, data: &[u8]) -> Result<String, &'static str> {
...
pub fn read_object(repo: &str, hash: &str) -> Result<Vec<u8>, &'static str> {
    ramfs::read_file(&object_path(repo, hash)).map_err(|_| "object not found")
}

pub fn store_tree(repo: &str, entries: &[(String, String)]) -> Result<String, &'static str> {
    let content: String = entries.iter().map(|(n, h)| format!("{} {}\n", h, n)).collect();
    store_blob(repo, content.as_bytes())
}

pub fn store_commit(repo: &str, tree: &str, parent: Option<&str>, msg: &str) -> Result<String, &'static str> {
    let p = parent.map_or(String::new(), |p| format!("parent {}\n", p));
    store_blob(repo, format!("tree {}\n{}message {}\n", tree, p, msg).as_bytes())
}

pub fn parse_commit(data: &[u8]) -> Option<(String, Option<String>, String)> {
