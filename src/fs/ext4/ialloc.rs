...
use super::inode::{Ext4Inode, write_inode};
            ...
            if bitmap[byte_idx] != 0xFF {
                for bit in 0..8 {
                    if (bitmap[byte_idx] & (1 << bit)) == 0 {
                        ...
                        if is_dir { gd.bg_used_dirs_count_lo += 1; }
                        write_group_desc(&mount.device, sb, group, &gd)?;
                        let ino = group * sb.s_inodes_per_group + byte_idx as u32 * 8 + bit as u32 + 1;
                        return Ok(ino);
                    }
                }
            }
...
pub fn init_inode(mount: &Arc<Ext4MountInfo>, ino: u32, mode: u16, uid: u32, gid: u32) -> Result<(), i32> {
    let now = crate::sys::clock::unix_timestamp() as u32;
    let inode = Ext4Inode {
        i_mode: mode, i_uid: uid as u16, i_size_lo: 0, i_atime: now, i_ctime: now, i_mtime: now,
        i_dtime: 0, i_gid: gid as u16, i_links_count: 1, i_blocks_lo: 0, i_flags: 0x80000,
        i_osd1: 0, i_block: [0; 15], i_generation: 0, i_file_acl_lo: 0, i_size_high: 0,
        i_obso_faddr: 0, i_osd2: [0; 12], i_extra_isize: 32, i_checksum_hi: 0,
        i_ctime_extra: 0, i_mtime_extra: 0, i_atime_extra: 0, i_crtime: now, i_crtime_extra: 0,
