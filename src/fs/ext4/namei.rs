...
pub fn ext4_mkdir(mount: &Arc<Ext4MountInfo>, parent_ino: u32, name: &str, mode: u16) -> Result<u32, i32> {
    let parent = read_inode(&mount.device, &mount.sb, parent_ino)?;
    if !parent.is_dir() { return Err(-20); }
    if dir_lookup(&mount.device, &mount.sb, &parent, name).is_ok() { return Err(-17); }
...
pub fn ext4_unlink(mount: &Arc<Ext4MountInfo>, parent_ino: u32, name: &str) -> Result<(), i32> {
    ...
    if inode.is_dir() { return Err(-21); }
    dir_remove_entry(&mount.device, &mount.sb, parent_ino, name)?;
    inode.i_links_count -= 1;
    if inode.i_links_count == 0 { free_inode(mount, ino)?; }
    else { write_inode(&mount.device, &mount.sb, ino, &inode)?; }
    Ok(())
}
