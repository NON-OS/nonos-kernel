...
pub fn ext4_read_data(mount: &Arc<Ext4MountInfo>, ino: u32, buf: &mut [u8], offset: u64) -> Result<usize, i32> {
    ...
    while pos < end {
        ...
        buf[bytes_read..bytes_read + bytes_in_block].copy_from_slice(&block_buf[block_offset..block_offset + bytes_in_block]);
...
pub fn ext4_read_symlink(mount: &Arc<Ext4MountInfo>, ino: u32) -> Result<alloc::string::String, i32> {
    let inode = read_inode(&mount.device, &mount.sb, ino)?;
    if !inode.is_symlink() { return Err(-22); }
    let size = inode.size() as usize;
    if size <= 60 {
        let bytes = unsafe { core::slice::from_raw_parts(inode.i_block.as_ptr() as *const u8, size) };
