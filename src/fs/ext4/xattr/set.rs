...
*/
pub fn ext4_setxattr(mount: &Arc<Ext4MountInfo>, ino: u32, name: &str, value: &[u8], flags: u32) -> Result<(), i32> {
    ...
    let xattr_block = if inode.i_file_acl_lo == 0 {
        ...
        unsafe { core::ptr::write(buf.as_mut_ptr() as *mut Ext4XattrHeader, hdr); }
...
fn scan_entries(buf: &[u8], block_size: usize, index: u8, attr_name: &str) -> Result<(usize, Option<usize>, usize), i32> {
...
fn write_entry(buf: &mut [u8], entry_offset: usize, value_offset: usize, index: u8, attr_name: &str, value: &[u8]) {
    ...
    unsafe {
        ...
        core::ptr::copy_nonoverlapping(attr_name.as_ptr(), buf.as_mut_ptr().add(entry_offset + 16), attr_name.len());
        core::ptr::copy_nonoverlapping(value.as_ptr(), buf.as_mut_ptr().add(value_offset), value.len());
    }
}
