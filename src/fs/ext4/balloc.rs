...
fn truncate_extents(mount: &Arc<Ext4MountInfo>, inode: &mut Ext4Inode, new_blocks: u32) -> Result<(), i32> {
...
fn truncate_indirect(mount: &Arc<Ext4MountInfo>, inode: &mut Ext4Inode, new_blocks: u64) -> Result<(), i32> {
    ...
    if new_blocks < 12 + ptrs_per_block as u64 + (ptrs_per_block * ptrs_per_block) as u64 && inode.i_block[14] != 0 {
    ...
    for i in 0..ptrs_per_block {
        ...
        if ptr != 0 {
            ...
            for j in 0..ptrs_per_block {
                let dptr = u32::from_le_bytes([ibuf[j*4], ibuf[j*4+1], ibuf[j*4+2], ibuf[j*4+3]]);
                if dptr != 0 {
