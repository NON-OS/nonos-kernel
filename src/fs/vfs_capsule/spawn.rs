...
pub fn spawn_vfs_capsule() -> Result<(), SpawnError> {
    ...
    let entry = load_elf_into_capsule_as(VFS_ELF, pid)?;

    // Capsule's own caps. CAP_VFS is the user-facing gate, not the
    // capsule's bit; the capsule itself needs IPC + Memory.
    let caps_bits = Capability::IPC.bit() | Capability::Memory.bit();
...
fn load_elf_into_capsule_as(elf: &'static [u8], pid: u32) -> Result<u64, SpawnError> {
    ...
    let load = load_elf_executable(elf).map_err(|err| {
        crate::sys::serial::println(b"[VFS-DEBUG] load_elf_executable error:");
