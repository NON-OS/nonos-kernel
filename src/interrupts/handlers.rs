...
use crate::interrupts::{pic, INTERRUPT_STATS, KEYBOARD_INTERRUPT_ID, TIMER_INTERRUPT_ID};
use alloc::format;
...
pub extern "x86-interrupt" fn divide_by_zero_handler(stack_frame: InterruptStackFrame) {
    ...
    panic!("EXCEPTION: Divide by zero at RIP: 0x{:016x}", stack_frame.instruction_pointer.as_u64());
}
...
pub extern "x86-interrupt" fn debug_handler(stack_frame: InterruptStackFrame) {
    ...
    crate::log::logger::log_critical(&format!(
        "DEBUG EXCEPTION at RIP: 0x{:016x}",
...
pub extern "x86-interrupt" fn nmi_handler(stack_frame: InterruptStackFrame) {
    ...
    panic!(
        "CRITICAL: Non-maskable interrupt at RIP: 0x{:016x}",
        stack_frame.instruction_pointer.as_u64()
    );
}
...
pub extern "x86-interrupt" fn overflow_handler(stack_frame: InterruptStackFrame) {
    ...
    panic!("EXCEPTION: Overflow at RIP: 0x{:016x}", stack_frame.instruction_pointer.as_u64());
}
...
pub extern "x86-interrupt" fn bound_range_exceeded_handler(stack_frame: InterruptStackFrame) {
    ...
    panic!(
        "EXCEPTION: Bound range exceeded at RIP: 0x{:016x}",
        stack_frame.instruction_pointer.as_u64()
    );
}
...
pub extern "x86-interrupt" fn invalid_opcode_handler(stack_frame: InterruptStackFrame) {
    ...
    panic!("EXCEPTION: Invalid opcode at RIP: 0x{:016x}", stack_frame.instruction_pointer.as_u64());
}
...
pub extern "x86-interrupt" fn device_not_available_handler(stack_frame: InterruptStackFrame) {
    ...
    panic!(
        "EXCEPTION: Device not available at RIP: 0x{:016x}",
        stack_frame.instruction_pointer.as_u64()
    );
}
...
pub extern "x86-interrupt" fn invalid_tss_handler(
    ...
    panic!(
        "EXCEPTION: Invalid TSS (error: 0x{:x}) at RIP: 0x{:016x}",
        error_code,
        stack_frame.instruction_pointer.as_u64()
    );
}
...
pub extern "x86-interrupt" fn segment_not_present_handler(
    ...
    panic!(
        "EXCEPTION: Segment not present (error: 0x{:x}) at RIP: 0x{:016x}",
        error_code,
        stack_frame.instruction_pointer.as_u64()
    );
}
...
pub extern "x86-interrupt" fn stack_segment_fault_handler(
    ...
    panic!(
        "EXCEPTION: Stack segment fault (error: 0x{:x}) at RIP: 0x{:016x}",
        error_code,
        stack_frame.instruction_pointer.as_u64()
    );
}
...
pub extern "x86-interrupt" fn general_protection_fault_handler(
    ...
    panic!(
        "EXCEPTION: General protection fault (error: 0x{:x}) at RIP: 0x{:016x}",
        error_code,
        stack_frame.instruction_pointer.as_u64()
    );
}
...
pub extern "x86-interrupt" fn page_fault_handler(
    ...
    panic!(
        "PAGE FAULT: Address 0x{:016x}, Error: {:?}, RIP: 0x{:016x}, CR3: 0x{:016x}",
...
pub extern "x86-interrupt" fn x87_floating_point_handler(stack_frame: InterruptStackFrame) {
    ...
    panic!(
        "EXCEPTION: x87 floating point at RIP: 0x{:016x}",
        stack_frame.instruction_pointer.as_u64()
    );
}
...
pub extern "x86-interrupt" fn alignment_check_handler(
    ...
    panic!(
        "EXCEPTION: Alignment check (error: 0x{:x}) at RIP: 0x{:016x}",
        error_code,
        stack_frame.instruction_pointer.as_u64()
    );
}
...
pub extern "x86-interrupt" fn simd_floating_point_handler(stack_frame: InterruptStackFrame) {
    ...
    panic!(
        "EXCEPTION: SIMD floating point at RIP: 0x{:016x}",
        stack_frame.instruction_pointer.as_u64()
    );
}
...
pub extern "x86-interrupt" fn virtualization_handler(stack_frame: InterruptStackFrame) {
    ...
    panic!("EXCEPTION: Virtualization at RIP: 0x{:016x}", stack_frame.instruction_pointer.as_u64());
}
