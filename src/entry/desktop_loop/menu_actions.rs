...
pub fn handle_context_menu_action(action: u8) {
    match action {
        DESKTOP_REFRESH => { desktop::refresh_desktop_icons(); set_needs_redraw(); }
        DESKTOP_SETTINGS => { window::open(WindowType::Settings); }
        DESKTOP_ABOUT => { window::open(WindowType::About); }
        DESKTOP_NEW_FOLDER => {
        ...
        DESKTOP_DELETE => { desktop::delete_desktop_selected(); set_needs_redraw(); }
        DESKTOP_GO_BACK => { desktop::desktop_navigate_back(); set_needs_redraw(); }
        FM_OPEN => { window::fm_open_selected(); }
        FM_COPY => { let _ = window::fm_copy_selected(); }
        ...
        EDIT_CUT => { window::text_editor::editor_cut(); }
        EDIT_COPY => { window::text_editor::editor_copy(); }
        EDIT_PASTE => { window::text_editor::editor_paste(); }
        EDIT_SELECT_ALL => { window::text_editor::editor_select_all(); }
        WIN_MINIMIZE => {
            let focused = window::FOCUSED_WINDOW.load(Ordering::Relaxed);
            if focused < window::MAX_WINDOWS { window::minimize(focused); }
        }
        WIN_MAXIMIZE => {
            let focused = window::FOCUSED_WINDOW.load(Ordering::Relaxed);
            if focused < window::MAX_WINDOWS { window::maximize(focused); }
        }
        WIN_CLOSE => {
            let focused = window::FOCUSED_WINDOW.load(Ordering::Relaxed);
            if focused < window::MAX_WINDOWS { window::close(focused); }
        }
        _ => {}
    }
    set_needs_redraw();
