...
pub fn test_log_manager_log_multiple() -> TestResult {
    ...
    if manager.entry_count() != 10 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_log_manager_log_all_severities() -> TestResult {
    ...
    if manager.entry_count() != 5 { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_log_manager_get_recent_returns_newest() -> TestResult {
    ...
    if recent[0].msg.as_str() != "second" { return TestResult::Fail; }
    if recent[1].msg.as_str() != "third" { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_log_manager_entry_count_increments() -> TestResult {
    ...
    for i in 1..=5 {
        ...
        if manager.entry_count() != i { return TestResult::Fail; }
    }
    TestResult::Pass
}
...
pub fn test_log_manager_clear_then_log() -> TestResult {
    ...
    if manager.entry_count() != 1 { return TestResult::Fail; }
    let entries = manager.get_entries();
    if entries[0].msg.as_str() != "after" { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_log_manager_hash_chain() -> TestResult {
    ...
    manager.log(Severity::Info, "second");
    let entries = manager.get_entries();
    if entries[0].hash == entries[1].hash { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_log_manager_hash_not_zero() -> TestResult {
    ...
    let entries = manager.get_entries();
    if entries[0].hash == [0u8; 32] { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_log_manager_empty_message() -> TestResult {
    ...
    let entries = manager.get_entries();
    if entries[0].msg.as_str() != "" { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_log_manager_long_message() -> TestResult {
    ...
    let entries = manager.get_entries();
    if !(entries[0].msg.len() > 0) { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_log_manager_message_truncation() -> TestResult {
    ...
    let entries = manager.get_entries();
    if !(entries[0].msg.len() <= 256) { return TestResult::Fail; }
    TestResult::Pass
}
    ...
    if let Some(ref mgr) = *lock {
        ...
        if !entries.is_empty() {
            ...
            if last.msg.as_str() == "critical test" {
                if last.sev != Severity::Fatal { return TestResult::Fail; }
            }
        }
    }
...
pub fn test_get_log_entries_function() -> TestResult {
    ...
    if !(!entries.is_empty() || entries.is_empty()) { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_get_recent_logs_function() -> TestResult {
    ...
    if !(recent.len() <= 10) { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_log_function_with_severity() -> TestResult {
    ...
    let count = log_entry_count();
    if !(count >= 5) { return TestResult::Fail; }
...
pub fn test_log_manager_timestamps_increase() -> TestResult {
    ...
    let entries = manager.get_entries();
    if entries.len() >= 2 {
        if !(entries[1].ts >= entries[0].ts) { return TestResult::Fail; }
    }
    TestResult::Pass
}
...
pub fn test_log_manager_hash_deterministic() -> TestResult {
    ...
    if entries1[0].hash != entries2[0].hash { return TestResult::Fail; }
    TestResult::Pass
}
...
pub fn test_log_manager_different_messages_different_hashes() -> TestResult {
    ...
    if entries_a[0].hash == entries_b[0].hash { return TestResult::Fail; }
    TestResult::Pass
}
