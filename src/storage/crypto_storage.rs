...
impl CryptoStorageDevice {
    ...
    pub fn new(block_device: Arc<BlockDevice>, config: CryptoConfig, device_name: String) -> Self {
    ...
    pub fn unlock(&self, passphrase: &[u8]) -> Result<(), &'static str> {
        ...
        let crypto_state =
            CryptoState { master_key, data_key, integrity_key, salt, key_generation: 1, tweak_key };
        ...
        *self.crypto_state.write() = Some(crypto_state);
        ...
        crate::log_info!("Unlocked encrypted storage device: {}", self.device_name);
    ...
    pub fn lock(&self) {
        ...
        crate::log_info!("Locked encrypted storage device: {}", self.device_name);
    }

    /// Read encrypted sector
    pub fn read_encrypted_sector(
        &self,
        sector_num: u64,
        buffer: &mut [u8],
    ) -> Result<(), IoStatus> {
        ...
        self.block_device
            .read_block(sector_num, &mut encrypted_buffer)
    ...
    pub fn write_encrypted_sector(&self, sector_num: u64, data: &[u8]) -> Result<(), IoStatus> {
        ...
        self.block_device
            .write_block(sector_num, &encrypted_buffer)
            .map_err(|_| IoStatus::DeviceError)?;
    ...
    fn derive_key(&self, passphrase: &[u8], salt: &[u8]) -> Result<[u8; 64], &'static str> {
        ...
        match self.config.kdf {
            KeyDerivationFunction::Argon2id => {
                ...
                for i in 0..64 {
                    key[i] = passphrase[i % passphrase.len()] ^ salt[i % salt.len()] ^ (i as u8);
                }
            }
    ...
    fn derive_integrity_key(&self, master_key: &[u8; 64]) -> Result<[u8; 32], &'static str> {
        ...
        for i in 0..32 {
            integrity_key[i] =
                master_key[i + 16] ^ master_key[(i + 32) % 64] ^ context[i % context.len()];
        }
    ...
    fn derive_tweak_key(&self, master_key: &[u8; 64]) -> Result<[u8; 32], &'static str> {
        ...
        for i in 0..32 {
            tweak_key[i] =
                master_key[(i + 8) % 64] ^ master_key[(i + 40) % 64] ^ context[i % context.len()];
        }
    ...
    pub fn secure_delete_sector(&self, sector_num: u64) -> Result<(), IoStatus> {
        ...
        for pass in 0..3 {
            ...
            self.block_device
                .write_block(sector_num, &random_data)
                .map_err(|_| IoStatus::DeviceError)?;
        }
    ...
    fn generate_random_data(&self, buffer: &mut [u8]) {
        ...
        for (i, byte) in buffer.iter_mut().enumerate() {
            ...
            rng_state[i % 32] =
                rng_state[i % 32].wrapping_mul(7).wrapping_add(13) ^ (entropy as u8);
            *byte = rng_state[i % 32];
        }
    }
...
impl CryptoStorageManager {
    pub const fn new() -> Self {
        CryptoStorageManager { devices: RwLock::new(Vec::new()), next_device_id: AtomicU32::new(0) }
    }
    ...
    pub fn create_encrypted_device(
        ...
        crate::log_info!("Created encrypted storage device ID {}", device_id);
