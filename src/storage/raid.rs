...
impl RaidArray {
    ...
    fn raid5_write(&self, lba: u64, data: &[u8]) -> Result<(), IoStatus> {
        ...
        let stripe_blocks = (self.config.stripe_size * 1024) / self.block_size;
        let stripe_num = lba / stripe_blocks as u64;
        let parity_device = (self.devices.len() - 1 - (stripe_num % self.devices.len() as u64) as usize) % self.devices.len();
        let data_devices = self.devices.len() - 1;
        let device_idx = (stripe_num % data_devices as u64) as usize;
        
        // Adjust device index if it would conflict with parity device
        let actual_device_idx = if device_idx >= parity_device { device_idx + 1 } else { device_idx };
    ...
    /// RAID 6 read/write implementations would be similar but with double parity
    ...
    pub fn start_rebuild(&mut self, failed_device: usize, replacement: Arc<BlockDevice>) -> Result<(), &'static str> {
        ...
        let rebuild_task = async move {
            ...
            for stripe_idx in 0..self.stripe_count.load(Ordering::Relaxed) {
                ...
                match self.config.level {
                    ...
                    _ => {
                        crate::log_warn!("Rebuild not supported for RAID level {:?}", self.config.level);
                    }
                }
        ...
        crate::sched::spawn_task("raid_rebuild", || { crate::log_info!("RAID rebuild task"); }, 50);
        
        crate::log_info!(
            "Started rebuild for RAID array device {}",
            failed_device
        );
    ...
    pub fn get_stats(&self) -> RaidArrayStats {
        RaidArrayStats {
            ...
            healthy_devices: self.device_health.iter().filter(|&&h| h == DeviceHealth::Healthy).count(),
