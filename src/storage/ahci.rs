...
use alloc::{vec::Vec, sync::Arc, string::{String, ToString}};
use core::sync::atomic::{AtomicU32, AtomicBool, Ordering};
use spin::{Mutex, RwLock};
use x86_64::VirtAddr;

use super::{StorageDevice, DeviceInfo, StorageType, DeviceCapabilities, IoRequest, IoResult, IoStatus, DeviceStatistics, SmartData, PowerState};
...
impl AhciDevice {
    ...
    pub unsafe fn new(
        ...
        let cmd_list = crate::memory::alloc_dma_coherent(1024).ok_or("Failed to allocate DMA memory")? as *mut [AhciCmdHeader; 32];
        let cmd_tables = crate::memory::alloc_dma_coherent(32 * 256 * 1024).ok_or("Failed to allocate DMA memory")? as *mut [AhciCmdTable; 32];
        let fis_base = crate::memory::alloc_dma_coherent(256).ok_or("Failed to allocate DMA memory")? as *mut [u8; 256];
        ...
        let features = DeviceCapabilities::READ | DeviceCapabilities::WRITE | 
                      DeviceCapabilities::NCQ | DeviceCapabilities::SMART |
                      DeviceCapabilities::TRIM | DeviceCapabilities::SECURE_ERASE;
    ...
    unsafe fn execute_ata_command(&self,
        slot: u8,
        lba: u64,
        sectors: u16,
        buffer: VirtAddr,
        ...
        while (port.ci & (1u32 << slot)) != 0 {
            if crate::time::current_ticks() - start_time > 5000000 { // 5 second timeout
        ...
        // Check for errors
        if (port.is & 0x40000000) != 0 { // Task File Error
...
impl StorageDevice for AhciDevice {
    ...
    fn submit_request(&self, request: IoRequest) -> Result<(), IoStatus> {
        ...
        unsafe {
            let result = match request.operation {
                ...
                super::IoOperation::SecureErase => {
                    self.secure_erase().map(|_| IoResult {
                        status: IoStatus::Success,
                        bytes_transferred: 0,
                        error_code: 0,
                        completion_time: crate::time::current_ticks(),
    ...
    fn supports_secure_erase(&self) -> bool {
        unsafe {
            ...
            let identify_data = core::slice::from_raw_parts(
                identify_buffer.virt_addr().as_ptr::<u8>(), 512
            );
    ...
    fn verify_sanitize_completion(&self) -> Result<(), &'static str> {
        unsafe {
            ...
            if (status & 0x01) != 0 {
                crate::log::logger::log_err!("Sanitize error: status=0x{:02x}, error=0x{:02x}", 
                    status, error);
            ...
            loop {
                ...
                if (ci & slot_mask) == 0 {
                    ...
                    if (is_reg & 0x40000000) != 0 {
                        ...
                        crate::log::logger::log_err!("Command {} error: status=0x{:02x}, error=0x{:02x}", 
                    ...
                    if (is_reg & 0x20000000) != 0 { // Host Bus Data Error
                        core::ptr::write_volatile(&mut port.is, is_reg);
                        return Err("Host bus data error");
                    }
                    
                    if (is_reg & 0x10000000) != 0 { // Host Bus Fatal Error
                        core::ptr::write_volatile(&mut port.is, is_reg);
                ...
                let is_reg = core::ptr::read_volatile(&port.is);
                if (is_reg & 0x78000000) != 0 { // Any error bits set
                    core::ptr::write_volatile(&mut port.is, is_reg);
    ...
    fn read_blocks(&self, start_block: u64, block_count: u32, buffer: &mut [u8]) -> Result<(), super::IoStatus> {
        if !self.ready.load(Ordering::Acquire) {
                    ...
                    match AhciDevice::new(self.hba_base, port_num as u8, signature) {
                        Ok(device) => {
                            crate::log_info!(
                                ...
                                port_num, device.info.vendor, device.info.model
                            );
...
pub fn scan_and_register_ahci_devices(storage_manager: &super::StorageManager) -> Result<(), &'static str> {
