...
impl SwapDevice for FileSwapDevice {
    ...
    fn statistics(&self) -> SwapDeviceStats {
        ...
        SwapDeviceStats {
            ...
            average_read_latency: AtomicU64::new(
                self.stats.average_read_latency.load(Ordering::Relaxed),
            ),
            average_write_latency: AtomicU64::new(
                self.stats.average_write_latency.load(Ordering::Relaxed),
            ),
...
impl SwapManager {
    ...
    pub fn swap_out_page(
        ...
        match device.write_page(slot, page_data) {
            Ok(()) => {
                ...
                Ok(SwapSlot { device_id: device.device_info().device_id, slot })
            }
            Err(e) => {
    ...
    pub fn free_swap_slot(&self, swap_slot: SwapSlot) {
        ...
        if let Some(device) =
            devices.iter().find(|d| d.device_info().device_id == swap_slot.device_id)
        {
    ...
    pub fn get_global_stats(&self) -> SwapGlobalStats {
        SwapGlobalStats {
            ...
            total_swap_out: AtomicU64::new(
                self.global_stats.total_swap_out.load(Ordering::Relaxed),
            ),
            swap_in_pages: AtomicU64::new(self.global_stats.swap_in_pages.load(Ordering::Relaxed)),
            swap_out_pages: AtomicU64::new(
                self.global_stats.swap_out_pages.load(Ordering::Relaxed),
            ),
            swap_errors: AtomicU64::new(self.global_stats.swap_errors.load(Ordering::Relaxed)),
            compression_savings: AtomicU64::new(
                self.global_stats.compression_savings.load(Ordering::Relaxed),
            ),
        }
    }
}
...
pub fn init() -> Result<(), SwapError> {
    ...
    let default_swap = Arc::new(FileSwapDevice::new(
        0,
...
pub fn create_swap_device(
    ...
    let encryption_key = if encrypted { Some(generate_swap_key()) } else { None };

    let swap_device =
        Arc::new(FileSwapDevice::new(device_id, file_path, size_mb, compression, encryption_key)?);
