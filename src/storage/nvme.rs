...
use alloc::{vec::Vec, boxed::Box, string::String, sync::Arc};
...
impl NvmeController {
    ...
    fn initialize_io_queues(&self) -> Result<(), &'static str> {
        ...
        for i in 1..=total_queues {
            ...
            let command_id = self.admin_queue.submit_command(
    ...
    fn identify_controller(&self) -> Result<(), &'static str> {
        ...
        unsafe {
            let identify_data = core::slice::from_raw_parts(
                identify_buffer.as_u64() as *const u8, 
                4096
            );
    ...
    pub fn submit_admin_command(&self, command: &NvmeCommand) -> Result<(), &'static str> {
        ...
        let sq_addr = self.admin_queue.sq_base.as_u64() + (sq_entry as u64 * core::mem::size_of::<NvmeCommand>() as u64);
    ...
    pub fn wait_for_completion_timeout(&self, command_id: u16, timeout_ms: u32) -> Result<NvmeCompletion, &'static str> {
        ...
        loop {
            ...
            let cq_addr = self.admin_queue.cq_base.as_u64() + (cq_head as u64 * core::mem::size_of::<NvmeCompletion>() as u64);
            
            let completion = unsafe {
                core::ptr::read_volatile(cq_addr as *const NvmeCompletion)
            };
...
impl StorageDevice for NvmeController {
    ...
    fn secure_erase(&self) -> Result<(), &'static str> {
        ...
        // Set sanitize action - cryptographic erase (fastest and most secure for encrypted drives)
        ...
        self.statistics.last_secure_erase_time.store(crate::time::timestamp_millis(), Ordering::Relaxed);
    ...
    fn verify_sanitize_completion(&self) -> Result<(), &'static str> {
        ...
        if (status & 0x07) != 0x01 { // Should be "Successfully completed"
...
fn allocate_contiguous_pages(count: usize) -> Result<PhysAddr, &'static str> {
    ...
    if let Some(frame) = crate::memory::phys::alloc_contig(count, count, crate::memory::phys::AllocFlags::ZERO) {
