...
use crate::crypto::{
    hash::blake3_hash,
    sig::ed25519::{Ed25519Keypair, Ed25519Signature},
};
...
impl AttestationManager {
    pub fn new() -> Result<Self, ZKError> {
        let signing_keypair = Ed25519Keypair::generate().map_err(|_| ZKError::CryptoError)?;
    ...
    pub fn verify_attestation(attestation: &KernelAttestation) -> Result<bool, ZKError> {
        ...
        if !attestation
            .signature
            .verify(
                message.as_slice().try_into().map_err(|_| ZKError::InvalidInput)?,
    ...
    fn sign_measurement(
        ...
        self.signing_keypair.sign(&message).map_err(|_| ZKError::CryptoError)
    }
    ...
    pub fn rotate_key(&mut self) -> Result<(), ZKError> {
        self.signing_keypair = Ed25519Keypair::generate().map_err(|_| ZKError::CryptoError)?;
        Ok(())
    }
}
    ...
    pub fn verify(&self, attestation: &KernelAttestation) -> Result<bool, ZKError> {
        match self {
            AttestationPolicy::SignatureOnly => {
                ...
                Ok(attestation
                    .signature
                    .verify(&message_hash, &pub_key_array)
                    .map_err(|_| ZKError::AttestationError("Verification failed".into()))?)
            }
...
pub fn get_attestation_manager() -> Option<&'static mut AttestationManager> {
    unsafe { GLOBAL_ATTESTATION_MANAGER.as_mut() }
}
