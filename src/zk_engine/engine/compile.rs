...
impl ZKEngine {
    pub fn compile_circuit(&self, constraints: Vec<Constraint>, num_witnesses: usize) -> Result<u32, ZKError> {
        if constraints.len() > self.config.max_constraints { return Err(ZKError::InvalidCircuit); }
        if num_witnesses > self.config.max_witnesses { return Err(ZKError::InvalidWitness); }
        let circuit_id = self.next_circuit_id.fetch_add(1, Ordering::SeqCst);
        let num_constraints = constraints.len();
        let mut builder = CircuitBuilder::new();
        for constraint in constraints { builder.add_constraint(constraint)?; }
        ...
        { let mut circuits = self.circuits.write(); let mut proving_keys = self.proving_keys.write(); let mut verifying_keys = self.verifying_keys.write();
            circuits.insert(circuit_id, Box::new(circuit)); proving_keys.insert(circuit_id, proving_key); verifying_keys.insert(circuit_id, verifying_key); }
