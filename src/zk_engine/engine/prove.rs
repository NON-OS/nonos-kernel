...
use super::super::groth16::{Groth16Prover, FieldElement};
...
impl ZKEngine {
    pub fn generate_proof(&self, circuit_id: u32, witness: Vec<Vec<u8>>, public_inputs: Vec<Vec<u8>>) -> Result<ZKProof, ZKError> {
        ...
        let (circuit, proving_key) = {
            let circuits = self.circuits.read(); let proving_keys = self.proving_keys.read();
        ...
        if witness.len() != circuit.num_variables { return Err(ZKError::InvalidWitness); }
        let witness_fe: Vec<FieldElement> = witness.iter().map(|bytes| FieldElement::from_bytes(bytes.as_slice()).unwrap_or(FieldElement::zero())).collect();
        let public_inputs_fe: Vec<FieldElement> = public_inputs.iter().map(|bytes| FieldElement::from_bytes(bytes.as_slice()).unwrap_or(FieldElement::zero())).collect();
        ...
        for input in &public_inputs { hasher_input.extend_from_slice(input); }
        let proof_hash = crate::crypto::hash::blake3_hash(&hasher_input);
        let mut hash_array = [0u8; 32]; hash_array.copy_from_slice(&proof_hash[..32]);
        let proof = ZKProof { circuit_id, proof_data, public_inputs, proof_hash: hash_array, created_at: crate::time::timestamp_millis() };
