...
pub(super) fn sign_measurement(mgr: &AttestationManager, measurement: &KernelMeasurement) -> Result<Ed25519Signature, ZKError> {
    let message = measurement.to_bytes();
    Ok(crate::crypto::ed25519::sign(&mgr.signing_keypair, &message))
}

pub(super) fn generate_integrity_proof(mgr: &AttestationManager, measurement: &KernelMeasurement) -> Result<Option<Proof>, ZKError> {
    let Some(engine) = mgr.zk_engine else { return Ok(None); };
    let Some(ref _circuit) = mgr.attestation_circuit else { return Ok(None); };
    let witness = measurement.to_witness()?;
    let public_inputs = measurement.to_field_elements()?;
    let circuit_id = 1;
    let public_inputs_bytes: Vec<Vec<u8>> = public_inputs.iter().map(|fe| fe.to_bytes().to_vec()).collect();
...
pub(super) fn build_attestation_circuit() -> Result<Circuit, ZKError> {
    ...
    builder.enforce_equal(LinearCombination::from_variable(temp2), LinearCombination::from_variable(integrity_hash_var));
    Ok(builder.build(4)?)
}
