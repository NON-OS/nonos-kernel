...
impl G1Point {
    pub fn add(&self, other: &G1Point) -> G1Point {
        ...
        let z1z1 = self.z.square(); let z2z2 = other.z.square();
        let u1 = self.x.mul(&z2z2); let u2 = other.x.mul(&z1z1);
        let s1 = self.y.mul(&z2z2).mul(&other.z); let s2 = other.y.mul(&z1z1).mul(&self.z);
        if u1 == u2 { if s1 == s2 { return self.double(); } else { return G1Point::infinity(); } }
        let h = u2.sub(&u1); let i = h.double().square(); let j = h.mul(&i);
        let r = s2.sub(&s1).double(); let v = u1.mul(&i);
    ...
    pub fn double(&self) -> G1Point {
        ...
        let a = self.x.square(); let b = self.y.square(); let c = b.square();
        let d = self.x.add(&b).square().sub(&a).sub(&c).double();
        let e = a.double().add(&a); let f = e.square();
    ...
    pub fn scalar_mul(&self, scalar: &[u64; 4]) -> G1Point {
        ...
        for &naf_digit in naf.iter().rev() {
            ...
            if naf_digit > 0 { result = result.add(self); }
            else if naf_digit < 0 { result = result.add(&self.neg()); }
        }
        result
    }
}
