...
pub fn sys_zk_compile_circuit(params_ptr: usize, process: &ProcessControlBlock) -> Result<usize, &'static str> {
    ...
    let constraints_data = unsafe { slice::from_raw_parts(params.constraints_ptr, params.constraints_len) };
    let constraints = deserialize_constraints(constraints_data).map_err(|_| "Invalid constraints format")?;
    ...
    unsafe { *(params.circuit_id_ptr) = circuit_id; }
    crate::log::info!("Process {} compiled circuit {} in {}ms", process.pid, circuit_id, compilation_time);
