...
pub(crate) fn test_ipv4_address_parsing() -> TestResult {
    ...
    if addr.octets() != [192, 168, 1, 1] { return TestResult::Fail; }

    let localhost = Ipv4Addr::new(127, 0, 0, 1);
    if !localhost.is_loopback() { return TestResult::Fail; }

    let broadcast = Ipv4Addr::new(255, 255, 255, 255);
    if !broadcast.is_broadcast() { return TestResult::Fail; }
...
pub(crate) fn test_ipv4_address_format() -> TestResult {
    ...
    if !addr.is_private() { return TestResult::Fail; }

    let public = Ipv4Addr::new(8, 8, 8, 8);
    if public.is_private() { return TestResult::Fail; }
...
pub(crate) fn test_mac_address_parsing() -> TestResult {
    ...
    if mac.octets() != [0x00, 0x11, 0x22, 0x33, 0x44, 0x55] { return TestResult::Fail; }

    let broadcast = MacAddress::broadcast();
    if broadcast.octets() != [0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF] { return TestResult::Fail; }
...
pub(crate) fn test_ethernet_frame_types() -> TestResult {
    ...
    if EtherType::Ipv6 as u16 != 0x86DD { return TestResult::Fail; }
    if EtherType::Arp as u16 != 0x0806 { return TestResult::Fail; }
...
pub(crate) fn test_tcp_state_machine() -> TestResult {
    ...
    if state != TcpState::Closed { return TestResult::Fail; }

    let listen = TcpState::Listen;
    if listen == TcpState::Closed { return TestResult::Fail; }

    let established = TcpState::Established;
    if established != TcpState::Established { return TestResult::Fail; }
...
pub(crate) fn test_udp_header_construction() -> TestResult {
    ...
    if header.length() != 100 { return TestResult::Fail; }
...
pub(crate) fn test_dns_query_construction() -> TestResult {
    ...
    if query.name() != b"example.com" { return TestResult::Fail; }
    if query.query_type() != QueryType::A { return TestResult::Fail; }

    let aaaa_query = DnsQuery::new(b"test.org", QueryType::AAAA);
    if aaaa_query.query_type() != QueryType::AAAA { return TestResult::Fail; }
...
pub(crate) fn test_firewall_rule_matching() -> TestResult {
    ...
    if rule.port() != 80 { return TestResult::Fail; }
