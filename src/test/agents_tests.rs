...
pub(crate) fn test_agent_config_default() -> TestResult {
    ...
    if config.name[0] != 0 { return TestResult::Fail; }

    for enabled in config.tools_enabled.iter() {
        if *enabled { return TestResult::Fail; }
    }
...
pub(crate) fn test_agent_creation() -> TestResult {
    ...
    if agent.config.max_tokens != 8192 { return TestResult::Fail; }
    if agent.name() != b"Test" { return TestResult::Fail; }
...
pub(crate) fn test_agent_registry() -> TestResult {
    ...
    if !agents.iter().any(|a| a.id == id) { return TestResult::Fail; }

    if !delete_agent(id) { return TestResult::Fail; }

    let agent_after = get_agent(id);
    if agent_after.is_some() { return TestResult::Fail; }
...
pub(crate) fn test_agent_tools() -> TestResult {
    ...
    let registered = register_tool(b"test_tool", |_args| {
        ToolResult::Success(b"tool executed".to_vec())
    });
    if !registered { return TestResult::Fail; }

    let tools = list_tools();
    if tools.len() != initial_count + 1 { return TestResult::Fail; }

    let result = execute_tool(b"test_tool", b"");
    match result {
        ToolResult::Success(output) => {
            if output.as_slice() != b"tool executed" { return TestResult::Fail; }
        }
...
pub(crate) fn test_agent_memory() -> TestResult {
    ...
    if mem.agent_id != 9999 { return TestResult::Fail; }

    mem.store(b"key1", b"value1", 5);
    let recalled = mem.recall(b"key1");
    if recalled.is_none() { return TestResult::Fail; }
    if recalled.unwrap().as_slice() != b"value1" { return TestResult::Fail; }

    let not_found = mem.recall(b"nonexistent");
    if not_found.is_some() { return TestResult::Fail; }

    mem.store(b"key1", b"updated", 5);
    let updated = mem.recall(b"key1");
    if updated.is_none() { return TestResult::Fail; }
    if updated.unwrap().as_slice() != b"updated" { return TestResult::Fail; }
...
pub(crate) fn test_agent_context() -> TestResult {
    ...
    if path.is_none() { return TestResult::Fail; }
    if path.unwrap() != b"/bin" { return TestResult::Fail; }

    ctx.add_history(b"ls -la");
    if ctx.history.len() != 1 { return TestResult::Fail; }
...
pub(crate) fn test_agent_tasks() -> TestResult {
    ...
    if task.description.as_slice() != b"Test task" { return TestResult::Fail; }

    update_task_status(id, TaskStatus::Running, None);
    let updated = get_task(id).unwrap();
    if updated.status != TaskStatus::Running { return TestResult::Fail; }

    update_task_status(id, TaskStatus::Complete, Some(b"Done"));
    let completed = get_task(id).unwrap();
    if completed.status != TaskStatus::Complete { return TestResult::Fail; }
    if completed.result.as_slice() != b"Done" { return TestResult::Fail; }
...
pub(crate) fn test_agent_scheduler() -> TestResult {
    ...
    if repeat_id == 0 { return TestResult::Fail; }
    if repeat_id == id { return TestResult::Fail; }

    if !cancel_schedule(id) { return TestResult::Fail; }
    if cancel_schedule(id) { return TestResult::Fail; }
...
pub(crate) fn test_agent_presets() -> TestResult {
    ...
    if coding.temperature != 70 { return TestResult::Fail; }
    if coding.system_prompt.is_empty() { return TestResult::Fail; }

    let file_mgr = file_manager_agent();
    if file_mgr.system_prompt.is_empty() { return TestResult::Fail; }

    let monitor = system_monitor();
    if monitor.system_prompt.is_empty() { return TestResult::Fail; }

    let web = web_researcher();
    if web.max_tokens != 8192 { return TestResult::Fail; }

    let automator = task_automator();
    if automator.system_prompt.is_empty() { return TestResult::Fail; }
