...
fn test_message_construction() -> TestResult {
    ...
    if msg.sender() != 1 { return TestResult::Fail; }
    if msg.receiver() != 2 { return TestResult::Fail; }
    if msg.msg_type() != MessageType::Request { return TestResult::Fail; }
    if msg.payload() != b"hello" { return TestResult::Fail; }
...
fn test_message_types() -> TestResult {
    ...
    if request == response { return TestResult::Fail; }
    if response == notification { return TestResult::Fail; }
    if notification == request { return TestResult::Fail; }
...
fn test_channel_creation() -> TestResult {
    ...
    if channel_id == 0 { return TestResult::Fail; }

    let channel_id2 = create_channel(1, 3);
    if channel_id2 == 0 { return TestResult::Fail; }
    if channel_id == channel_id2 { return TestResult::Fail; }
...
fn test_channel_send_receive() -> TestResult {
    ...
    if !sent { return TestResult::Fail; }

    let received = receive(channel_id, 101);
    if received.is_none() { return TestResult::Fail; }

    let recv_msg = received.unwrap();
    if recv_msg.payload() != b"test data" { return TestResult::Fail; }
...
fn test_pipe_operations() -> TestResult {
    ...
    if read_end < 0 || write_end < 0 { return TestResult::Fail; }

    let data = b"pipe test";
    let written = pipe_write(write_end, data);
    if written != data.len() as isize { return TestResult::Fail; }

    let mut buf = [0u8; 32];
    let read = pipe_read(read_end, &mut buf);
    if read != data.len() as isize { return TestResult::Fail; }
    if &buf[..data.len()] != data { return TestResult::Fail; }
...
fn test_inbox_operations() -> TestResult {
    ...
    if count_after != 0 { return TestResult::Fail; }
