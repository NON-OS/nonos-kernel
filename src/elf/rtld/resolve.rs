...
fn lookup_in_object(obj: &super::load::LoadedObject, name: &str) -> Option<SymbolResolution> {
    if obj.dynamic == 0 { return None; }
    let dyn_info = parse_dynamic_section(obj.dynamic, obj.base);
    if dyn_info.symtab == 0 || dyn_info.strtab == 0 { return None; }
    if dyn_info.gnu_hash != 0 {
        return gnu_hash_lookup(dyn_info.gnu_hash, dyn_info.strtab, dyn_info.symtab, name, obj.base);
    }
    if dyn_info.sysv_hash != 0 {
        return sysv_hash_lookup(dyn_info.sysv_hash, dyn_info.strtab, dyn_info.symtab, name, obj.base);
    }
...
fn linear_symbol_search(symtab: usize, strtab: usize, name: &str, base: usize) -> Option<SymbolResolution> {
...
pub fn gnu_hash_lookup(hash_table: usize, strtab: usize, symtab: usize, name: &str, base: usize) -> Option<SymbolResolution> {
    ...
    let bloom_word = unsafe { *((bloom_ptr + ((hash as usize / 64) % bloom_size) * 8) as *const u64) };
    let mask = (1u64 << (hash % 64)) | (1u64 << ((hash >> bloom_shift) % 64));
    if (bloom_word & mask) != mask { return None; }
    ...
    loop {
        ...
        if (chain_val | 1) == (hash | 1) {
            ...
            if sym_name == name && sym.st_shndx != 0 {
                return Some(SymbolResolution {
                    name: String::from(name), address: base + sym.st_value as usize,
                    size: sym.st_size as usize, object_base: base,
                    bind: (sym.st_info >> 4) & 0xf, sym_type: sym.st_info & 0xf,
                });
            }
        }
...
fn gnu_hash(name: &str) -> u32 {
    let mut h: u32 = 5381;
    for c in name.bytes() { h = h.wrapping_mul(33).wrapping_add(c as u32); }
    h
}
