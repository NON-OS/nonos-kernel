...
pub enum LazyBindingState { Disabled, Enabled, Forced }

static LAZY_STATE: Mutex<LazyBindingState> = Mutex::new(LazyBindingState::Enabled);

pub fn get_lazy_state() -> LazyBindingState { *LAZY_STATE.lock() }

pub fn set_lazy_state(state: LazyBindingState) { *LAZY_STATE.lock() = state; }

pub fn lazy_bind(obj_base: usize, reloc_idx: usize) -> usize {
    let objects = super::load::get_loaded_objects();
    let obj = match objects.iter().find(|o| o.base == obj_base) { Some(o) => o, None => return 0 };
    ...
    unsafe { ptr::write(reloc_addr as *mut usize, sym_value); }
    sym_value
}

fn build_lazy_context(obj: &super::load::LoadedObject) -> LazyContext {
    LazyContext { base: obj.base, symtab: 0, strtab: 0, jmprel: 0, pltrelsz: 0 }
}

struct LazyContext { base: usize, symtab: usize, strtab: usize, jmprel: usize, pltrelsz: usize }
...
pub fn setup_plt_resolver(obj: &super::load::LoadedObject, got_plt: usize) {
    ...
    unsafe {
        ...
        ptr::write((got_plt + 16) as *mut usize, plt_resolver as usize);
    }
}
...
pub fn force_bind_now() {
    ...
    for obj in &objects {
        ...
        if ctx.jmprel != 0 && ctx.pltrelsz > 0 {
            ...
            for i in 0..count { lazy_bind(obj.base, i); }
        }
    }
}
