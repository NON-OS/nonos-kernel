...
impl LibraryManager {
    ...
    pub fn initialize(&mut self, id: usize) -> ElfResult<()> {
        ...
        if let Some(ref dynlink) = library.image.dynlink_info {
            ...
            if let Some((addr, size)) = dynlink.init_array { runner = runner.with_init_array(crate::elf::init::InitArrayInfo::new(addr, size)); }
            runner.run_all()?;
        }
    ...
    pub fn finalize(&mut self, id: usize) -> ElfResult<()> {
        ...
        if let Some(ref dynlink) = library.image.dynlink_info {
            ...
            if let Some((addr, size)) = dynlink.fini_array { runner = runner.with_fini_array(crate::elf::fini::FiniArrayInfo::new(addr, size)); }
    ...
    pub fn unload(&mut self, id: usize) -> ElfResult<()> {
        let should_unload = { let library = self.libraries.get_mut(&id).ok_or(ElfError::LibraryNotFound)?; library.release() };
        if should_unload {
            self.finalize(id)?;
            if let Some(library) = self.libraries.remove(&id) {
                self.name_index.remove(&library.name);
                if let Some(ref soname) = library.soname { self.soname_index.remove(soname); }
