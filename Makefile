...
ifeq ($(UNAME_S),Darwin)
	...
	cd $(ZK_CIRCUIT_DIR) && rm -rf target && RUSTFLAGS="" CARGO_UNSTABLE_BUILD_STD= $(CARGO) +nightly-2026-01-16 build --release --bin generate-keys --bin generate-proof --target x86_64-unknown-linux-gnu
endif
...
ifeq ($(UNAME_S),Darwin)
	...
	cd $(BOOTLOADER_DIR)/tools/embed-zk-proof && rm -rf target && RUSTFLAGS="" CARGO_UNSTABLE_BUILD_STD= $(CARGO) +nightly-2026-01-16 build --release --target x86_64-unknown-linux-gnu
...
esp: bootloader kernel sign-kernel embed-zk-proof
	...
	@cp $(TARGET_DIR)/x86_64-unknown-uefi/release/nonos_boot.efi $(ESP_DIR)/EFI/Boot/BOOTX64.EFI
