...
ifeq ($(UNAME_S),Darwin)
	...
	cd $(ZK_CIRCUIT_DIR) && RUSTFLAGS="" $(CARGO) build --release --bin generate-keys --bin generate-proof --target x86_64-unknown-linux-gnu
endif
...
ifeq ($(UNAME_S),Darwin)
	...
	cd $(BOOTLOADER_DIR)/tools/embed-zk-proof && RUSTFLAGS="" $(CARGO) build --release --target x86_64-unknown-linux-gnu
